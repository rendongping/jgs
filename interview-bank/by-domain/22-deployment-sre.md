...
> 本题库共收录 **91** 道面试题（基础 24 / 进阶 24 / 深入 22 / 架构 21）。
...
**题目描述**：
请解释负载均衡的概念，以及前端部署中如何利用负载均衡。

**参考答案**：

负载均衡（Load Balancing）是将流量分发到多台服务器的技术，避免单点过载，提升可用性和性能。

一、常见负载均衡器：

1. **硬件**：F5、A10。
2. **软件**：Nginx、HAProxy、Traefik。
3. **云 LB**：AWS ALB、阿里云 SLB、腾讯云 CLB。
4. **DNS 负载均衡**：按地域或权重返回不同 IP。

二、前端利用方式：

1. **静态资源 CDN**：
   - CDN 边缘节点本身就是负载均衡，将用户请求调度到最近节点。

2. **API 网关/BFF**：
   - 在网关层做负载均衡，将请求分发到多个 BFF 实例。

3. **多区域部署**：
   - 通过 DNS/GSLB 将用户请求导向最近或最健康的区域。
...
**口头回答版**：
> 负载均衡是把流量分到多台服务器，避免单点过载。前端可以通过 CDN、API 网关、多区域 DNS 来利用。算法有轮询、加权轮询、最少连接、IP Hash。负载均衡器可以是 Nginx、云 LB 或硬件 F5。

---

### FB-22-CO-B-011：什么是反向代理？
...
**题目描述**：
请解释反向代理的概念，以及它在部署中的作用。

**参考答案**：

反向代理（Reverse Proxy）位于客户端和后端服务器之间，客户端只与反向代理交互，由反向代理转发请求到后端服务器。

一、主要作用：

1. **负载均衡**：将请求分发到多个后端实例。
2. **SSL 终止**：在代理层处理 HTTPS，后端可用 HTTP。
3. **缓存**：缓存静态资源和 API 响应。
4. **安全**：隐藏后端服务器真实 IP，过滤恶意请求。
5. **压缩**：Gzip/Brotli 压缩响应。
6. **路由**：按路径/域名转发到不同服务。

二、常见工具：

- Nginx、Apache、Traefik、Caddy、Envoy。
...
**口头回答版**：
> 反向代理在客户端和后端之间，客户端只和它交互，它再转发到后端。作用有负载均衡、SSL 终止、缓存、安全、压缩、路由。常用 Nginx、Traefik、Envoy。和正向代理不同，正向代理是代理客户端，反向代理是代理服务器。

---

### FB-22-CO-B-012：什么是容器化部署？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：22 部署与 SRE
**标签**：容器化、Docker、Kubernetes、部署
**出现频率**：高频
**预计回答时长**：3-5 分钟

**题目描述**：
请解释容器化部署的概念，以及它对前端部署的价值。

**参考答案**：

容器化部署是将应用及其依赖打包到容器镜像中，通过容器运行时（如 Docker）在任何环境中一致地运行。

一、核心价值：

1. **环境一致性**：
   - 开发、测试、生产环境一致，避免“我本地能跑”。
...
1. **SSR 应用**：
   - Next.js、Nuxt.js 等需要 Node.js 运行时的应用适合容器化。

2. **BFF 服务**：
   - 前端配套的后端服务通常容器化部署。

3. **静态站点**：
   - 可以用 Nginx/Caddy 容器托管构建产物。

三、基本流程：

```
构建镜像 → 推送到镜像仓库 → Kubernetes/Docker 运行 → 流量接入
```

四、常用工具：

- Docker、containerd、Kubernetes、Helm。
...
**口头回答版**：
> 容器化是把应用和依赖打包成镜像，环境一致、可移植、能隔离资源、方便弹性伸缩。前端 SSR、BFF 服务、静态站点都可以容器化。流程是构建镜像、推仓库、K8s 运行、接流量。工具是 Docker、Kubernetes、Helm。

---

### FB-22-CO-B-013：什么是自动扩缩容？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：22 部署与 SRE
**标签**：自动扩缩容、HPA、Kubernetes、弹性
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请解释自动扩缩容的概念，以及前端服务如何利用自动扩缩容。

**参考答案**：

自动扩缩容（Auto Scaling）是根据负载自动增加或减少实例数量，保证服务能力和成本平衡。

一、类型：

1. **水平扩容（HPA）**：
   - 增加/减少 Pod/实例数量。
   - Kubernetes HPA 根据 CPU、内存、自定义指标扩缩。
...
**口头回答版**：
> 自动扩缩容是根据负载自动增减实例。Kubernetes 有 HPA 水平扩容、VPA 垂直扩容、Cluster Autoscaler 节点扩容。前端 SSR 和 BFF 可以根据 CPU、QPS、延迟扩缩。要设上下限，考虑冷启动，缩容要平滑。

---

### FB-22-CO-B-014：什么是服务发现？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：22 部署与 SRE
**标签**：服务发现、DNS、Kubernetes、微服务
**出现频率**：中频
**预计回答时长**：2-3 分钟

**题目描述**：
请解释服务发现的概念，以及它在部署架构中的作用。

**参考答案**：

服务发现是指在分布式系统中，服务实例能够自动注册自己的位置，并被其他服务或客户端找到。

一、为什么需要：

1. 容器和实例动态创建/销毁，IP 不固定。
2. 手动维护服务地址不可扩展。
3. 需要自动处理故障实例。
...
2. **服务端发现**：
   - 通过负载均衡器/网关统一转发。
   - 例如 Kubernetes Service + Ingress。

三、Kubernetes 中的服务发现：

1. **Service**：
   - 为 Pod 提供稳定的 ClusterIP 和 DNS 名称。
...
**口头回答版**：
> 服务发现是服务实例自动注册并被找到。因为容器 IP 不固定，需要自动维护地址。分客户端发现和服务端发现。Kubernetes 里用 Service 和 CoreDNS 做服务发现，给 Pod 稳定 DNS 名。前端 BFF 也通过 Service 被发现。

---

### FB-22-CO-A-018：如何设计前端灰度放量策略？
...
1. **网关层**：
   - Nginx/Envoy 根据 cookie/header 分流。

2. **CDN 层**：
   - 通过 Edge Logic 返回不同版本资源。
...
```nginx
# Nginx 按用户 ID hash 分流
split_clients $user_id $variant {
  1%    "v2";
  99%   "v1";
}
```
...
**常见错误**：
- 放量速度过快，异常时影响面大
- 没有按用户维度灰度，导致同一用户体验不一致

**口头回答版**：
> 灰度放量可以按百分比、用户属性、业务属性、时间。实现可以在网关层按 cookie/header 分流，CDN 层返回不同版本，配置中心动态控制。放量时监控错误率、性能、业务指标，异常回滚。Nginx 可以用 split_clients 按用户 ID hash 分流。

---

### FB-22-CO-A-019：如何设计前端回滚策略？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：高级 / 专家
**面试知识域**：22 部署与 SRE
**标签**：回滚、部署、CDN、版本化、SRE
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
**口头回答版**：
> 前端回滚触发条件有错误率突增、功能不可用、性能退化。策略包括版本回滚、配置回滚、流量切换、CDN 刷新。要实现自动或一键回滚。保留最近稳定版本 artifact，数据库变更要向后兼容。回滚后验证功能并复盘。

---

### FB-22-CO-A-020：什么是 Error Budget？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：高级 / 专家
**面试知识域**：22 部署与 SRE
**标签**：Error Budget、SLO、SRE、可靠性
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请解释 Error Budget 的概念，以及如何用它平衡可靠性和发布速度。

**参考答案**：

Error Budget（错误预算）是指在一定时间内允许服务出现故障的“预算”。它由 SLO 推导而来，用于量化可靠性目标。
...
例如 SLO 是 99.9%，则每月允许的不可用时间为：

```
30 天 × 24 小时 × 0.1% = 43.2 分钟
```
...
1. 定义 SLI（如可用性、错误率、延迟）。
2. 设定 SLO（如 99.9% 可用性）。
3. 监控 Error Budget 消耗速度。
4. 预算耗尽时触发发布冻结或降级。

四、注意事项：

1. SLO 不要追求 100%，成本极高。
2. Error Budget 要按服务或页面维度拆分。
3. 预算消耗过快时要及时告警。
...
**常见错误**：
- 把所有服务都设成 99.999% SLO
- 不监控 Error Budget 消耗情况

**口头回答版**：
> Error Budget 是根据 SLO 算出来的允许故障预算，比如 99.9% 可用性意味着每月允许 43.2 分钟不可用。它用来平衡发布速度和稳定性，预算足时加快发布，预算耗尽时暂停发布修稳定。要先定义 SLI、设 SLO，监控消耗。

---

### FB-22-CO-A-021：如何建立前端性能基线？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：高级 / 专家
**面试知识域**：22 部署与 SRE
**标签**：性能基线、Web Vitals、Lighthouse、监控
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明如何为前端应用建立性能基线，并用于发布回归检测。
...
1. Lighthouse CI 集成到 PR。
2. 真实用户监控（RUM）持续采集。
3. 实验室测试（Lab Data）定期跑测。
...
**常见错误**：
- 只关注 Lighthouse 分数，忽略真实用户数据
- 基线建立后从不更新

**口头回答版**：
> 前端性能基线要选 Web Vitals 和业务指标，在代表页面和设备上多次测量，记录 P50/P75/P95。用 Lighthouse CI 和 RUM 自动采集。发布时对比基线，超阈值告警或阻塞。定期更新基线，识别瓶颈。

---

### FB-22-CO-A-022：什么是 Toil？SRE 如何减少 Toil？
...
**题目描述**：
请解释 SRE 中的 Toil 概念，以及如何减少 Toil。

**参考答案**：

Toil 是指重复性、手动性、可自动化的运维工作，它没有长期价值，但会消耗工程师时间。
...
- SRE 团队应将 50% 以上时间投入到工程化工作，而非 Toil。
...
**口头回答版**：
> Toil 是重复、手动、可自动化的运维工作，比如手动重启、清日志、扩容。SRE 要通过自动化、自助服务、标准化、平台化来减少 Toil。目标是让 SRE 把一半以上时间投入工程化，而不是重复劳动。

---

### FB-22-EN-P-026：前端容器化部署最佳实践

**题型**：工程化题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：22 部署与 SRE
**标签**：前端、容器化、Docker、Kubernetes、最佳实践
**出现频率**：中频
**预计回答时长**：8-12 分钟
...
1. 构建时注入 public path、API base。
2. 运行时通过 envsubst 替换 Nginx 配置中的变量。
3. 敏感信息通过 Secret 挂载。

四、健康检查：

1. 提供 /health 端点。
2. K8s liveness/readiness probe 配置合理。

五、可观测性：

1. Nginx access log 结构化。
2. 容器指标采集。
3. 错误日志上报。
...
**常见错误**：
- 镜像包含源码和 devDependencies
- Nginx 配置没有处理前端路由

**口头回答版**：
> 前端容器化要多阶段构建，用 alpine，非 root 运行。Nginx 配 try_files 处理前端路由，静态资源长期缓存。运行时通过 envsubst 注入环境变量。提供 health 接口给 K8s 探针，结构化日志，配 CSP 这些安全头。

---

### FB-22-SC-P-002：SSR 前端部署策略

**题型**：场景设计题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：22 部署与 SRE
**标签**：SSR、部署、Next.js、Nuxt、灰度、回滚
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
请设计一个 SSR 前端应用的部署策略，支持灰度、回滚和降级。
...
```
CDN / LB → Ingress → K8s Service → SSR Pod
                ↓
            降级静态站点
```
...
1. 按流量百分比灰度。
2. 按用户属性灰度。
3. 通过 Ingress 权重或 Service 多版本分发。
...
**口头回答版**：
> SSR 部署用 K8s Rolling Update，前面接 CDN/LB 和 Ingress。灰度按流量或用户属性，回滚保留旧镜像一键切。SSR 异常时要有静态降级页面。页面和 API 数据加缓存，注意失效策略。

---

### FB-22-PE-P-002：前端高可用灰度发布性能优化

**题型**：性能题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：22 部署与 SRE
**标签**：前端、灰度发布、高可用、性能、CDN
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
请说明如何优化前端高可用灰度发布过程中的性能瓶颈。
...
1. **版本化静态资源**：
   - JS/CSS 文件名加 hash，长期缓存。
   - HTML 不缓存或短缓存。

2. **灰度一致性**：
   - 同一用户始终访问同一版本，避免资源混用。
   - 使用 sticky session 或用户 ID hash。
...
4. **后端分流优化**：
   - 使用边缘计算或网关层做灰度判断，减少回源。

5. **监控与阈值**：
   - 实时监控灰度用户的 LCP、INP、CLS。
   - 性能退化超阈值自动回滚。
...
**口头回答版**：
> 灰度发布性能瓶颈有资源混用、后端压力、灰度逻辑延迟、CDN 缓存。优化要版本化静态资源长期缓存，保证同一用户始终同一版本，CDN 预热灰度资源，网关层做灰度判断。监控灰度用户 Web Vitals，退化超阈值自动回滚。预发布环境先跑性能测试。

---

### FB-22-CP-P-023：前端故障发现与止损

**题型**：综合开放题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：22 部署与 SRE
**标签**：前端、故障发现、止损、Incident Response、SRE
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
请结合高可用架构和告警体系，谈谈前端服务的故障发现与止损实践。
...
1. **实时监控**：
   - RUM（真实用户监控）采集 Web Vitals、JS 错误、API 失败。
   - 服务端监控 QPS、错误率、延迟。
...
**口头回答版**：
> 前端故障发现靠 RUM 监控 Web Vitals 和 JS 错误，服务端监控 QPS、错误率、延迟。告警设错误率、P99 延迟、核心流程成功率阈值。止损先回滚、降级、限流、关 Feature Toggle、切流量，先恢复再排查。事后复盘更新 Runbook。

---

### FB-22-CO-P-024：Kubernetes Ingress 流量切分

**题型**：概念题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：22 部署与 SRE
**标签**：Kubernetes、Ingress、流量切分、灰度、金丝雀
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明 Kubernetes Ingress 如何实现流量切分，以及在前端容器化部署中的应用。
...
1. 部署两个版本的 Deployment（v1 和 v2）。
2. 创建两个 Service 指向不同版本。
3. Ingress 根据权重或 header 将流量分发到不同 Service。

三、注意事项：

1. 灰度期间监控 v2 的错误率和性能。
2. 保证同一用户的会话一致性（sticky session）。
3. 回滚时快速调整权重到 0%。

四、高级方案：

- 使用 Flagger、Argo Rollouts 实现自动金丝雀发布。
- 结合 Prometheus 指标自动推进或回滚。
...
**常见错误**：
- 灰度时没做 session 一致性导致用户体验不一致
- 回滚时只改 Deployment 不改 Ingress 权重

**口头回答版**：
> K8s Ingress 可以用 Nginx canary 注解按权重、header、cookie 切分流量。前端容器化部署时，v1 和 v2 各一个 Deployment 和 Service，Ingress 分发流量。灰度时监控 v2 指标，回滚把权重调到 0。高级方案用 Flagger、Argo Rollouts 自动金丝雀。

---

### FB-22-SD-R-033：如何设计支持多区域部署的 SSG 前端系统？

**题型**：系统设计题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：22 部署与 SRE
**标签**：SSG、多区域、CDN、数据合规、部署
**出现频率**：低频
**预计回答时长**：10-15 分钟

**题目描述**：
请设计一个支持多区域部署的 SSG 前端系统，兼顾数据合规和访问延迟。
...
```
用户 → DNS/GSLB → 区域 CDN → 区域存储（S3/OSS） → 构建流水线
```

二、区域划分：

1. 按法律合规要求划分，如中国、欧盟、美国。
2. 每个区域独立部署一套 SSG 产物。
...
1. 敏感数据不跨区域传输。
2. 每个区域使用独立 CMS/数据源。
3. 符合 GDPR、个人信息保护法等法规。

五、访问延迟：

1. 用户通过 DNS 解析到最近区域。
2. CDN 边缘缓存静态页面。
3. 增量静态再生（ISR）减少全量构建时间。
...
**口头回答版**：
> 多区域 SSG 系统按合规要求划分区域，每个区域独立构建和部署。用户通过 DNS 到最近区域，CDN 缓存静态页面。构建时按区域拉取数据源，符合 GDPR 等法规。用 ISR 减少构建时间，保留区域历史版本支持回滚，监控各区域可用性。

---

### FB-22-CP-R-032：前端自动化发布风险控制

**题型**：综合开放题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：22 部署与 SRE
**标签**：前端、自动化发布、风险控制、质量门禁、金丝雀
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
请结合质量门禁和金丝雀发布，谈谈如何构建前端自动化发布风险控制体系。
...
**口头回答版**：
> 前端自动化发布风险控制要有质量门禁：lint、typecheck、测试、Lighthouse、包体积、安全扫描。发布用金丝雀，按百分比放量，每个阶段监控指标，异常自动回滚。灰度先内部再外部，避开高峰。发布即代码，失败复盘不追责。

---

### FB-22-CO-R-002：自动扩容与 Web Vitals 的关系

**题型**：概念题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：22 部署与 SRE
**标签**：自动扩容、Web Vitals、性能、SRE
**出现频率**：低频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明自动扩容与前端 Web Vitals 性能指标的核心区别与联系。
...
|------|----------|------------|
| 关注点 | 服务端资源容量 | 用户体验 |
| 指标 | CPU、内存、QPS | LCP、INP、CLS |
| 触发 | 资源使用超阈值 | 页面加载和交互质量 |
| 作用 | 保证服务不宕机 | 衡量用户体验好坏 |

二、联系：

1. **扩容可以缓解性能问题**：
   - 当 QPS 过高导致响应慢时，扩容可以降低延迟，改善 INP。

2. **性能问题不一定需要扩容**：
   - LCP 差可能是因为图片太大、JS 阻塞，扩容不一定能解决。

3. **联动优化**：
   - 监控 Web Vitals 发现性能下降。
   - 分析是否是服务端压力导致，再决定是否扩容。

三、实践建议：

1. 不要把自动扩容当作唯一性能优化手段。
2. 结合 RUM 数据和资源监控综合判断。
3. 前端优化（代码分割、资源压缩、缓存）和服务端扩容并重。
...
**口头回答版**：
> 自动扩容关注服务端资源容量，Web Vitals 关注用户体验。扩容可以缓解高并发导致的响应慢，但 LCP 差可能是因为资源加载问题，扩容不一定解决。要结合 RUM 和资源监控综合判断，前端优化和扩容并重。

---

### FB-22-SC-R-002：Kubernetes 前端 Error Budget 与发布节奏

**题型**：场景设计题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：22 部署与 SRE
**标签**：Kubernetes、Error Budget、SRE、发布节奏、前端
**出现频率**：低频
**预计回答时长**：8-12 分钟

**题目描述**：
请说明在 Kubernetes 容器化场景下，如何为前端服务设计 Error Budget 与发布节奏。
...
2. 用 Grafana 展示 Error Budget 消耗趋势。
3. HPA 根据负载自动扩容，减少因过载导致的预算消耗。
4. PodDisruptionBudget 保证升级时有足够可用实例。

四、发布策略：

1. 工作日白天发布，保留 on-call。
2. 重大变更走金丝雀。
3. 回滚时间目标 < 5 分钟。
...
**常见错误**：
- SLO 设定过高，导致没有创新空间
- Error Budget 消耗不监控，失去指导意义

**口头回答版**：
> K8s 前端服务 Error Budget 要先定义 SLI 和 SLO，比如可用性 99.9%、P99 延迟 500ms。预算充足时正常发布，紧张时加强门禁延长灰度，耗尽时发布冻结。用 Prometheus + Grafana 监控预算消耗，HPA 自动扩容，PDB 保证升级可用性。

---

### FB-22-PE-R-002：如何排查并优化前端高可用灰度发布中的性能瓶颈？

**题型**：性能题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：22 部署与 SRE
**标签**：前端、灰度发布、性能瓶颈、高可用、优化
**出现频率**：低频
**预计回答时长**：8-12 分钟

**题目描述**：
请说明如何排查并优化前端高可用灰度发布过程中的性能瓶颈。
...
1. **监控数据**：
   - 对比灰度用户和非灰度用户的 Web Vitals。
   - 分析 P99 延迟、错误率、资源加载时间。

2. **链路追踪**：
   - 追踪请求从 CDN → Ingress → SSR/BFF → 后端的全链路耗时。

3. **资源分析**：
   - 检查新版本是否有大体积资源、未压缩资源、阻塞渲染。

4. **日志分析**：
   - 聚合 JS 错误、API 超时、CDN 回源日志。
...
**口头回答版**：
> 排查灰度性能瓶颈要对比灰度和非灰度用户的 Web Vitals，做全链路追踪，分析资源和日志。常见瓶颈有大资源、SSR 慢、API 超时、CDN 未命中。优化要代码分割、SSR 缓存、CDN 预热、边缘灰度判断。建立性能基线，退化自动回滚。
