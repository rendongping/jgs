...
> 本题库共收录 **79** 道面试题（基础 16 / 进阶 28 / 深入 23 / 架构 12）。
...
1. 解决跨组件共享状态、避免 prop drilling
2. 统一数据流向，提升可预测性和可调试性
3. 常见场景：用户态、主题、购物车、表单、路由参数
4. 区分 UI 状态与远端状态
5. 状态应集中必要、分散合理
...
**参考资源**：
- [Redux Style Guide](https://redux.js.org/style-guide)
- [React State Management](https://react.dev/learn/thinking-in-react)
...
**岗位层级**：中级 / 高级
**面试知识域**：29 数据与状态
**标签**：Redux、MobX、响应式、不可变、状态管理
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请比较 Redux 和 MobX 在数据模型、更新方式和适用场景上的差异。

**参考答案**：

核心思路如下：

1. Redux：单一不可变状态树，显式 action + reducer，适合复杂协作
2. MobX：响应式对象，自动追踪依赖，更新隐式，适合快速开发
3. Redux 强调可预测和调试；MobX 强调简洁和自动化
4. Redux 模板多但生态强；MobX 学习曲线低但调试需理解依赖
5. 选型：团队熟悉度、应用规模、可维护性要求
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：状态、局部状态、全局状态、Context、提升
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明哪些状态应该全局管理，哪些应该留在组件局部。

**参考答案**：

核心思路如下：

1. 全局状态：跨页面/模块共享、用户会话、主题、权限、购物车
2. 局部状态：表单草稿、UI 展开收起、纯组件内部数据
3. 远端状态：优先交给专门数据层管理
4. Context 适合低频更新、作用域明确的状态
5. 避免为了共享而过度提升状态
...
1. 区分服务端状态与客户端状态
2. 使用 SWR/React Query/TanStack Query 管理远端状态
3. 设置缓存时间、stale-while-revalidate、重新获取策略
4. 基于 key 做缓存，mutation 后主动失效或乐观更新
5. 离线场景：持久化缓存 + 重试
...
在实际落地 设计前端数据缓存策略 时，建议结合 数据缓存、SWR、React Query 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：架构师 / 专家
**面试知识域**：29 数据与状态
**标签**：数据同步、状态管理、乐观更新、冲突、离线
**出现频率**：中频
**预计回答时长**：15-30 分钟

**题目描述**：
请为一个多端协作应用设计数据同步与状态管理架构。

**参考答案**：

核心思路如下：

1. 分层：本地状态、远端状态、同步层、持久化层
2. 乐观更新：本地先更新，失败回滚
3. 冲突处理：最后写入获胜、版本向量、业务合并策略
4. 离线支持：操作队列 + 幂等 key
5. 实时同步：WebSocket/SSE 推送变更
6. 状态可观测：日志、回放、时间旅行
...
在实际落地 设计一个前端数据同步与状态管理方案 时，建议结合 数据同步、状态管理、乐观更新 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：中级 / 高级
**面试知识域**：29 数据与状态
**标签**：CQRS、Event Sourcing、命令、查询、事件
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请解释 CQRS 和 Event Sourcing，并说明前端如何配合。

**参考答案**：

核心思路如下：

1. CQRS：命令和查询分离，读写模型不同
2. Event Sourcing：以事件序列代替当前状态，状态可重放
3. 前端配合：命令通过 API 发出，查询使用 BFF 聚合视图
4. 事件驱动 UI：订阅事件流更新局部状态
5. 适合审计、复杂业务、实时协作
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：表单、状态、React Hook Form、Formik、校验
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. 字段状态：value、 touched、error、dirty
2. 使用 React Hook Form / Formik / Formily 等库抽象
3. 表单级与字段级校验分离
4. 动态表单：schema 驱动渲染与校验
5. 提交状态：loading、success、error、retry
...
在实际落地 处理前端表单复杂状态 时，建议结合 表单、状态、React Hook Form 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：乐观更新、悲观更新、一致性、回滚、体验
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请比较乐观更新和悲观更新，并说明各自适用场景。

**参考答案**：

核心思路如下：

1. 悲观更新：等服务端返回成功后再更新 UI，安全但慢
2. 乐观更新：先更新 UI 再请求，体验好但需处理失败
3. 适用：点赞、收藏、购物车可用乐观；支付、下单用悲观
4. 失败处理：回滚状态、提示用户、重试
5. 幂等是乐观更新的基础
...
**岗位层级**：中级 / 高级
**面试知识域**：29 数据与状态
**标签**：Context、状态管理、依赖注入、作用域
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请设计一个可在组件树中共享、支持作用域隔离的数据源方案。

**参考答案**：

核心思路如下：

1. 使用 Context + useReducer 提供作用域内状态
2. 状态提升 + props 传递适合小范围
3. 依赖注入容器：提供可替换的实现
4. 按业务模块拆分多个 Context，避免全局单 store
5. 状态变更通过 reducer/action，保持可预测
...
在实际落地 设计跨组件共享的数据源 时，建议结合 Context、状态管理、依赖注入 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
1. 把嵌套数据拆分为独立实体表，通过 ID 引用
2. 避免数据重复，更新一处全局一致
3. 便于查询、分页、缓存、乐观更新
4. 工具：Redux Toolkit createEntityAdapter、Normalizr
5. 适合列表、关联数据、评论等场景
...
**岗位层级**：中级 / 高级
**面试知识域**：29 数据与状态
**标签**：状态管理、性能、选择器、memo、订阅
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明如何优化 Redux/Zustand 等状态管理库的性能。

**参考答案**：

核心思路如下：

1. 使用选择器只订阅需要的切片，避免全量渲染
2. 拆分 store 或按模块组织
3. 状态更新保持不可变，避免不必要变更
4. 使用 Reselect 缓存派生状态
5. 对大型列表做分页或虚拟化
...
在实际落地 状态管理库的性能优化 时，建议结合 状态管理、性能、选择器 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**题目描述**：
请设计前端权限数据与路由、菜单、按钮的联动方案。

**参考答案**：

核心思路如下：

1. 权限数据作为远端状态，登录后拉取并缓存
2. 路由守卫根据权限过滤路由和菜单
3. 按钮/字段级通过高阶组件或 hook 判断
4. 权限变更时重新计算可见性
5. 敏感操作服务端鉴权，前端仅做体验控制
...
在实际落地 设计前端权限状态与路由联动 时，建议结合 权限、状态、路由 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：派生状态、selector、computed、状态
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明派生状态应放在哪里计算，如何避免重复。
...
1. 派生状态可由原始状态计算得出
2. 计算位置：组件内 useMemo、selector、store computed
3. 多次使用应缓存，如 Reselect、MobX computed
4. 避免把派生状态也存入 store 导致同步问题
5. 复杂派生可在 BFF 计算后返回
...
1. 使用命令模式：每个操作封装为 do/undo
2. 维护历史栈：past、present、future
3. 状态不可变，undo 时恢复上一个快照
4. 限制历史长度，避免内存无限增长
5. 支持分支场景：合并、冲突处理
...
在实际落地 设计一个支持 Undo/Redo 的前端状态系统 时，建议结合 Undo、Redo、状态历史 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
1. 分页状态：page、pageSize、total、items
2. 无限滚动：游标/cursor、hasMore、items
3. 数据合并：新页数据追加到列表，避免替换
4. 缓存：按查询条件 key 缓存分页结果
5. 刷新：下拉刷新重置状态，上拉加载更多
...
在实际落地 处理前端分页与无限滚动状态 时，建议结合 分页、无限滚动、状态 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：初级 / 高级
**面试知识域**：29 数据与状态
**标签**：React、useState、useReducer、状态
**出现频率**：高频
**预计回答时长**：3-5 分钟

**题目描述**：
请说明 useState 和 useReducer 的使用场景。
...
1. useState 适合简单独立状态
2. useReducer 适合状态相互依赖、转换逻辑复杂
3. reducer 便于测试和集中管理状态变更
4. 多个相关状态可用 reducer 统一处理
5. 复杂表单、多步骤流程适合 useReducer
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：多标签页、BroadcastChannel、localStorage、状态同步
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. BroadcastChannel 同源跨标签页通信
2. localStorage/storage 事件做兼容降级
3. 同步范围：登录态、主题、语言、购物车
4. 冲突处理：服务端权威版本、时间戳
5. 注意隐私和安全，避免同步敏感数据
...
在实际落地 前端如何处理多标签页状态同步 时，建议结合 多标签页、BroadcastChannel、localStorage 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：时间旅行、Redux DevTools、状态回放、调试
**出现频率**：低频
**预计回答时长**：3-5 分钟
...
1. 记录所有状态变更 action 序列
2. 可以回到任意历史状态查看 UI
3. 基于不可变状态实现快照回放
4. 价值：定位 bug、复现问题、理解数据流
5. Redux DevTools、Vue DevTools 支持
...
**岗位层级**：架构师 / 专家
**面试知识域**：29 数据与状态
**标签**：实时协作、CRDT、OT、状态同步、冲突
**出现频率**：低频
**预计回答时长**：15-30 分钟

**题目描述**：
请设计类似在线文档的实时协作前端状态系统。

**参考答案**：

核心思路如下：

1. 选择 OT 或 CRDT 作为冲突解决算法
2. 操作本地先执行，再同步给其他用户
3. 使用 WebSocket 或 WebRTC 传输操作
4. 版本向量或 Lamport 时钟定序
5. 离线支持：本地操作队列，联网后合并
6. Undo 需考虑远程操作影响
...
在实际落地 设计一个前端实时协作的状态系统 时，建议结合 实时协作、CRDT、OT 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：中级 / 高级
**面试知识域**：29 数据与状态
**标签**：Zustand、Redux Toolkit、状态管理、轻量
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请比较 Zustand 和 Redux Toolkit，说明各自适合的场景。

**参考答案**：

核心思路如下：

1. Zustand：轻量、无样板、适合小到中型应用
2. Redux Toolkit：生态强、DevTools、中间件、适合大型复杂应用
3. Zustand 支持多 store，Redux 通常单 store
4. RTK Query 内置数据获取与缓存
5. 根据团队规模和需求选型
...
2. 虚拟滚动只渲染可视行
3. 单元格编辑状态局部管理
4. 筛选排序在后端或 Web Worker
5. 选中状态用 Set 存储行 ID
6. 避免全表 re-render，使用 memo
...
在实际落地 前端大数据表格的状态与渲染优化 时，建议结合 大数据、表格、状态 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：副作用、异步、Redux Saga、Thunk、数据流
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 副作用与纯状态更新分离
2. Redux Thunk：简单异步逻辑
3. Redux Saga：复杂流程、竞态处理
4. RTK Query：封装数据获取、缓存、失效
5. 组件层：useEffect + 数据层库
...
**岗位层级**：中级 / 高级
**面试知识域**：29 数据与状态
**标签**：搜索、筛选、URL、状态同步、history
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请设计列表页的搜索筛选状态，支持刷新不丢失和分享。

**参考答案**：

核心思路如下：

1. 筛选条件同步到 URL query，刷新/分享可恢复
2. 状态对象与 URL 互相映射
3. 防抖处理输入，避免频繁请求
4. 重置按钮清空本地和 URL 状态
5. 服务端根据 query 返回结果
...
在实际落地 设计前端搜索筛选状态 时，建议结合 搜索、筛选、URL 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
1. 不可变：创建新对象而不是修改原对象
2. 优势：可预测、可回放、便于浅比较
3. React/Vue 依赖不可变或响应式追踪变化
4. 工具：Immer、Immutable.js、展开运算符
5. 注意：深层拷贝性能开销，使用结构共享
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：表单、配置化、Schema、状态引擎、动态
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请设计一个通过 JSON Schema 驱动表单渲染和状态管理的引擎。

**参考答案**：

核心思路如下：

1. 用 JSON Schema 统一描述字段：类型、默认值、校验规则、联动条件与布局权重，避免字段定义散落在 UI 组件里。
2. 状态引擎按字段路径（如 `user.profile.name`）构建嵌套状态树，支持数组、对象与动态增删；变更通过不可变更新触发订阅。
3. 联动层采用依赖图或事件总线：当字段 A 变化时，根据规则重新计算字段 B 的显隐、可编辑态、候选值与校验规则。
4. 校验分层执行：输入即时校验、失焦校验、提交前全量校验，并支持异步服务端校验；错误信息按字段路径回写。
5. 渲染层通过组件映射表将 Schema 映射为具体表单控件，支持自定义组件、自定义校验器和自定义布局注册。
6. 性能与可扩展：大数据表单启用虚拟滚动/分片渲染，联动计算使用 memoization；提供插件机制与运行时调试面板。
...
在实际落地 设计一个前端配置化表单状态引擎 时，建议结合 表单、配置化、Schema 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
1. 本地状态：商品列表、数量、选中、价格计算
2. 持久化：localStorage/IndexedDB，登录后同步到服务端
3. 数量加减可乐观更新，失败回滚
4. 价格等敏感信息以服务端为准
5. 未登录与登录状态平滑迁移
...
在实际落地 前端购物车状态如何设计 时，建议结合 购物车、状态、持久化 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：高级 / 资深
**面试知识域**：29 数据与状态
**标签**：selector、Reselect、派生状态、缓存
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明 selector 模式的作用和最佳实践。

**参考答案**：

核心思路如下：

1. selector 从 store 中派生数据
2. 可组合：简单 selector 组合成复杂 selector
3. 缓存：输入不变输出不变，避免重复计算
4. 解耦组件与 store 结构
5. Reselect、Zustand selector、Vue getter 都是实现
...
**岗位层级**：中级 / 高级
**面试知识域**：29 数据与状态
**标签**：状态、UI、渲染、调试、React
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请给出状态更新后组件未重新渲染的排查思路。

**参考答案**：

核心思路如下：

1. 确认状态确实已更新，检查 DevTools
2. 检查是否直接修改了原状态
3. 检查 selector 是否返回相同引用
4. 检查组件是否被 memo 且 props 未变
5. 检查状态是否更新到了正确的作用域
6. 使用日志或时间旅行回放定位
...
在实际落地 状态更新后 UI 未更新如何排查 时，建议结合 状态、UI、渲染 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
1. 数据只能从父到子、从 state 到 view，通过 action 改变 state
2. 代表：Flux、Redux、Vuex
3. 优势：数据流向清晰、易于追踪、调试
4. 与双向绑定对比：更可控但代码更多
5. 现代框架推荐单向数据流为主
...
1. 根据场景选 SSE 或 WebSocket
2. 连接管理：重连、心跳、断线提示
3. 推送事件映射到 store action 更新状态
4. 消息去重与顺序处理
5. 避免高频消息导致频繁渲染：节流/批量
...
在实际落地 前端如何处理服务端推送状态 时，建议结合 SSE、WebSocket、推送 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
**岗位层级**：架构师 / 专家
**面试知识域**：29 数据与状态
**标签**：跨页面、状态共享、Storage、BroadcastChannel、同步
**出现频率**：低频
**预计回答时长**：15-30 分钟

**题目描述**：
请设计一个支持多页面/子应用共享的状态层。
...
1. 核心状态抽象为可序列化对象
2. 存储介质：IndexedDB/localStorage/BroadcastChannel
3. 状态变更走统一 API，自动同步到存储和其他页面
4. 冲突解决策略：时间戳、版本号、服务端仲裁
5. 权限与隐私：敏感状态不共享
6. API 设计：订阅、读取、写入、清除
...
在实际落地 设计一个跨页面共享的状态层 时，建议结合 跨页面、状态共享、Storage 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
1. reducer 保持纯函数，不直接发起请求
2. 副作用集中在 middleware/saga/effect
3. 组件层使用自定义 hook 封装副作用
4. 异步取消和竞态处理
5. 测试时副作用可 mock
...
在实际落地 设计前端搜索状态与历史记录 时，建议结合 搜索、历史、状态 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
...
1. 多个子组件需要共享状态时，把状态放到共同父组件
2. 子组件通过 props 接收值和回调
3. 适合层级不深、共享范围小的场景
4. 过深时用 Context 或状态管理库
5. 保持单向数据流
...
1. React 18 自动批处理多个 setState
2. 减少渲染次数，提升性能
3. flushSync 可强制同步更新但慎用
4. 在事件处理、异步代码中行为不同
5. 合并相关状态更新
...
在实际落地 状态更新批处理与性能 时，建议结合 批处理、setState、调度 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
