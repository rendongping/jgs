...
> 本题库共收录 **69** 道面试题（基础 21 / 进阶 28 / 深入 13 / 架构 7）。
...
**口头回答版**：
> 语义化 HTML 是指使用恰当的 HTML 标签来表达内容的结构和含义，而不仅仅是为了展示效果。 例如用 `<header>`、`<nav>`、`<main>`、`<article>`、`<section>`、`<footer>` 等标签明确页面结构，而不是全部使用 `<div>` 和 `<span>`。 可访问性：屏幕阅读器可以更好地解析页面结构，帮助视障用户理解内容。 SEO：搜索引擎通过语义标签理解页面内容的重要性和层次，有助于排名。
...
**口头回答版**：
> CSS 盒模型由四部分组成： Content：内容区域，显示文本和图像。 Padding：内边距，内容与边框之间的空间。 Border：边框。
...
**口头回答版**：
> - 类型（元素）选择器：div、p - 类选择器：.class - ID 选择器：#id - 属性选择器：[type="text"]、[class^="btn"]
...
**口头回答版**：
> display 控制元素的盒类型和布局行为。 - block：块级元素，独占一行，可设置宽高，默认宽度填满父容器。 - inline：行内元素，不独占一行，宽高由内容决定，设置 width/height 无效，margin-top/bottom 多数情况下不生效。 - inline-block：行内块元素，对外表现为行内（可并排），对内可设置宽高和 margin/padding。
...
**口头回答版**：
> CSS 继承是指某些属性值会自动从父元素传递给子元素，无需显式设置。 继承是层叠（cascade）的一部分。 会继承的属性（通常与文本相关）： - 字体：font-family、font-size、font-weight、line-height
...
**口头回答版**：
> position 用于指定元素的定位方式： - static：默认值，元素处于正常文档流，不受 top/right/bottom/left 和 z-index 影响。 - relative：相对定位，元素仍在文档流中占据原位置，根据原位置偏移，不脱离文档流。 - absolute：绝对定位，元素脱离文档流，相对于最近的已定位祖先元素（非 static）定位；若无则相对于初始包含块（通常是 viewport）。
...
**口头回答版**：
> Flexbox（弹性盒布局）是一种一维布局模型，用于在容器内高效地分配、对齐空间，适合处理沿单一方向（水平或垂直）排列的项目。 容器属性（flex container）： - display: flex：启用 flex 布局。 - flex-direction：主轴方向，如 row（默认）、row-reverse、column、column-reverse。
...
**口头回答版**：
> CSS Grid 是一种二维布局模型，可以同时处理行和列，适合复杂的网页整体布局。 Flexbox 是一维布局模型，主要处理单行或单列的项目排列。 | 特性 | Flexbox | CSS Grid | |------|---------|----------|
...
**口头回答版**：
> 媒体查询（Media Query）允许根据设备特性（如视口宽度、屏幕分辨率、方向等）应用不同的 CSS 样式，是实现响应式设计的核心技术。 - 手机：< 768px - 平板：768px - 1023px - 桌面：1024px - 1279px
...
**口头回答版**：
> 外边距折叠是指两个或多个垂直相邻的块级元素的外边距（margin）合并为一个外边距，最终取值取其中最大的那个，而不是相加。 相邻兄弟元素：上方元素的 margin-bottom 与下方元素的 margin-top 折叠。 父子元素：子元素的 margin-top / margin-bottom 与父元素的对应外边距折叠（父元素无 border/padding/inline content/BFC 分隔时）。 空块元素：自身 margin-top 和 margin-bottom 折叠。
...
**口头回答版**：
> BFC 是 CSS 视觉格式化模型中的一个独立渲染区域，区域内的元素按照特定规则布局，且与外部元素相互隔离。 内部块级盒子在垂直方向一个接一个放置。 垂直方向 margin 会在同一个 BFC 内发生折叠。 BFC 区域不会与浮动元素重叠。
...
**口头回答版**：
> CSS 自定义属性（又称 CSS 变量）允许开发者定义可在样式表中复用的值，以 -- 开头，通过 var() 函数引用。 可维护性：一处修改，全局生效。 作用域：可在 :root、组件级或媒体查询中定义，支持级联覆盖。 动态性：可通过 JavaScript 读写，适合主题切换。
...
**口头回答版**：
> | 特性 | transition | animation | |------|------------|-----------| | 触发方式 | 需要状态变化触发（如 hover、class 切换） | 自动播放，可通过关键帧控制 | | 复杂度 | 两个状态之间的过渡 | 可定义多个关键帧，控制复杂动画 |

---
### FB-06-CO-B-017：DOCTYPE 的作用是什么？省略它会发生什么？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级
**面试知识域**：06 HTML/CSS
**标签**：DOCTYPE、渲染模式、HTML、怪异模式
**出现频率**：高频
**预计回答时长**：2-3 分钟
...
在实际落地 DOCTYPE 的作用是什么省略它会发生什么 时，建议结合 DOCTYPE、渲染模式、HTML 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 声明作用（50%）：指定渲染模式
- 标准模式（25%）：按规范解析
- 怪异模式（25%）：旧布局兼容性表现
...
### FB-06-CO-B-018：HTML5 新增了哪些结构化标签？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级
**面试知识域**：06 HTML/CSS
**标签**：HTML5、语义化、header、main、article
**出现频率**：高频
**预计回答时长**：3-5 分钟
...
### FB-06-CO-B-019：meta viewport 有哪些常见属性及作用？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级
**面试知识域**：06 HTML/CSS
**标签**：viewport、移动端、响应式、meta
**出现频率**：高频
**预计回答时长**：3-5 分钟
...
**评分维度**：
- width（30%）：视口宽度
- initial-scale（30%）：初始缩放
- 缩放限制（20%）：max/min scale
- 可访问性（20%）：user-scalable=no 的问题
...
**口头回答版**：
> 最终 B（蓝色）可能显示在 A（红色）上层。 z-index 只对已定位元素（position 不为 static）和 flex/grid 子元素生效。 更重要的是，.parent 设置了 position: relative 和 z-index: 1，会各自创建独立的层叠上下文。 .child-a 的 z-index: 999 和 .child-b 的 z-index: 2 只在各自的父层叠上下文中比较。
...
**口头回答版**：
> 层叠上下文是 HTML 元素在三维空间（z 轴）上的渲染顺序概念。 同一个层叠上下文内的元素按一定规则（如 z-index、出现顺序）堆叠；不同层叠上下文之间，父元素的层叠顺序决定整体层级，子元素无法跨越父层叠上下文比较。 创建层叠上下文的常见条件： - position 为 absolute / relative / fixed / sticky 且 z-index 不为 auto
...
**口头回答版**：
> 方案一：Flexbox（推荐） 优点：代码简洁，兼容性好，支持多元素居中。 缺点：父元素会成为 flex 容器，可能影响子元素默认行为。 优点：代码最简洁，二维布局能力强。
...
**口头回答版**：
> 响应式设计是指一套代码适配不同屏幕尺寸和设备的能力。 媒体查询：根据视口宽度、设备特性应用不同样式。 流式布局：使用百分比、fr、vw/vh、max-width/min-width 等相对单位。 弹性图片/媒体：img { max-width: 100%; height: auto; }。
...
**口头回答版**：
> Specificity 计算（内联, ID, 类/伪类/属性, 类型/伪元素）： - A：#app .list .item a → (0, 1, 2, 1) - B：.list .item a.active → (0, 0, 3, 1) - C：#app #nav a.active → (0, 2, 1, 1)
...
**口头回答版**：
> BEM 是一种 CSS 类名命名方法论，代表： - Block（块）：独立的可复用组件，如 .button、.card。 - Element（元素）：块的组成部分，用 __ 连接，如 .button__icon、.card__title。 - Modifier（修饰符）：块或元素的状态/变体，用 -- 连接，如 .button--primary、.button--disabled。
...
**口头回答版**：
> CSS 预处理器在编译阶段扩展了 CSS 语法，主要特性包括： 变量：定义可复用的颜色、字体、间距等值。 嵌套：按 HTML 结构嵌套选择器，减少重复书写。 Mixin / 函数：复用代码片段，支持参数化。
...
**口头回答版**：
> CSS-in-JS 是一种在 JavaScript 代码中编写 CSS 的技术，通常以组件作用域样式为目标，常见库有 styled-components、Emotion、Linaria、vanilla-extract。 组件作用域样式：自动生成唯一类名，避免全局命名冲突。 动态样式：可直接使用 JS 变量、props、state 生成样式。 代码组织：样式与组件同处一处，便于维护。
...
**口头回答版**：
> 原子化 CSS 是指每个类只负责一个单一、不可再分的样式职责，如 .m-4 只设置 margin: 1rem，.text-center 只设置 text-align: center。 通过组合大量原子类来构建界面。 Tailwind CSS 是最流行的 Utility-first CSS 框架，提供大量预定义工具类，并支持 JIT（Just-In-Time）编译按需生成样式。 开发速度快：无需写自定义 CSS，直接在 HTML/JSX 中组合类名。
...
**口头回答版**：
> 常见 CSS 性能优化手段： 减少重排（Reflow）和重绘（Repaint） - 避免频繁修改影响布局的属性（如 width、height、top、left）。 - 优先使用 transform 和 opacity 做动画，它们通常可由 GPU 合成。
...
**口头回答版**：
> 现象：.container 的边框会坍塌，高度变为 0（或只剩边框本身），因为子元素都设置了 float: left，脱离了正常文档流，父元素无法感知它们的高度。 原因：浮动元素脱离文档流，父元素在计算高度时不会包含浮动子元素。 触发 BFC：给父元素添加 overflow: hidden 或 display: flow-root。 clearfix 伪元素：
...
**口头回答版**：
> 使用语义化标签 &lt;nav&gt; 包裹菜单。 桌面端使用 Flexbox 水平排列菜单项。 小屏幕下通过复选框 hack 或 JS 控制菜单显示/隐藏。 使用媒体查询切换布局。

---
### FB-06-CO-A-009：什么是重排（Reflow）、重绘（Repaint）和合成（Composite）？
...
**评分维度**：
- 批量修改（25%）：className/cssText
- 读写分离（25%）：避免强制同步布局
- 动画优化（25%）：transform/opacity
- DOM 批量操作（25%）：DocumentFragment
...
- 特殊性（30%）：选择器权重
- 继承（20%）：继承属性列表
- 关键字（20%）：initial/inherit/unset/revert
...
### FB-06-CO-A-012：CSS 的 :is()、:where()、:has() 伪类有什么作用？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：CSS 选择器、is、where、has
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
### FB-06-CO-A-013：CSS 隔离方案有哪些？如何选型？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：CSS 隔离、BEM、CSS Modules、Shadow DOM、Scoped
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
**评分维度**：
- BEM（20%）：命名约定
- CSS Modules（20%）：构建时局部类名
- Shadow DOM（20%）：原生隔离
- CSS-in-JS（20%）：运行时唯一类名
- 选型（20%）：根据项目规模
...
### FB-06-CO-A-014：CSS 的 contain 属性有什么作用？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：contain、布局边界、渲染优化、CSS
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
**评分维度**：
- 各值含义（50%）：layout/paint/size/style
- 优化效果（30%）：减少重排重绘范围
- 副作用（20%）：可能产生包含块
...
### FB-06-CO-A-015：如何实现单行文本溢出省略与多行溢出省略？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：文本溢出、ellipsis、CSS
**出现频率**：高频
**预计回答时长**：3-5 分钟
...
**评分维度**：
- 单行方案（40%）：white-space/overflow/text-overflow
- 多行方案（40%）：-webkit-line-clamp
- 可访问性（20%）：title 属性或完整文本
...
### FB-06-PE-A-002：CSS 加载与渲染性能有哪些优化手段？

**题型**：性能优化题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：CSS 性能、关键 CSS、加载优化、渲染
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
在实际落地 CSS 加载与渲染性能有哪些优化手段 时，建议结合 CSS 性能、关键 CSS、加载优化 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 加载（30%）：关键 CSS、异步、preload
- 解析（30%）：选择器复杂度
- 渲染（40%）：重排、合成、content-visibility
...
### FB-06-SC-A-003：如何设计一个响应式图片组件？

**题型**：场景设计题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：响应式图片、picture、srcset、sizes
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
**评分维度**：
- srcset/sizes（40%）：分辨率切换
- picture（30%）：艺术形式与格式切换
- alt（30%）：可访问性
...
### FB-06-SC-A-004：实现一个可拖拽排序列表的 HTML/CSS 思路

**题型**：场景设计题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：拖拽排序、HTML5 Drag、CSS、交互
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
在实际落地 实现一个可拖拽排序列表的 HTML/CSS 思路 时，建议结合 拖拽排序、HTML5 Drag、CSS 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 事件（30%）：dragstart/dragover/drop
- 位置计算（30%）：鼠标位置与元素边界
- 视觉反馈（40%）：拖拽样式、占位符
...
### FB-06-EN-A-002：CSS 预处理器与 PostCSS 在现代工程中的角色

**题型**：工程化题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：CSS 预处理器、PostCSS、工程化、CSS
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
在实际落地 CSS 预处理器与 PostCSS 在现代工程中的角色 时，建议结合 CSS 预处理器、PostCSS、工程化 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 预处理器（40%）：变量、嵌套、混合
- PostCSS（40%）：插件化后处理
- 选型（20%）：项目需求与团队熟悉度
...
### FB-06-CO-A-016：什么是渐进增强与优雅降级？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：渐进增强、优雅降级、兼容性、前端策略
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
在实际落地 渐进增强与优雅降级 时，建议结合 渐进增强、优雅降级、兼容性 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 渐进增强（40%）：由基础到增强
- 优雅降级（40%）：由完整到兼容
- 场景（20%）：目标用户与浏览器分布
...
**口头回答版**：
> CSS 视觉格式化模型定义了浏览器如何将文档树转换为可视化布局。 视口（Viewport）与初始包含块：根元素的包含块通常是 viewport 的矩形区域。 包含块（Containing Block）：每个元素的位置和尺寸计算都相对于其包含块。 - 静态/相对定位元素的包含块是最近的块级祖先的内容框。
...
**口头回答版**：
> - DOM 顺序自然（main 在 left/right 之前），有利于 SEO 和可访问性。 - 无需负 margin 或复杂 calc。 - 响应式改造简单。
...
**口头回答版**：
> 关键 CSS 是指渲染首屏（above-the-fold）内容所必需的最小 CSS 集合。 将其内联到 `<head>` 中，可以让浏览器在下载外部 CSS 文件之前就开始渲染首屏，减少首次绘制（FP）和首次内容绘制（FCP）时间。 - 使用 Critical、Penthouse、CriticalCSS 等工具分析页面并提取首屏 CSS。 - 结合 Puppeteer/Playwright 模拟不同视口尺寸。
...
**口头回答版**：
> will-change 提示浏览器某个元素即将发生变化，让浏览器提前进行优化。 例如声明 will-change: transform 会让浏览器将该元素提升为独立的合成层（compositor layer），动画可以在 GPU 上直接合成，避免主线程重排和重绘。 浏览器接收到 will-change 后，会为元素创建独立的图层。 属性变化时，浏览器只需重新合成图层，而无需重新计算布局或绘制。
...
**口头回答版**：
> | 方法论 | 核心思想 | 特点 | 适用场景 | |--------|----------|------|----------| | OOCSS | 结构与皮肤分离、容器与内容分离 | 高度抽象可复用对象，如 .btn、.skin | 快速构建、组件复用 | | SMACSS | 按功能分类：Base、Layout、Module、State、Theme | 分类清晰，规则轻量化 | 中大型项目 |
...
**口头回答版**：
> CSS 选择器匹配是从右向左进行的（Right-to-Left），以便快速排除不匹配的元素。 .header .nav ul li a span： - 层级过深，浏览器需要从所有 span 开始，逐级向上验证祖先。 - 建议直接使用类名，如 .nav-link-text { color: red; }。
...
**口头回答版**：
> 设计 Token 层： - 将颜色、间距、字体、圆角、阴影等抽象为 token。 - 分为基础 token（如 --color-blue-500）和语义化 token（如 --color-primary、--bg-surface）。 CSS 变量驱动主题：
...
**口头回答版**：
> 全局 CSS（早期） - 所有样式在一个或多个大文件中，通过命名约定避免冲突。 - 问题：命名冲突、特异性战争、难以维护。 命名方法论（BEM/OOCSS/SMACSS/ITCSS）

---
### FB-06-CO-P-004：CSS Houdini 是什么？Paint API 能做什么？

**题型**：概念题
**难度**：🔴 深入
**岗位层级**：高级 / 专家
**面试知识域**：06 HTML/CSS
**标签**：CSS Houdini、Paint API、CSS 扩展、Typed OM
**出现频率**：低频
**预计回答时长**：5-8 分钟
...
**评分维度**：
- 组成（40%）：Paint/Layout/Animation/Typed OM
- Paint API（40%）：自定义绘制
- 应用（20%）：复杂背景、边框效果
...
### FB-06-CO-P-005：CSS 的 @layer 层叠层有什么作用？

**题型**：概念题
**难度**：🔴 深入
**岗位层级**：高级 / 专家
**面试知识域**：06 HTML/CSS
**标签**：@layer、层叠层、优先级、CSS
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
### FB-06-CO-P-006：什么是 CSS Subgrid？它解决了什么问题？

**题型**：概念题
**难度**：🔴 深入
**岗位层级**：高级 / 专家
**面试知识域**：06 HTML/CSS
**标签**：Subgrid、CSS Grid、布局、CSS
**出现频率**：低频
**预计回答时长**：3-5 分钟
...
### FB-06-CO-P-007：什么是 CSS 逻辑属性（Logical Properties）？

**题型**：概念题
**难度**：🔴 深入
**岗位层级**：高级 / 专家
**面试知识域**：06 HTML/CSS
**标签**：逻辑属性、inline、block、国际化
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
**评分维度**：
- 文本流向（40%）：inline/block 与书写模式
- RTL 支持（30%）：自动适配左右
- 映射关系（30%）：margin-inline-start 对应 left/right
...
### FB-06-CO-P-008：渲染合成层（Compositing Layer）与 will-change 的关系？

**题型**：概念题
**难度**：🔴 深入
**岗位层级**：高级 / 专家
**面试知识域**：06 HTML/CSS
**标签**：合成层、will-change、GPU、性能
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
**口头回答版**：
> 命名规范：采用 BEM 或 CSS Modules，确保类名不冲突，保持低 specificity。 - 基础 token（颜色阶梯、间距阶梯）和语义 token（primary、background、text）。 - 使用 CSS 自定义属性 + Less/Sass 变量双轨，既支持运行时切换又支持编译时覆盖。 - 默认提供亮色/暗色主题。
...
**口头回答版**：
> 大型项目 CSS 管理方案： - 统一采用 BEM / ITCSS / CUBE CSS 等方法论。 - 制定团队 CSS 编写规范，纳入 Code Review  checklist。 - 业务组件使用 CSS Modules 或 Scoped CSS。
...
**口头回答版**：
> - 确定首屏包含的组件：导航、Banner、商品卡片首行、搜索框等。 - 使用 Chrome DevTools 的 Coverage 工具找出未使用 CSS。 提取 Critical CSS - 使用 Critical/Penthouse 按视口尺寸（如 375x667、1440x900）提取首屏 CSS。
...
**口头回答版**：
> Design Token 体系设计： - 基础 Token（Primitive）：品牌色板、灰阶、字体族、间距阶梯等，如 color-blue-500。 - 语义 Token（Semantic）：与用途绑定，如 color-primary、bg-surface、text-error。 - 组件 Token（Component）：具体组件使用的 token，如 button-primary-bg、input-border-color。
...
**口头回答版**：
> | 维度 | 原子化 CSS | 语义化类名（BEM） | |------|------------|-------------------| | 开发速度 | 快，无需写 CSS | 中等，需要命名和编写 | | 一致性 | 高，基于设计 token | 依赖团队规范 |
...
**口头回答版**：
> CSS 变量驱动主题 - 使用 :root、[data-theme="dark"]、[data-theme="light"] 定义颜色变量。 - 使用 [data-density="compact"] 等定义间距/字体尺寸变量。 颜色 token 设计
...
**口头回答版**：
> Monorepo 样式组织方案： - ui-core 提供全局变量、mixins、工具类、reset。 - 各组件包导入 ui-core 的变量，保持 token 一致。 - BEM 命名空间：每个包使用包前缀，如 .uc-button、.admin-header。

---
### FB-06-CO-B-014：什么是语义化 HTML？为什么要语义化？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：06 HTML/CSS
**标签**：position、Flexbox、BFC、CSS 变量、BEM
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
**口头回答版**：
> 语义化 HTML 是指使用恰当的 HTML 标签表达内容的含义，而不仅仅是呈现效果。 主要收益： - 可访问性：屏幕阅读器依赖语义标签进行导航。 - SEO：搜索引擎更容易理解页面结构。 - 可维护性：代码更易读，结构更清晰。

---

### FB-06-CO-B-015：解释 BEM 命名规范，并说明其优缺点。
...
**口头回答版**：
> BEM 是 Block-Element-Modifier 的缩写，通过命名约定组织 CSS 类名： - Block：独立模块，如 .card - Element：块的组成部分，如 .card__title - Modifier：状态或变体，如 .card--featured 优点： - 命名即作用域，降低冲突。 - 无需工具即可实现模块化。 缺点： - 类名较长。 - 深层嵌套组件表达力有限。

---

### FB-06-CP-B-001：CSS 中 `em`、`rem`、`px`、`vw` 有什么区别？如何选择？
...
在实际落地 CSS 中 `em`、`rem`、`px`、`vw` 有什么区别如何选择 时，建议结合 position、Flexbox、BFC 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 区分四个单位（50%）
- 给出选择建议（30%）
- 提到 `clamp()` 等现代方案（20%）
...
**口头回答版**：
> | 单位 | 参考基准 | 适用场景 | |------|---------|---------| | px | 物理像素 | 边框、阴影等不需要缩放的场景 | | em | 父元素字体大小 | 需要相对父级缩放的场景 | | rem | 根元素字体大小 | 字体大小、间距，便于整体缩放 | | vw/vh | 视口宽高 | 全屏布局、流体排版 | 选择建议： - 字体和间距优先使用 rem。 - 边框使用 px。 - 响应式布局可结合 clamp() 使用 vw。

---

### FB-06-CO-B-016：什么是 CSS 优先级？如何计算？
...
**口头回答版**：
> CSS 优先级由选择器的特殊性决定，从高到低： 1. 内联样式（1000） 2. ID 选择器（100） 3. 类选择器、属性选择器、伪类（10） 4. 元素选择器、伪元素（1） !important 可以覆盖优先级，但应尽量避免。 当优先级相同时，后声明的规则生效。

---

### FB-06-CD-B-001：如何实现一个三栏布局，中间自适应？
...
**口头回答版**：
> 方案一：Flexbox （代码示例） 方案二：Grid （代码示例） Grid 更适合二维布局，Flexbox 更适合一维布局。

---

### FB-06-EN-A-001：什么是 CSS 工程化？前端项目中为什么需要 CSS 工程化？
...
**题目描述**：
什么是 CSS 工程化？前端项目中为什么需要 CSS 工程化。
...
在实际落地 CSS 工程化前端项目中为什么需要 CSS 工程化 时，建议结合 position、Flexbox、BFC 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 定义 CSS 工程化（30%）
- 列举痛点（40%）
- 列举工程化手段（30%）
...
**口头回答版**：
> CSS 工程化是指建立一套可预测、可复用、可度量的样式生产体系，解决大型项目中的样式管理问题。 需要工程化的原因： - 全局命名冲突 - 选择器优先级战争 - 样式冗余与包体积膨胀 - 主题切换困难 - 响应式逻辑分散 - 可访问性合规要求 工程化手段包括：BEM、ITCSS、CSS Modules、CSS-in-JS、Atomic CSS、Design Token、Stylelint 等。

---

### FB-06-SC-A-002：解释一下容器查询（Container Query）及其使用场景。
...
**题目描述**：
解释一下容器查询（Container Query）及其使用场景。。
...
在实际落地 解释一下容器查询（Container Query）及其使用场景。 时，建议结合 position、Flexbox、BFC 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 解释容器查询原理（40%）
- 写出基本语法（30%）
- 给出使用场景（30%）
...
**口头回答版**：
> 容器查询允许组件根据其所在容器的大小调整样式，而不是基于整个视口。 （代码示例） 使用场景： - 同一个组件在不同布局中表现不同。 - 侧边栏、网格项等局部容器需要响应式。 - 组件库中需要组件级响应式能力。

---

### FB-06-CO-A-007：如何减少 CSS 引起的重排和重绘？
...
**口头回答版**：
> 减少重排： - 避免频繁读写会触发布局的属性（width、height、top、margin）。 - 批量修改样式时，先读取再写入，或使用 requestAnimationFrame。 - 使用 transform 和 opacity 做动画。 减少重绘： - 避免频繁修改颜色、背景等外观属性。

---

### FB-06-CO-A-008：Design Token 是什么？为什么现代设计系统需要它？
...
**口头回答版**：
> Design Token 是设计系统中的最小原子，如颜色、字体、间距、圆角等，以代码可消费的形式存在。 为什么需要： - 统一设计与开发之间的语言。 - 支持主题切换（浅色/深色/多品牌）。 - 避免硬编码，减少样式冗余。
