...
> 本题库共收录 **65** 道面试题（基础 14 / 进阶 28 / 深入 14 / 架构 9）。
...
### FB-34-CO-B-009：SVG、Canvas、WebGL 的适用场景

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：34 可视化与图形
**标签**：SVG、Canvas、WebGL、渲染、可视化
**出现频率**：高频
**预计回答时长**：3-5 分钟
...
1. SVG：矢量、DOM 可操作、适合静态/交互图表、少元素
2. Canvas：位图、性能高、适合动态绘制、中等元素量
3. WebGL：GPU 加速、适合海量数据、3D、复杂渲染
4. SVG 元素多会卡顿，Canvas 元素多可优化
5. 选型依据：数据量、交互复杂度、是否需要 DOM 事件
...
**口头回答版**：
> SVG；Canvas；WebGL；SVG 元素多会卡顿，Canvas 元素多可优化；选型依据。同时要避免海量数据用 SVG。

---

### FB-34-CO-A-016：图表库选型要考虑哪些因素

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：图表库、ECharts、D3、AntV、选型
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
**口头回答版**：
> 功能覆盖；性能；可定制性；生态与文档；框架兼容性；包体积和许可证。同时要避免为了炫酷选择过度复杂的库。

---

### FB-34-PE-A-013：大数据量图表如何优化

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：大数据、图表、抽稀、聚合、Canvas、WebGL
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明当数据量超过十万点时图表的优化手段。

**参考答案**：

核心思路如下：

1. 数据抽稀：按像素/区域聚合，减少绘制点
2. 分层渲染：重要点优先
3. 使用 Canvas/WebGL 替代 SVG
4. 视口裁剪：只绘制可见区域
5. 虚拟滚动或缩放时动态加载
6. Web Worker 预处理数据
...
在实际落地 大数据量图表如何优化 时，建议结合 大数据、图表、抽稀 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：数据抽稀、分层渲染 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 数据抽稀；分层渲染；使用 Canvas/WebGL 替代 SVG；视口裁剪；虚拟滚动或缩放时动态加载；Web Worker 预处理数据。同时要避免直接绘制所有点。

---

### FB-34-SC-A-011：如何设计一个可配置的图表组件

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：图表组件、配置化、Schema、ECharts、封装
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 统一配置规范：chartType、data、axis、legend、theme
2. 基于底层库（ECharts/AntV）封装
3. 支持事件回调和自定义 tooltip
4. 主题与样式可配置
5. 提供默认配置和可覆盖项
6. 校验配置合法性
...
在实际落地 设计一个可配置的图表组件 时，建议结合 图表组件、配置化、Schema 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：统一配置规范、基于底层库（ECharts/AntV）封装 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 统一配置规范；基于底层库（ECharts/AntV）封装；支持事件回调和自定义 tooltip；主题与样式可配置；提供默认配置和可覆盖项；校验配置合法性。同时要避免封装过度导致灵活性差。

---

### FB-34-CO-P-024：D3 与 ECharts 的取舍

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：34 可视化与图形
**标签**：D3、ECharts、可视化、定制、开发效率
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请比较 D3 和 ECharts 在可视化项目中的适用场景。

**参考答案**：

核心思路如下：

1. ECharts：开箱即用、配置驱动、适合常规图表
2. D3：底层灵活、适合高度定制和新颖可视化
3. ECharts 开发效率高，D3 可控性强
4. 复杂交互和自定义布局用 D3
5. 常规 BI 报表用 ECharts
...
**口头回答版**：
> ECharts；D3；ECharts 开发效率高，D3 可控性强；复杂交互和自定义布局用 D3；常规 BI 报表用 ECharts。同时要避免简单图表用 D3 过度开发。

---

### FB-34-PE-P-023：可视化大屏的渲染性能优化

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：34 可视化与图形
**标签**：大屏、可视化、性能、Canvas、WebGL、动画
**出现频率**：高频
**预计回答时长**：8-15 分钟
...
在实际落地 可视化大屏的渲染性能优化 时，建议结合 大屏、可视化、性能 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：使用 Canvas/WebGL 渲染大量元素、数据采样和聚合 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 使用 Canvas/WebGL 渲染大量元素；数据采样和聚合；动画使用 requestAnimationFrame；静态区域使用缓存位图；WebSocket 数据批量更新；分层渲染与脏矩形优化。同时要避免大量 DOM 图表。

---

### FB-34-SC-A-012：如何实现图表的响应式布局

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：响应式、图表、ResizeObserver、重绘、自适应
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
1. 监听容器 resize：ResizeObserver 或 window resize
2. 防抖/节流触发图表 resize
3. 图表库提供 resize API
4. 移动端适配：简化图表、调整 legend 位置
5. 保持比例或重新计算布局
...
在实际落地 实现图表的响应式布局 时，建议结合 响应式、图表、ResizeObserver 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：监听容器 resize、防抖/节流触发图表 resize 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 监听容器 resize；防抖/节流触发图表 resize；图表库提供 resize API；移动端适配；保持比例或重新计算布局。同时要避免只监听 window resize。

---

### FB-34-CO-A-017：什么是数据可视化中的视觉编码

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：视觉编码、位置、颜色、大小、形状、通道
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
**口头回答版**：
> 视觉编码；常用通道；选择原则；避免同时用过多通道导致混乱；颜色注意色盲友好。同时要避免用颜色表示数量大小。

---

### FB-34-SC-P-022：如何设计交互式地图应用

**题型**：场景设计题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：34 可视化与图形
**标签**：地图、GIS、Marker、聚合、性能
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请设计一个支持大量 marker 和交互的地图应用。

**参考答案**：

核心思路如下：

1. 地图库选择：Leaflet、Mapbox、高德、百度
2. Marker 聚合：按缩放级别聚合
3. 视口裁剪：只渲染可见 marker
4. 数据分层：热力图、点图、区域图按需切换
5. 事件处理：点击、hover、信息窗
6. 离线缓存地图瓦片
...
在实际落地 设计交互式地图应用 时，建议结合 地图、GIS、Marker 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：地图库选择、Marker 聚合 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 地图库选择；Marker 聚合；视口裁剪；数据分层；事件处理；离线缓存地图瓦片。同时要避免直接渲染上万 DOM marker。

---

### FB-34-PE-A-014：动画性能优化在可视化中的实践

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：动画、可视化、requestAnimationFrame、GPU、性能
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 使用 requestAnimationFrame 对齐刷新率
2. 优先用 transform/opacity
3. 避免布局属性动画
4. 减少同时动画元素
5. 使用 FLIP 技术
6. 复杂动画用 Canvas/WebGL
...
在实际落地 动画性能优化在可视化中的实践 时，建议结合 动画、可视化、requestAnimationFrame 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：使用 requestAnimationFrame 对齐刷新率、优先用 transform/opacity 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 使用 requestAnimationFrame 对齐刷新率；优先用 transform/opacity；避免布局属性动画；减少同时动画元素；使用 FLIP 技术；复杂动画用 Canvas/WebGL。同时要避免用 setInterval 驱动动画。

---

### FB-34-CO-P-025：颜色与主题系统在可视化中的设计

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：34 可视化与图形
**标签**：颜色、主题、可视化、色板、暗色模式
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 定义语义化颜色：主色、辅色、状态色、中性色
2. 提供分类色板和连续色板
3. 支持暗色/亮色主题切换
4. 色盲友好：避免仅依赖红绿
5. 主题通过 token 或 CSS 变量实现
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：定义语义化颜色、提供分类色板和连续色板 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 定义语义化颜色；提供分类色板和连续色板；支持暗色/亮色主题切换；色盲友好；主题通过 token 或 CSS 变量实现。同时要避免颜色硬编码。

---

### FB-34-SC-A-013：如何导出图表为图片/PDF

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：图表、导出、Canvas、SVG、图片、PDF
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. Canvas 用 toDataURL/toBlob
2. SVG 序列化为 data URL
3. 服务端渲染：Puppeteer 截屏保证一致性
4. PDF：jsPDF 或服务端生成
5. 处理高清屏：设置缩放比例
6. 注意跨域图片资源
...
在实际落地 导出图表为图片/PDF 时，建议结合 图表、导出、Canvas 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：Canvas 用 toDataURL/toBlob、SVG 序列化为 data URL 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> Canvas 用 toDataURL/toBlob；SVG 序列化为 data URL；服务端渲染；PDF；处理高清屏；注意跨域图片资源。同时要避免导出模糊。

---

### FB-34-CO-A-018：可视化中的数据更新策略
...
1. 增量更新：只更新变化的数据项
2. 避免每次都 setOption 全量更新
3. 使用 diff 算法最小化 DOM/Canvas 重绘
4. 动画过渡：数据变化时平滑动画
5. 大数据更新用 Web Worker
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：增量更新、避免每次都 setOption 全量更新 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 增量更新；避免每次都 setOption 全量更新；使用 diff 算法最小化 DOM/Canvas 重绘；动画过渡；大数据更新用 Web Worker。同时要避免每次数据变化都销毁重绘。

---

### FB-34-SD-R-029：设计一个可视化搭建平台

**题型**：系统设计题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：34 可视化与图形
**标签**：可视化、搭建、低代码、DSL、组件
**出现频率**：低频
**预计回答时长**：15-30 分钟

**题目描述**：
请设计一个支持拖拽配置生成看板的可视化搭建平台。

**参考答案**：

核心思路如下：

1. 画布层：采用绝对定位或网格布局的画布，支持拖拽、缩放、对齐辅助线、多选编组与撤销重做；组件以树形结构管理。
2. 组件库：抽象图表、指标卡、表格、过滤器等物料，定义统一的 Props、数据接口与事件契约，支持按业务扩展。
3. 数据绑定：引入数据源概念（API、数据库、静态 JSON），通过字段映射与转换器将数据注入组件；支持参数联动与过滤。
4. 渲染引擎：将页面与组件状态序列化为 DSL（JSON），在运行时根据 DSL 实例化组件；渲染层与编辑器解耦，保证预览与运行一致。
5. 主题与样式：提供主题变量、全局样式配置与组件级样式覆盖；支持暗色模式、响应式布局与导出图片/PDF。
6. 发布与权限：版本管理、页面级/组件级权限、预览沙箱、灰度发布；配合埋点分析用户使用情况并持续迭代。
...
在实际落地 设计一个可视化搭建平台 时，建议结合 可视化、搭建、低代码 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：画布与组件树、组件库 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 画布与组件树；组件库；数据绑定；主题与样式；渲染引擎；预览与发布。同时要避免DSL 与组件实现强耦合。

---

### FB-34-PE-P-024：Canvas 高清屏渲染优化

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：34 可视化与图形
**标签**：Canvas、DPR、高清屏、模糊、Retina
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请说明 Canvas 在 Retina 屏上模糊的原因和解决方案。
...
1. 原因：CSS 像素与物理像素比不同
2. 设置 canvas width/height 为 DPR 倍
3. CSS 设置为原始尺寸
4. context.scale(dpr, dpr)
5. 使用 devicePixelRatio 动态计算
...
在实际落地 Canvas 高清屏渲染优化 时，建议结合 Canvas、DPR、高清屏 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：原因、设置 canvas width/height 为 DPR 倍 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 原因；设置 canvas width/height 为 DPR 倍；CSS 设置为原始尺寸；context.scale(dpr, dpr)；使用 devicePixelRatio 动态计算。同时要避免只设置 CSS 尺寸。

---

### FB-34-CO-B-010：什么是图表的可访问性

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：34 可视化与图形
**标签**：可视化、a11y、可访问性、ARIA、键盘
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请说明如何让图表对屏幕阅读器和键盘用户友好。
...
1. 提供文本摘要和数据表格替代
2. ARIA 标签描述图表类型和关键数据
3. 键盘可操作：Tab、Enter、方向键
4. 颜色不是唯一信息通道
5. 高对比度主题
...
在实际落地 图表的可访问性 时，建议结合 可视化、a11y、可访问性 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：提供文本摘要和数据表格替代、ARIA 标签描述图表类型和关键数据 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 提供文本摘要和数据表格替代；ARIA 标签描述图表类型和关键数据；键盘可操作；颜色不是唯一信息通道；高对比度主题。同时要避免图表纯视觉无文本。

---

### FB-34-SC-A-014：可视化项目中的数据格式设计
...
在实际落地 可视化项目中的数据格式设计 时，建议结合 数据格式、图表、标准 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：区分维度与指标、标准格式 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 区分维度与指标；标准格式；支持行/列两种组织方式；元数据；转换层。同时要避免每个图表自定义格式。

---

### FB-34-PE-A-015：WebGL 在可视化中的使用时机

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：WebGL、GPU、海量数据、Three.js、Deck.gl
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明何时应该引入 WebGL 做可视化。
...
2. 复杂 3D 场景
3. 需要高级渲染效果
4. 库选择：Three.js、Deck.gl、Filament
5. 代价：学习成本、兼容性、调试难度
...
在实际落地 WebGL 在可视化中的使用时机 时，建议结合 WebGL、GPU、海量数据 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：数据量巨大、复杂 3D 场景 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 数据量巨大；复杂 3D 场景；需要高级渲染效果；库选择；代价。同时要避免小数据量也上 WebGL。

---

### FB-34-CO-P-026：可视化中的坐标系与投影

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：34 可视化与图形
**标签**：坐标系、投影、地理、极坐标、笛卡尔
**出现频率**：低频
**预计回答时长**：5-8 分钟
...
**口头回答版**：
> 笛卡尔坐标系；极坐标系；地理投影；选择合适的坐标系减少误导；注意投影带来的面积/距离变形。同时要避免误导性坐标轴。

---

### FB-34-SC-P-023：如何设计可视化组件的测试策略
...
在实际落地 设计可视化组件的测试策略 时，建议结合 可视化、测试、截图 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：单元测试、视觉回归 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 单元测试；视觉回归；交互测试；数据边界；性能测试。同时要避免只测数据不测渲染。

---

### FB-34-CP-R-031：可视化系统的性能与体验平衡
...
在实际落地 可视化系统的性能与体验平衡 时，建议结合 可视化、性能、体验 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：数据聚合降低精度换取性能、缩放时动态加载细节 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 数据聚合降低精度换取性能；缩放时动态加载细节；交互响应优先于全精度；分层展示；使用 progressive rendering。同时要避免追求全精度导致卡顿。

---

### FB-34-CO-A-019：什么是图表中的视觉欺骗
...
在实际落地 图表中的视觉欺骗 时，建议结合 视觉欺骗、坐标轴、比例 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：截断 Y 轴夸大差异、饼图角度和面积误导 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 截断 Y 轴夸大差异；饼图角度和面积误导；双 Y 轴随意组合；颜色饱和误导重要性；避免。同时要避免为突出效果截断坐标轴。

---

### FB-34-PE-A-016：3D 可视化性能优化

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：3D、WebGL、LOD、纹理、性能
**出现频率**：低频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明 WebGL 3D 场景的优化手段。
...
在实际落地 3D 可视化性能优化 时，建议结合 3D、WebGL、LOD 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：LOD、纹理压缩和 mipmap 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> LOD；纹理压缩和 mipmap；视锥体裁剪；实例化渲染减少 draw call；烘焙光照减少实时计算；使用对象池复用对象。同时要避免高模 everywhere。

---

### FB-34-SC-A-015：如何设计实时数据可视化

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：实时、可视化、WebSocket、更新、动画
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
在实际落地 设计实时数据可视化 时，建议结合 实时、可视化、WebSocket 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：数据通道、数据缓冲与批量更新 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 数据通道；数据缓冲与批量更新；时间轴/滚动窗口展示最近 N 条；异常高亮与告警；暂停/回放历史；性能优化。同时要避免每帧更新都重绘。

---

### FB-34-EN-A-017：可视化组件库的工程化实践

**题型**：工程化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：可视化、组件库、Monorepo、文档、主题
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
在实际落地 可视化组件库的工程化实践 时，建议结合 可视化、组件库、Monorepo 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：Monorepo 管理核心、图表、主题、文档、统一数据接口和事件规范 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> Monorepo 管理核心、图表、主题、文档；统一数据接口和事件规范；按需引入减少包体积；文档；主题与样式变量化；版本管理与兼容性。同时要避免图表各自为政。

---

### FB-34-CO-B-011：可视化图表的基本元素
...
在实际落地 可视化图表的基本元素 时，建议结合 图表、坐标轴、图例 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：坐标轴、图例 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 坐标轴；图例；标题和副标题；数据系列与图形标记；提示框 tooltip；网格线辅助阅读。同时要避免缺少坐标轴标签。

---

### FB-34-SC-A-016：图表响应式适配方案

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：响应式、图表、ResizeObserver、移动端
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请说明如何让图表在不同尺寸容器中正确显示。
...
在实际落地 图表响应式适配方案 时，建议结合 响应式、图表、ResizeObserver 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：监听容器尺寸变化、图表库 resize 方法 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 监听容器尺寸变化；图表库 resize 方法；移动端简化图表元素；调整 legend 和 label 位置；保持宽高比或重算布局。同时要避免只监听 window resize。

---

### FB-34-PE-A-017：Canvas 离屏渲染优化

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：Canvas、离屏渲染、性能、缓存、Worker
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明 Canvas 离屏渲染的应用场景。

**参考答案**：

核心思路如下：

1. 在 OffscreenCanvas 预渲染静态内容
2. 主 Canvas 只需合成
3. 可在 Worker 中执行
4. 适合复杂背景、频繁重绘
5. 注意内存占用
...
在实际落地 Canvas 离屏渲染优化 时，建议结合 Canvas、离屏渲染、性能 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：在 OffscreenCanvas 预渲染静态内容、主 Canvas 只需合成 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 在 OffscreenCanvas 预渲染静态内容；主 Canvas 只需合成；可在 Worker 中执行；适合复杂背景、频繁重绘；注意内存占用。同时要避免所有绘制都离屏。

---

### FB-34-CO-A-020：数据可视化颜色无障碍

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：可视化、颜色、色盲、无障碍、对比度
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
在实际落地 数据可视化颜色无障碍 时，建议结合 可视化、颜色、色盲 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：不仅用颜色区分，还加形状/纹理/标签、避免红绿组合 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 不仅用颜色区分，还加形状/纹理/标签；避免红绿组合；使用色盲友好调色板；确保对比度；提供数据表格替代。同时要避免只用颜色区分类别。

---

### FB-34-SC-A-017：图表导出图片方案

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：34 可视化与图形
**标签**：图表、导出、图片、Canvas、SVG
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
在实际落地 图表导出图片方案 时，建议结合 图表、导出、图片 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：Canvas 用 toDataURL/toBlob、SVG 序列化为 data URL 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> Canvas 用 toDataURL/toBlob；SVG 序列化为 data URL；处理高清屏 DPR；跨域资源需转 base64；服务端 Puppeteer 做复杂导出。同时要避免导出模糊。
