...
> 本题库共收录 **134** 道面试题（基础 31 / 进阶 39 / 深入 35 / 架构 29）。
...
### FB-13-CO-B-021：ESLint 和 Prettier 有什么区别？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：13 代码质量
**标签**：ESLint、Prettier、代码规范、格式化
**出现频率**：高频
**预计回答时长**：2-3 分钟

**题目描述**：
请说明 ESLint 和 Prettier 的区别，以及如何在一个项目中同时使用它们。

**参考答案**：

ESLint 是静态代码分析工具，主要做：

- 语法错误检测。
- 代码风格规则（如引号、分号）。
- 潜在问题检测（如未使用变量、闭包陷阱）。
- 可通过插件扩展规则（如 React、Vue、TypeScript）。

Prettier 是代码格式化工具，主要做：
...
1. 用 Prettier 负责格式，ESLint 负责逻辑和风格中的语义问题。
2. 关闭 ESLint 中与 Prettier 冲突的规则：
   - 使用 `eslint-config-prettier`。
3. 在 CI 中先跑 Prettier 检查，再跑 ESLint。
4. 配合 lint-staged 自动格式化。
...
**常见错误**：
- 用 ESLint 做大量格式规则，和 Prettier 冲突
- 项目中只配一个工具，忽略另一个

**口头回答版**：
> ESLint 是做静态分析和语法、风格检查的，Prettier 是专门格式化代码样式的。一般让 Prettier 管格式，ESLint 管逻辑和语义问题，用 `eslint-config-prettier` 关闭冲突规则。CI 里先跑 Prettier 再跑 ESLint。

---

### FB-13-EN-B-004：如何配置 ESLint 的共享规则？

**题型**：工程化题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：13 代码质量
**标签**：ESLint、共享配置、Monorepo、代码规范
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请说明如何在 Monorepo 或多项目中共享 ESLint 配置，并允许子项目覆盖。
...
- 共享配置包设为 `private: true`，不发布。
- 用 `peerDependencies` 声明 ESLint 版本，避免重复安装。
- 不同技术栈提供不同预设（react、node、ts）。
- 子项目覆盖时只覆盖少量规则，保持整体一致。

**评分维度**：
- 共享配置包结构（40%）
- 继承和覆盖机制（30%）
- peerDependencies 使用（20%）
- 多预设设计（10%）

**常见错误**：
- 子项目复制整份 ESLint 配置
- 共享配置包没有声明 peerDependencies 导致版本不一致

**口头回答版**：
> 共享 ESLint 配置可以抽成一个包，比如 `packages/eslint-config`，里面分 react、node、ts 几个预设。子项目用 `extends` 继承，需要时本地覆盖。共享包用 peerDependencies 声明 eslint 版本，避免各项目版本不统一。

---

### FB-13-CO-B-022：什么是 Husky 和 lint-staged？

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：13 代码质量
**标签**：Husky、lint-staged、Git hooks、代码质量
**出现频率**：高频
**预计回答时长**：2-3 分钟

**题目描述**：
请解释 Husky 和 lint-staged 的作用，以及它们如何配合保证代码质量。

**参考答案**：

Husky 是 Git hooks 管理工具，可以在 Git 生命周期（如 pre-commit、commit-msg）中执行脚本。

lint-staged 是在 Git 暂存区文件上运行命令的工具，只对本次提交的文件执行 lint/format，避免全量检查浪费时间。

配合流程：

1. 开发者 `git commit`。
2. Husky 触发 `pre-commit` hook。
3. lint-staged 只对暂存区文件跑 `eslint --fix` 和 `prettier --write`。
4. 如果有错误无法自动修复，提交被拦截。
...
注意：
- Husky v9 配置方式有变化，使用 `prepare` 脚本初始化。
- lint-staged 不应用于运行全量测试，避免提交过慢。

**评分维度**：
- 说明 Husky 作用（25%）
- 说明 lint-staged 作用（25%）
- 说明配合流程（40%）
- 提到版本差异（10%）

**常见错误**：
- 在 lint-staged 里跑全量测试
- Husky 配置后没有正确初始化导致 hook 不生效

**口头回答版**：
> Husky 管理 Git hooks，比如 pre-commit。lint-staged 只对本次提交的文件跑 lint 和 format。配合起来就是提交时自动修复代码风格问题，有问题就拦截提交。注意 lint-staged 不要跑全量测试，不然提交会很慢。

---

### FB-13-EN-B-005：如何在 CI 中统一代码质量检查？

**题型**：工程化题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：13 代码质量
**标签**：CI、代码质量、ESLint、Prettier、测试
**出现频率**：高频
**预计回答时长**：3-5 分钟

**题目描述**：
请说明如何在 CI 中统一执行代码质量检查，并保证本地和 CI 结果一致。
...
2. **静态分析**：`eslint .`
3. **类型检查**：`tsc --noEmit`
4. **单元测试**：`vitest run` 或 `jest --ci`
5. **覆盖率**：设置最低覆盖率阈值。

二、保证本地和 CI 一致：

1. 所有工具版本通过 lockfile 锁定。
2. 使用 `package.json` 统一脚本：
   ```json
   {
     "scripts": {
       "lint": "eslint .",
   ...
   ```
3. CI 中执行相同脚本，不单独写命令。
4. 本地用 Husky + lint-staged，CI 跑全量检查。
...
- 任何检查失败都阻止后续部署。
- 使用 `--max-warnings=0` 让 ESLint warning 也失败。
...
**常见错误**：
- CI 和本地脚本不一致，结果不同
- 允许 warning 通过，质量逐渐退化

**口头回答版**：
> CI 里统一跑 format check、ESLint、typecheck、test。所有命令封装成 package.json 脚本，本地和 CI 用同一套。安装时用 frozen-lockfile 锁定版本。失败要阻断部署，ESLint 可以设 `--max-warnings=0`。

---

### FB-13-CO-B-023：什么是测试金字塔？
...
**题目描述**：
请解释测试金字塔，并说明前端项目中如何实践。

**参考答案**：

测试金字塔由 Mike Cohn 提出，形状越底层测试越多、成本越低、速度越快；越顶层测试越少、成本越高、速度越慢。
...
- 单元测试用 Vitest/Jest + Testing Library。
- 集成测试测组件组合、API 调用。
- E2E 用 Playwright/Cypress 覆盖登录、下单等关键链路。
- 不要颠倒金字塔，变成“冰淇淋”形状（E2E 过多）。
...
**口头回答版**：
> 测试金字塔底层是大量单元测试，中间是集成测试，顶层是少量 E2E。单元测试快、便宜、定位准，E2E 慢、贵、覆盖核心链路。前端一般用 Vitest/Jest 做单元，Testing Library 做集成，Playwright/Cypress 做 E2E。

---

### FB-13-CO-A-020：如何定义代码质量度量指标？

**题型**：概念题
**难度**：🟡 进阶
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：代码质量、度量指标、覆盖率、技术债务
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明前端项目中可以定义哪些代码质量度量指标，以及如何使用它们。

**参考答案**：

一、测试相关指标：

1. **代码覆盖率**：行覆盖率、分支覆盖率、函数覆盖率。
2. **测试数量与比例**：单元/集成/E2E 测试数量占比。
3. **Flaky test 率**：不稳定测试的比例，越低越好。

二、代码规范指标：

1. **ESLint/TypeScript 错误数**。
2. **Prettier 格式违规数**。
3. **代码重复率**（Duplication）。

三、复杂度指标：

1. **圈复杂度（Cyclomatic Complexity）**：函数分支数，建议 < 10。
2. **认知复杂度（Cognitive Complexity）**：代码可读性指标。
3. **文件/函数行数**：过大文件或函数需要拆分。

四、可维护性指标：

1. **技术债务比率**（SonarQube 的 debt ratio）。
2. **代码龄**：老旧代码占比。
3. **依赖健康度**：过时依赖、漏洞依赖数量。

五、业务相关指标：

1. **Bug 密度**：每千行代码的 bug 数。
2. **缺陷逃逸率**：线上 bug 占总 bug 的比例。
3. **平均修复时间（MTTR）**。
...
**口头回答版**：
> 代码质量指标可以分测试、规范、复杂度、可维护性和业务几类。测试看覆盖率和 flaky 率，规范看 ESLint 错误和重复率，复杂度看圈复杂度，可维护性看技术债务比和依赖健康度。CI 里收集趋势，设阈值告警，但不要只看覆盖率。

---

### FB-13-CP-A-019：如何选择 Jest 还是 Vitest？

**题型**：对比题
**难度**：🟡 进阶
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：Jest、Vitest、测试框架、选型
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请对比 Jest 和 Vitest，说明各自优势和适用场景。

**参考答案**：

| 维度 | Jest | Vitest |
|------|------|--------|
| 生态 | 成熟，社区大，文档丰富 | 新兴，但发展迅速 |
| 速度 | 快 | 基于 Vite，通常更快 |
| ESM 支持 | 需要配置，部分场景不友好 | 原生 ESM 支持 |
| TypeScript | 需 ts-jest/babel-jest | 原生支持 |
| 配置 | 配置项多，学习曲线较陡 | 与 Vite 配置复用，简洁 |
| 兼容性 | 与旧项目兼容好 | 适合 Vite 项目 |
| Watch 模式 | 成熟 | 快速 |
| Snapshot | 支持 | 支持，内置美观 diff |
| Mock | 强大 | 兼容 Jest API |
...
- **选 Jest**：
  - 项目历史悠久，已经深度使用 Jest。
  - 依赖大量 Jest 插件和自定义 transformer。
  - 团队对 Jest 非常熟悉。

- **选 Vitest**：
  - 项目使用 Vite 构建。
  - 希望原生 TypeScript/ESM 支持。
  - 希望更快的 watch 和更简洁的配置。

迁移建议：

- Vitest 兼容大部分 Jest API，迁移成本相对较低。
- 可以先在新项目试点，再逐步迁移老项目。
...
**口头回答版**：
> Jest 生态成熟、兼容好，老项目常用。Vitest 基于 Vite，原生支持 ESM 和 TypeScript，配置简单，速度更快。新项目或用 Vite 的项目优先 Vitest，老项目如果深度用 Jest 可以不动。迁移成本不高，因为 Vitest 兼容大部分 Jest API。

---

### FB-13-EN-A-011：如何设计前端代码 review 流程？

**题型**：工程化题
**难度**：🟡 进阶
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：Code Review、流程、质量、协作
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请设计一个前端代码 review 流程，兼顾质量、效率和开发者体验。
...
1. 作者先自测，保证 CI 通过。
2. PR 描述写清楚：背景、改动点、影响范围、测试方式、截图。
3. 关联 issue 或需求文档。

二、Reviewer 分配：

1. 通过 CODEOWNERS 自动分配相关模块 owner。
2. 核心变更需要架构师或安全人员参与。
3. PR 不宜过大，建议 < 400 行。

三、Review 关注点：

1. **正确性**：逻辑是否符合需求，边界条件是否处理。
2. **可维护性**：命名、注释、复杂度、重复代码。
3. **性能**：是否有明显性能隐患。
4. **安全**：输入校验、XSS、敏感数据处理。
5. **测试**：是否补充了测试，测试是否有意义。

四、Review 工具：

- GitHub/GitLab PR review。
- Danger.js 自动检查 PR 大小、changelog、测试覆盖率变化。
- SonarQube 自动评论代码问题。
...
**口头回答版**：
> 代码 review 流程要先作者自测、CI 通过、PR 描述清楚。用 CODEOWNERS 自动分配 reviewer，PR 别太大。review 关注正确性、可维护性、性能、安全和测试。可以用 Danger.js 自动检查 PR 大小和覆盖率。评论要明确可操作，合并前必须 CI 绿。

---

### FB-13-SE-A-020：什么是 OWASP Top 10 在前端的体现？

**题型**：安全题
**难度**：🟡 进阶
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：OWASP、安全、前端、XSS、CSRF
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明 OWASP Top 10 中哪些风险与前端开发密切相关，以及如何防范。
...
1. **A01: Broken Access Control（访问控制失效）**：
   - 前端隐藏菜单但后端未鉴权。
   - 防范：前端做 UI 级控制，后端做真正鉴权。

2. **A03: Injection（注入攻击）**：
   - XSS：用户输入未转义直接插入 DOM。
   - 防范：使用框架自动转义、Content Security Policy、DOMPurify。

3. **A04: Insecure Design（不安全设计）**：
   - 敏感操作缺少二次确认、验证码。
   - 防范：关键操作设计防重放、防暴力破解。

4. **A05: Security Misconfiguration（安全配置错误）**：
   - Source Map、内部 API 暴露到公网。
   - 防范：生产关闭 Source Map，正确配置 CORS、HSTS。

5. **A07: Identification and Authentication Failures（身份认证失败）**：
   - Token 存储在 localStorage、长期不刷新。
   - 防范：使用 httpOnly cookie、短 token + refresh token。

6. **A08: Software and Data Integrity Failures（软件和数据完整性失效）**：
   - 加载第三方 CDN 脚本无 SRI。
   - 防范：使用 Subresource Integrity（SRI）。

二、防御纵深：

- 输入校验、输出编码、HTTPS、CSP、CORS、SRI、安全头。
- 安全扫描（npm audit、Snyk）纳入 CI。
...
**口头回答版**：
> 前端相关的 OWASP 风险有访问控制失效、XSS、不安全设计、安全配置错误、身份认证失败、数据完整性失效。比如 XSS 要框架转义和 CSP，token 不要放 localStorage，用 httpOnly cookie。还要配置 CORS、HSTS、SRI，Source Map 不要暴露到生产。
...
1. **静态分析**：
   - SonarQube 的 debt ratio、code smell。
   - ESLint 复杂规则告警。
   - 重复代码检测。
...
1. **登记债务**：建立 TECH-DEBT-REGISTER.md。
2. **分配时间**：每个迭代固定比例时间处理债务。
3. **小步重构**：每次 PR 顺手清理相关债务。
4. **专项重构**：对重大债务安排独立项目。
5. **验证收益**：重构后监控指标和 bug 率。
...
**口头回答版**：
> 识别技术债务可以用 SonarQube、ESLint 做静态分析，看运行时错误率和开发体验指标，还有架构上的循环依赖、过期依赖。排序先处理影响稳定和频繁改动的地方。清理要登记债务，每个迭代分配时间，小步重构，重大债务做专项。review 和 CI 质量门禁可以防止新增债务。

---

### FB-13-EN-P-017：如何设计可维护的测试用例？

**题型**：工程化题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：测试、可维护性、Testing Library、测试设计
**出现频率**：高频
**预计回答时长**：8-12 分钟

**题目描述**：
请说明如何编写可维护、稳定、易读的前端测试用例。
...
- 优先使用用户可见的属性：`getByRole`、`getByText`、`getByLabelText`。
- 避免使用测试 ID 或 CSS 类名作为唯一选择器，除非必要。
- 减少因 UI 调整导致测试失效。

三、数据准备：

- 使用 factory 函数生成测试数据，避免硬编码大量对象。
- Mock 外部依赖（API、定时器、随机数）。
- 每个测试独立设置和清理，避免共享状态。

四、避免脆弱测试：

1. 不要断言具体实现细节，断言行为和输出。
2. 避免 `setTimeout` 真实等待，用 `waitFor` 或 fake timers。
3. 不要测试第三方库的内部行为。

五、命名与组织：

- 测试名描述行为和期望：`describe('when user submits form', ...)`。
- 按模块/功能分文件，保持测试和源码同目录或邻近。
...
**口头回答版**：
> 可维护测试用例用 AAA 结构，一个测试一个关注点。选择器优先用用户可见的 role、text、label，少用测试 ID 和 CSS 类。数据用 factory 生成，mock 外部依赖，每个测试独立。不要断言实现细节，别用真实 setTimeout，测试名要描述行为。

---

### FB-13-SE-P-018：如何做前端安全测试？

**题型**：安全题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：前端安全、测试、XSS、CSRF、渗透测试
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
请说明前端安全测试可以覆盖哪些方面，使用什么工具和方法。

**参考答案**：

一、静态安全测试（SAST）：

1. 依赖漏洞扫描：`npm audit`、`pnpm audit`、Snyk。
2. 代码规则：`eslint-plugin-security`、`semgrep`。
3. Secret 扫描：git-secrets、truffleHog。

二、动态安全测试（DAST）：

1. 使用 OWASP ZAP、Burp Suite 扫描运行中的应用。
2. 检查 XSS、CSRF、点击劫持、CORS 配置。

三、专项测试：

1. **XSS 测试**：
   - 输入 `<script>alert(1)</script>` 到搜索框、评论框。
   - 检查是否被转义或 CSP 拦截。

2. **CSRF 测试**：
   - 验证敏感操作是否有 CSRF token 或 SameSite cookie。

3. **点击劫持测试**：
   - 检查 `X-Frame-Options` 或 CSP frame-ancestors。

4. **敏感信息泄露**：
   - 检查 Source Map、.env、内部 API 是否暴露。

四、自动化集成：

- 将 SAST 纳入 CI，高危漏洞阻塞合并。
- 定期跑 DAST，尤其是在重大变更后。
- 安全测试报告归档，跟踪修复进度。
...
**口头回答版**：
> 前端安全测试分静态和动态。静态用 npm audit、Snyk、eslint-plugin-security、secret 扫描。动态用 OWASP ZAP 扫运行中的应用。专项测 XSS、CSRF、点击劫持、Source Map 泄露。要把 SAST 放 CI 里，高危漏洞阻塞合并。关键业务还要做人工渗透测试。

---

### FB-13-CP-P-021：如何制定代码规范落地策略？

**题型**：综合开放题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：代码规范、落地策略、ESLint、Prettier、治理
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
请说明如何在团队中制定并落地前端代码规范，避免规范成为一纸空文。

**参考答案**：

一、规范制定：

1. 基于行业最佳实践（Airbnb、Standard、TypeScript 推荐）。
2. 结合团队现状，避免规则过严导致抵触。
3. 通过 RFC 或技术委员会讨论确定。
4. 写成文档，说明每条规则的原因和例外场景。

二、工具化：

1. 使用 ESLint + Prettier 自动检查。
2. 共享配置包，统一团队规则。
3. Husky + lint-staged 提交前拦截。
4. CI 中 `--max-warnings=0` 强制通过。

三、渐进式落地：

1. 新项目直接应用全部规则。
2. 老项目先开最基础规则，逐步加严。
3. 对遗留问题可以用 `eslint-disable-next-line` + 注释说明，但要有清理计划。

四、培训与反馈：

1. 新人 onboarding 学习规范。
2. 定期收集开发者反馈，调整不合理规则。
3. 代码 review 中解释规范原因，而不只是说“不符合规范”。

五、度量与持续改进：

1. 统计 ESLint 错误数、warning 数趋势。
2. 奖励规范执行好的团队或个人。
3. 每季度 review 一次规则，删除过时规则。
...
**常见错误**：
- 规范由一个人拍脑袋决定，团队不认同
- 规则过严，导致大家大量 disable，规范形同虚设

**口头回答版**：
> 制定规范要参考行业标准，结合团队情况，通过 RFC 讨论确定。工具化用 ESLint、Prettier、Husky、lint-staged，CI 强制通过。老项目渐进式落地，先基础规则再逐步加严。要培训新人、收集反馈、定期 review 规则，统计错误趋势。

---

### FB-13-CO-P-017：什么是 mutation testing？

**题型**：概念题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：变异测试、mutation testing、测试质量、覆盖率
**出现频率**：低频
**预计回答时长**：5-8 分钟

**题目描述**：
请解释变异测试（mutation testing）的概念，以及它与传统代码覆盖率的区别。

**参考答案**：

变异测试是一种评估测试质量的方法。它通过自动修改源码中的小部分（如把 `>` 改成 `>=`，把 `true` 改成 `false`），生成多个“变异体”（mutants），然后运行测试套件。

如果测试能发现变异体导致的错误（即测试失败），说明该变异体被“杀死”。被杀死的变异体比例越高，测试质量越好。

与代码覆盖率的区别：

- **覆盖率**：衡量代码是否被测试执行过，不衡量测试是否有效。
- **变异测试**：衡量测试是否能发现代码中的错误，更真实反映测试有效性。
...
如果测试只测 `isPositive(1)`，覆盖率 100%，但无法杀死这个变异体，说明测试不够充分。

工具：Stryker（JS/TS 变异测试工具）。

注意事项：

- 变异测试运行慢，不适合每次 CI 跑，可在夜间或关键模块跑。
- 等价变异体（语义相同但写法不同）会导致误报。
...
**口头回答版**：
> 变异测试是自动改源码生成变异体，看测试能不能发现这些改动。能发现就是杀死了变异体。它比拼覆盖率更能反映测试质量，因为覆盖率只说明代码被执行过，不代表测试有效。比如把 `>` 改成 `>=`，如果测试没测边界就杀不死。JS 里用 Stryker。运行慢，适合夜间或关键模块跑。

---

### FB-13-SC-P-006：如何设计质量门禁（Quality Gate）？

**题型**：场景设计题
**难度**：🟠 高级
**岗位层级**：高级 / 专家
**面试知识域**：13 代码质量
**标签**：质量门禁、CI、SonarQube、覆盖率、阈值
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
请设计一个前端项目的质量门禁（Quality Gate），在 CI 中拦截不符合标准的代码。
...
| 维度 | 检查项 | 阈值 |
|------|--------|------|
| 格式 | Prettier check | 0 违规 |
| 静态分析 | ESLint | 0 error，warning 逐步收敛 |
| 类型 | TypeScript | 0 类型错误 |
| 测试 | 单元测试通过 | 100% pass |
| 覆盖率 | 行覆盖率 | >= 80% |
| 重复代码 | SonarQube duplication | <= 3% |
| 复杂度 | SonarQube complexity | 函数 <= 10 |
| 安全 | npm audit | 0 high/critical |
...
- 新代码覆盖率必须 >= 80%，老代码逐步提升。
- SonarQube 可设置“新代码”质量门禁，避免一刀切。

四、例外处理：

- 明确允许 `eslint-disable` 的场景和审批流程。
- 遗留问题单独登记，限期清理。

五、反馈机制：

- CI 失败时明确告诉开发者哪项门禁未通过。
- 提供修复命令和文档链接。
- 定期 review 阈值合理性，避免过松或过严。
...
**口头回答版**：
> 质量门禁可以包括 format、ESLint、typecheck、测试、覆盖率、重复代码、复杂度、安全漏洞。CI 里依次跑，失败阻断合并。阈值可以分新代码和老代码，新代码要求高，老代码逐步提升。要允许合理例外，CI 失败时给明确修复指引。

---


### FB-13-SD-R-032：设计一个前端质量保障平台

**题型**：系统设计题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：13 代码质量
**标签**：质量保障平台、质量门户、指标、CI、可视化
**出现频率**：低频
**预计回答时长**：10-15 分钟

**题目描述**：
请设计一个前端质量保障平台，能够汇总各项目的代码质量、测试、安全、性能数据，并提供可视化看板和告警。
...
1. **CI 数据**：
   - pipeline 成功率、耗时、失败原因分类。
   - 测试数量、通过率、覆盖率、flaky test 数。

2. **静态分析数据**：
   - ESLint/SonarQube 问题数和趋势。
   - 重复代码、复杂度、技术债务比。

3. **安全数据**：
   - npm audit、Snyk 漏洞数和等级。
   - Secret 扫描结果。

4. **性能数据**：
   - 构建产物体积、核心 Web Vitals、Lighthouse 分数。

二、数据存储：

- 构建记录、项目元数据存关系型数据库。
- 历史指标存时序数据库（InfluxDB/TimescaleDB）。
- 报告文件存对象存储。
...
**口头回答版**：
> 前端质量保障平台要采集 CI、静态分析、安全、性能数据，存到时序数据库和关系型数据库。然后做项目级和团队级看板，展示质量评分、趋势、覆盖率、漏洞。设置覆盖率下降、新增高危漏洞等告警。提供 Open API 给内部工具，按团队隔离权限。数据用来驱动改进，不要唯指标论。

---

### FB-13-CP-R-006：如何在大型组织中推行代码质量文化？

**题型**：综合开放题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：13 代码质量
**标签**：代码质量文化、组织推进、治理、培训
**出现频率**：低频
**预计回答时长**：8-12 分钟

**题目描述**：
请说明在大型组织中如何推行代码质量文化，让质量成为团队的共同责任。

**参考答案**：

一、建立共识：

1. 明确代码质量对业务的价值：减少 bug、提升迭代速度、降低维护成本。
2. 通过事故复盘展示低质量代码的代价。
3. 让技术负责人和 PM 都认同质量投入。

二、标准化与工具化：

1. 制定统一的代码规范和质量门禁。
2. 提供共享的 ESLint/Prettier/TSConfig 配置。
3. 把质量检查自动化，减少人为疏忽。

三、培训与赋能：

1. 新人 onboarding 包含代码质量课程。
2. 定期组织代码 smell、重构、测试工作坊。
3. 建立内部技术博客和案例库。
...
1. 核心模块要有 owner 对质量负责。
2. 质量指标作为团队 health check 的一部分，但不直接 KPI。
3. 对重复出现的质量问题进行复盘和改进。
...
**口头回答版**：
> 推行代码质量文化首先要让所有人认同质量价值，不能靠惩罚。然后标准化工具化，统一规范和门禁，自动化检查。培训新人、组织工作坊，review 里表扬好代码，把清技术债务纳入迭代。核心模块要有 owner，质量指标作为 health check 但不直接 KPI，定期收集反馈持续改进。

---

### FB-13-SC-R-001：如何设计全链路质量度量体系？

**题型**：系统设计题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：13 代码质量
**标签**：质量度量、全链路、DORA、MTTR、DevOps
**出现频率**：低频
**预计回答时长**：10-15 分钟

**题目描述**：
请设计一个覆盖从需求到上线的全链路质量度量体系。
...
| 需求 | 需求变更率、需求清晰度评分 |
| 设计 | 架构评审通过率、设计文档完整性 |
| 开发 | 代码覆盖率、复杂度、ESLint 问题数 |
| 测试 | 测试通过率、flaky test 率、bug 逃逸率 |
| 发布 | 部署频率、变更前置时间、发布成功率 |
| 运维 | MTTR、MTBF、线上错误率 |

二、DORA 核心指标：

1. **部署频率**：单位时间部署到生产的次数。
2. **变更前置时间**：从代码提交到上线的时间。
3. **变更失败率**：导致故障的发布占比。
4. **恢复服务时间（MTTR）**：故障发生后恢复的时间。

三、数据采集：

- 需求/项目管理工具（Jira/飞书项目）。
- Git 仓库（提交、PR、review 数据）。
- CI/CD 平台（构建、测试、部署数据）。
- 监控平台（错误、性能、可用性数据）。
...
**口头回答版**：
> 全链路质量度量要覆盖需求、设计、开发、测试、发布、运维各阶段。开发阶段看覆盖率和复杂度，测试看通过率和 bug 逃逸率，发布和运维用 DORA 指标：部署频率、变更前置时间、变更失败率、恢复时间。数据从 Jira、Git、CI、监控平台采集，做成团队看板，异常告警。指标不要太多，要和改进行动挂钩。

---

### FB-13-CO-R-001：代码质量与交付速度如何平衡？

**题型**：概念题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：13 代码质量
**标签**：代码质量、交付速度、平衡、技术债务
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
在项目进度紧张时，如何平衡代码质量和交付速度？

**参考答案**：

一、核心原则：

- 质量不是交付速度的对立面，低质量代码会在后期严重拖慢速度。
- 平衡的关键是区分“必须做好的事”和“可以暂时妥协的事”。
...
1. 非核心模块的测试覆盖率可以稍低，但要有计划补齐。
2. 部分重构可以延后，但要登记技术债务。
3. 文档可以精简，但核心 API 必须说明。

四、具体策略：

1. **MVP 思维**：先交付可用版本，再迭代优化。
2. **自动化**：把重复检查交给 CI，减少人工时间。
3. **小步快跑**：小 PR、小发布，降低风险。
4. **明确债务清单**：每次妥协都要记录，安排后续偿还。
5. **预留重构时间**：每个迭代固定 10-20% 时间处理债务。

五、长期视角：

- 持续交付能力取决于代码健康度。
- 短期牺牲质量要有明确的偿还计划，否则债务会指数增长。
...
**口头回答版**：
> 质量和速度不是对立的，低质量代码后面会拖慢速度。底线是安全、核心测试、代码审查、类型检查不能妥协。可以临时妥协的是边缘模块的覆盖率、部分重构和文档，但要登记技术债务。用 MVP 思维、自动化、小步快跑，每个迭代留时间还债。

---

### FB-13-EN-R-001：如何设计前端自动化回归测试体系？

**题型**：工程化题
**难度**：🔴 架构
**岗位层级**：专家 / 架构
**面试知识域**：13 代码质量
**标签**：回归测试、自动化、E2E、视觉回归、CI
**出现频率**：中频
**预计回答时长**：8-12 分钟

**题目描述**：
请设计一个前端自动化回归测试体系，覆盖功能、视觉和性能回归。
...
3. **E2E 测试**：
   - 覆盖核心用户链路（登录、下单、支付）。
   - 使用 Playwright/Cypress，夜间或发布前跑。

二、视觉回归：

1. 使用 Storybook + Chromatic / Loki / Percy。
2. 对关键组件截图对比，发现 UI 变化。
3. 基线版本截图存于服务，PR 时对比差异。

三、性能回归：

1. 使用 Lighthouse CI 或 Web Vitals 监控。
2. 每次构建后跑性能测试，记录 LCP、FID/INP、CLS。
3. 设置阈值，性能下降阻塞合并。
...
1. 测试数据独立，避免互相影响。
2. 用 mock 替代不稳定的外部依赖。
3. 对 flaky test 零容忍，发现立即修复或删除。

六、报告与告警：

- 失败时自动截图、录屏、保存日志。
- 与 IM 集成通知相关负责人。
- 定期 review 回归测试覆盖率和有效性。
...
**口头回答版**：
> 自动化回归测试分功能、视觉、性能。功能用单元、集成、E2E；视觉用 Storybook + Chromatic 截图对比；性能用 Lighthouse CI 监控 Web Vitals。PR 跑单元集成，夜间跑 E2E，组件变更跑视觉，每日跑性能。测试数据要独立，flaky test 要及时修。失败时自动截图录屏通知负责人。
