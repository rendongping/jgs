...
> 本题库共收录 **86** 道面试题（基础 11 / 进阶 32 / 深入 24 / 架构 19）。
...
### FB-26-CO-B-009：什么是微前端，它解决了什么问题

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、微服务、前端集成、独立部署
**出现频率**：高频
**预计回答时长**：3-5 分钟

**题目描述**：
请解释微前端的概念、产生背景以及它主要解决的大型前端应用痛点。

**参考答案**：

核心思路如下：

1. 微前端把前端应用拆分为可独立开发、部署、运行的子应用
2. 解决巨石应用构建慢、发布耦合、技术栈锁定、团队边界不清
3. 常见集成方式：iframe、Web Components、Module Federation、路由分发
4. 适合组织规模大、业务域清晰、团队自治的场景
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：微前端把前端应用拆分为可独立开发、部署、运行的子应用、解决巨石应用构建慢、发布耦合、技术栈锁定、团队边界不清 等关键点
...
**口头回答版**：
> 微前端把前端应用拆分为可独立开发、部署、运行的子应用；解决巨石应用构建慢、发布耦合、技术栈锁定、团队边界不清；常见集成方式；适合组织规模大、业务域清晰、团队自治的场景。同时要避免为了技术炫技强行拆分。

---

### FB-26-CO-A-019：微前端与 iframe 的区别与取舍

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、iframe、隔离、通信、体验
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请比较微前端方案与 iframe 在隔离性、通信、体验、SEO 等方面的差异。

**参考答案**：

核心思路如下：

1. iframe 隔离最强但体验差：弹窗遮罩、路由同步、性能开销
2. 微前端集成更自然，用户体验接近单页应用
3. iframe 适合异构系统或第三方页面集成
4. 微前端需要解决样式隔离、JS 沙箱、公共依赖共享
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：iframe 隔离最强但体验差、微前端集成更自然，用户体验接近单页应用 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> iframe 隔离最强但体验差；微前端集成更自然，用户体验接近单页应用；iframe 适合异构系统或第三方页面集成；微前端需要解决样式隔离、JS 沙箱、公共依赖共享。同时要避免为了隔离一刀切用 iframe。

---

### FB-26-CO-P-003：Module Federation 的工作原理

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：Module Federation、Webpack、远程模块、运行时共享
**出现频率**：高频
**预计回答时长**：8-15 分钟

**题目描述**：
请说明 Webpack Module Federation 的核心概念、加载流程以及共享依赖管理。

**参考答案**：

核心思路如下：

1. 核心：远程入口文件 remoteEntry.js 暴露模块，消费端动态加载
2. ContainerPlugin 暴露模块，ContainerReferencePlugin 消费远程模块
3. shared 配置实现依赖共享与版本匹配，避免重复加载
4. 加载流程：加载 remoteEntry -> 初始化共享作用域 -> import 远程模块
5. 适用场景：同技术栈子应用、组件级共享、动态能力扩展
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：核心、ContainerPlugin 暴露模块，ContainerReferencePlugin 消费远程模块 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 核心；ContainerPlugin 暴露模块，ContainerReferencePlugin 消费远程模块；shared 配置实现依赖共享与版本匹配，避免重复加载；加载流程；适用场景。同时要避免远程模块未做版本管理。

---

### FB-26-SC-A-014：微前端子应用间如何通信

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、通信、事件总线、共享状态、URL
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请设计微前端子应用间的通信机制，并说明不同耦合程度下的选择。

**参考答案**：

核心思路如下：

1. 低耦合：URL 参数、浏览器事件、postMessage
2. 中耦合：基座提供全局事件总线，约定事件名与数据格式
3. 紧耦合：共享状态库（如 Redux、Pinia）仅适用于同技术栈且强协同
4. 避免直接调用子应用内部 API，保持边界清晰
5. 通信协议版本化，便于独立演进
...
在实际落地 微前端子应用间如何通信 时，建议结合 微前端、通信、事件总线 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：低耦合、中耦合 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 低耦合；中耦合；紧耦合；避免直接调用子应用内部 API，保持边界清晰；通信协议版本化，便于独立演进。同时要避免子应用直接互相 import。

---

### FB-26-SD-R-027：设计一个企业级微前端基座

**题型**：系统设计题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：26 微前端
**标签**：微前端、基座、路由、沙箱、生命周期
**出现频率**：高频
**预计回答时长**：15-30 分钟

**题目描述**：
请设计一个支持多子应用、多技术栈的微前端基座，覆盖路由、加载、沙箱、通信、错误处理。

**参考答案**：

核心思路如下：

1. 路由分发：根据 path 匹配子应用，支持注册表与动态加载
2. 生命周期：bootstrap、mount、unmount，确保子应用正确挂载卸载
3. JS 沙箱：Proxy 隔离 window/document，快照恢复
4. CSS 隔离：Shadow DOM、Scoped CSS、命名空间、构建时前缀
5. 公共依赖：shared 配置或 externals + 全局 UMD
6. 错误处理：子应用加载失败降级、超时、重试
...
在实际落地 设计一个企业级微前端基座 时，建议结合 微前端、基座、路由 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：路由分发、生命周期 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 路由分发；生命周期；JS 沙箱；CSS 隔离；公共依赖；错误处理。同时要避免基座过于复杂侵入业务。

---

### FB-26-CO-A-020：微前端的样式隔离方案有哪些

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、CSS 隔离、Shadow DOM、Scoped CSS、命名空间
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. Shadow DOM：最强隔离，但兼容性、第三方组件、表单提交需注意
2. Scoped CSS：Vue scoped / CSS Modules，构建时限定作用域
3. 命名空间约定：BEM、前缀，依赖团队自律
4. 构建时前缀：PostCSS 自动加子应用前缀
5. 基座统一重置与主题变量，减少全局样式冲突

需要避免的典型误区：

- 完全依赖团队命名约定
- Shadow DOM 一刀切导致组件库无法使用
- 全局样式如 antd 被重复加载

**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：Shadow DOM、Scoped CSS 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> Shadow DOM；Scoped CSS；命名空间约定；构建时前缀；基座统一重置与主题变量，减少全局样式冲突。同时要避免完全依赖团队命名约定。

---

### FB-26-PE-A-016：微前端如何优化首屏加载性能

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、性能、首屏、预加载、懒加载
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明微前端架构下首屏性能优化的关键手段。

**参考答案**：

核心思路如下：

1. 按需加载：只加载当前路由子应用及必要资源
2. 预加载：空闲时预加载常用子应用入口
3. 共享依赖：统一 externals 或 shared，避免重复加载框架
4. 产物优化：路由级 code split、tree shaking、压缩
5. 基座瘦身：基座只负责路由和加载，不承载业务逻辑

需要避免的典型误区：

- 所有子应用一次性加载
- 每个子应用都打包一份 React/Vue
- 基座体积过大


**补充说明**：

在实际落地 微前端如何优化首屏加载性能 时，建议结合 微前端、性能、首屏 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：按需加载、预加载 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 按需加载；预加载；共享依赖；产物优化；基座瘦身。同时要避免所有子应用一次性加载。

---

### FB-26-FS-A-011：微前端子应用加载失败如何排查

**题型**：故障排查题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、故障排查、加载、网络、资源
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
某子应用在微前端基座中白屏，请给出排查思路。

**参考答案**：

核心思路如下：

1. 查看网络面板：remoteEntry 或子应用资源是否 404/超时
2. 检查控制台报错：JS 执行错误、依赖共享冲突、全局变量污染
3. 确认子应用独立运行是否正常，区分基座问题与子应用问题
4. 查看生命周期导出是否正确
5. 通过 Source Map 还原堆栈，定位代码问题
...
在实际落地 微前端子应用加载失败如何排查 时，建议结合 微前端、故障排查、加载 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：查看网络面板、检查控制台报错 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 某子应用在微前端基座中白屏，查看网络面板；检查控制台报错；确认子应用独立运行是否正常，区分基座问题与子应用问题；查看生命周期导出是否正确；通过 Source Map 还原堆栈，定位代码问题。同时要避免不区分子应用独立运行与基座集成问题。

---

### FB-26-EN-A-015：微前端项目的 CI/CD 如何设计

**题型**：工程化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、CI/CD、独立部署、版本管理、回滚
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请为微前端架构设计 CI/CD 流程，支持子应用独立部署和基座统一回滚。

**参考答案**：

核心思路如下：

1. 子应用独立仓库或 Monorepo 中的独立包
2. 独立构建产物上传 CDN 或静态服务器，生成带版本号的入口地址
3. 基座配置子应用注册表，支持按环境指向不同版本
4. 发布时先灰度子应用，再更新注册表
5. 回滚：注册表切回旧版本，无需重新构建
...
在实际落地 微前端项目的 CI/CD 如何设计 时，建议结合 微前端、CI/CD、独立部署 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：子应用独立仓库或 Monorepo 中的独立包、独立构建产物上传 CDN 或静态服务器，生成带版本号的入口地址 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 子应用独立仓库或 Monorepo 中的独立包；独立构建产物上传 CDN 或静态服务器，生成带版本号的入口地址；基座配置子应用注册表，支持按环境指向不同版本；发布时先灰度子应用，再更新注册表；回滚。同时要避免所有子应用必须同时发布。

---

### FB-26-CP-R-027：微前端拆分粒度如何把握

**题型**：综合开放题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：26 微前端
**标签**：微前端、拆分粒度、业务域、团队边界
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请说明微前端子应用拆分的原则，如何避免过细或过粗。

**参考答案**：

核心思路如下：

1. 按业务域而非技术层拆分，每个子应用对应一个 Bounded Context
2. 团队边界：一个子应用最好由一个团队负责
3. 粒度适中：过细导致集成复杂、通信频繁；过粗失去独立价值
4. 公共页面或组件下沉为共享包
5. 随着业务演进可合并或拆分
...
在实际落地 微前端拆分粒度如何把握 时，建议结合 微前端、拆分粒度、业务域 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：按业务域而非技术层拆分，每个子应用对应一个 Bounded Context、团队边界 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 按业务域而非技术层拆分，每个子应用对应一个 Bounded Context；团队边界；粒度适中；公共页面或组件下沉为共享包；随着业务演进可合并或拆分。同时要避免按页面拆分导致子应用过多。

---

### FB-26-CO-P-004：微前端中的 JS 沙箱实现方式

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：微前端、JS 沙箱、Proxy、快照、with
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请解释微前端 JS 沙箱的作用与实现方案，分析各自的优缺点。

**参考答案**：

核心思路如下：

1. Proxy 沙箱：代理 window，隔离全局变量，子应用卸载后恢复
2. 快照沙箱：挂载前拍 window 快照，卸载后恢复，简单但无法多实例
3. with + iframe：隔离强但通信复杂
4. VM / QuickJS：更严格但性能与兼容性有代价
5. 现代框架：Qiankun 使用 Proxy + 快照降级
...
**口头回答版**：
> Proxy 沙箱；快照沙箱；with + iframe；VM / QuickJS；现代框架。同时要避免认为沙箱能 100% 隔离。

---

### FB-26-SC-A-015：微前端中如何共享登录态与权限

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、登录态、权限、Token、SSO
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请设计微前端架构下的统一登录态与权限管理方案。

**参考答案**：

核心思路如下：

1. 登录态由基座统一维护，通过 props 或全局状态注入子应用
2. Token 存储在 httpOnly cookie 或基座内存，避免各子应用直接处理安全逻辑
3. 权限数据由基座获取并下发，子应用只做渲染控制
4. 子应用独立运行时通过 mock 或本地存储模拟权限
5. 退出登录时基座统一清理并通知子应用

需要避免的典型误区：

- 每个子应用各自请求登录态
- Token 存在 localStorage 被各子应用读取
- 权限逻辑散落在子应用难以审计

**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：登录态由基座统一维护，通过 props 或全局状态注入子应用、Token 存储在 httpOnly cookie 或基座内存，避免各子应用直接处理安全逻辑 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 登录态由基座统一维护，通过 props 或全局状态注入子应用；Token 存储在 httpOnly cookie 或基座内存，避免各子应用直接处理安全逻辑；权限数据由基座获取并下发，子应用只做渲染控制；子应用独立运行时通过 mock 或本地存储模拟权限；退出登录时基座统一清理并通知子应用。同时要避免每个子应用各自请求登录态。

---

### FB-26-PE-P-002：微前端如何治理公共依赖版本

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：微前端、公共依赖、版本、共享、Module Federation
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
多个子应用使用不同版本 React，如何在基座中统一或兼容。

**参考答案**：

核心思路如下：

1. 优先统一主框架版本，由基座 externals 加载
2. Module Federation shared 按 semver 匹配，版本不一致时降级加载
3. 子应用构建时输出依赖清单，基座运行时决定加载策略
4. 关键依赖升级走 RFC，制定迁移窗口
5. 提供 polyfill/shim 处理破坏性变更

需要避免的典型误区：

- 每个子应用都打包 React 导致体积爆炸
- 强制所有子应用同时升级大版本
- 版本冲突未处理导致运行时 hook 错误

**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：优先统一主框架版本，由基座 externals 加载、Module Federation shared 按 semver 匹配，版本不一致时降级加载 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 多个子应用使用不同版本 React，如何在基座中统一或兼容。优先统一主框架版本，由基座 externals 加载；Module Federation shared 按 semver 匹配，版本不一致时降级加载；子应用构建时输出依赖清单，基座运行时决定加载策略；关键依赖升级走 RFC，制定迁移窗口；提供 polyfill/shim 处理破坏性变更。同时要避免每个子应用都打包 React 导致体积爆炸。

---

### FB-26-SD-P-021：设计微前端下的路由与导航体系

**题型**：系统设计题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：微前端、路由、History、基座、子应用
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请设计微前端基座与子应用的路由协作方案，支持浏览器前进后退和子应用内跳转。

**参考答案**：

核心思路如下：

1. 基座路由作为一级路由，子应用路由作为二级路由
2. 基座监听 popstate，根据 path 激活对应子应用
3. 子应用使用自己的路由库，base 路径由基座注入
4. URL 约定：#/app/sub-path 或 /app/sub-path
5. 导航菜单由基座统一渲染或各子应用注册

需要避免的典型误区：

- 基座和子应用同时操作 history 导致冲突
- 子应用内跳转基座无法感知
- 刷新 404 未配置服务端路由
...
在实际落地 设计微前端下的路由与导航体系 时，建议结合 微前端、路由、History 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：基座路由作为一级路由，子应用路由作为二级路由、基座监听 popstate，根据 path 激活对应子应用 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 基座路由作为一级路由，子应用路由作为二级路由；基座监听 popstate，根据 path 激活对应子应用；子应用使用自己的路由库，base 路径由基座注入；URL 约定；导航菜单由基座统一渲染或各子应用注册。同时要避免基座和子应用同时操作 history 导致冲突。

---

### FB-26-CO-A-021：Qiankun 与 Module Federation 的主要区别

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：Qiankun、Module Federation、微前端、运行时、构建时
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请比较 Qiankun 和 Module Federation 在集成方式、技术栈要求、共享能力上的差异。

**参考答案**：

核心思路如下：

1. Qiankun 运行时加载子应用 HTML/JS，框架无关，提供沙箱和生命周期
2. Module Federation 构建时约定远程模块，运行时动态导入，同构技术栈体验更好
3. Qiankun 适合异构系统快速接入
4. MF 适合同技术栈、组件级共享、低耦合
5. 两者可结合：基座用 Qiankun 管理子应用，子应用内用 MF 共享组件
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：Qiankun 运行时加载子应用 HTML/JS，框架无关，提供沙箱和生命周期、Module Federation 构建时约定远程模块，运行时动态导入，同构技术栈体验更好 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> Qiankun 运行时加载子应用 HTML/JS，框架无关，提供沙箱和生命周期；Module Federation 构建时约定远程模块，运行时动态导入，同构技术栈体验更好；Qiankun 适合异构系统快速接入；MF 适合同技术栈、组件级共享、低耦合；两者可结合。同时要避免认为 MF 只能用于微前端。

---

### FB-26-FS-P-018：微前端子应用内存泄漏如何定位与治理

**题型**：故障排查题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：微前端、内存泄漏、卸载、事件、定时器
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
子应用频繁切换后内存持续增长，请给出排查和治理方案。

**参考答案**：

核心思路如下：

1. 确认泄漏发生在子应用还是基座，用 Memory 面板对比快照
2. 检查 unmount 时是否清理事件监听、定时器、WebSocket、IntersectionObserver
3. 检查全局变量是否未释放、DOM 是否未移除
4. 基座确保卸载子应用 DOM 并调用生命周期
5. 建立自动化测试：切换子应用后内存回归基线
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：确认泄漏发生在子应用还是基座，用 Memory 面板对比快照、检查 unmount 时是否清理事件监听、定时器、WebSocket、IntersectionObserver 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 子应用频繁切换后内存持续增长，确认泄漏发生在子应用还是基座，用 Memory 面板对比快照；检查 unmount 时是否清理事件监听、定时器、WebSocket、IntersectionObserver；检查全局变量是否未释放、DOM 是否未移除；基座确保卸载子应用 DOM 并调用生命周期；建立自动化测试。同时要避免子应用没有 unmount 生命周期。

---

### FB-26-SC-P-023：微前端下如何保障全链路质量

**题型**：场景设计题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：微前端、E2E、集成测试、契约、质量
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请说明微前端架构下的测试策略，如何防止子应用独立正常但集成失败。

**参考答案**：

核心思路如下：

1. 子应用独立单测 + 集成测试，保障各自质量
2. 基座集成测试：验证路由、加载、通信、公共依赖
3. 契约测试：子应用暴露的生命周期、API、事件格式受约束
4. E2E：覆盖跨子应用流程，如 A 应用下单到 B 应用支付
5. 可视化回归：防止子应用样式污染基座
...
在实际落地 微前端下如何保障全链路质量 时，建议结合 微前端、E2E、集成测试 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：子应用独立单测 + 集成测试，保障各自质量、基座集成测试 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 子应用独立单测 + 集成测试，保障各自质量；基座集成测试；契约测试；E2E；可视化回归。同时要避免只测子应用不测集成。

---

### FB-26-CP-P-025：何时应该选择微前端，何时不应该

**题型**：综合开放题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：微前端、适用场景、组织、成本、过度设计
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. 适合：多团队并行、业务域清晰、独立部署诉求强、技术栈需要渐进演进
2. 适合：巨石应用构建/发布严重阻塞
3. 不适合：团队规模小、业务边界模糊、为拆分而拆分
4. 不适合：对首屏性能要求极高且子应用过多
5. 关键判断：独立部署和团队自治带来的收益是否大于集成复杂度
...
在实际落地 何时应该选择微前端，何时不应该 时，建议结合 微前端、适用场景、组织 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：适合、适合 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 适合；适合；不适合；不适合；关键判断。同时要避免为了简历亮点引入微前端。

---

### FB-26-EN-R-029：微前端架构下的监控与可观测体系

**题型**：工程化题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：26 微前端
**标签**：微前端、监控、可观测、Trace、错误上报
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请为微前端系统设计监控方案，能区分基座与子应用的问题。

**参考答案**：

核心思路如下：

1. 统一 Trace：基座生成 trace_id，子应用请求携带
2. 错误上报：区分基座错误、子应用错误、加载错误
3. 性能：记录子应用加载耗时、首次渲染时间、资源体积
4. SLI/SLO：按子应用定义可用性、错误率目标
5. 仪表盘：基座视角 + 子应用视角，支持下钻
...
在实际落地 微前端架构下的监控与可观测体系 时，建议结合 微前端、监控、可观测 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：统一 Trace、错误上报 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 统一 Trace；错误上报；性能；SLI/SLO；仪表盘。同时要避免所有错误混在一起无法定位子应用。

---

### FB-26-CO-A-022：微前端如何影响 SEO

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、SEO、SSR、预渲染、爬虫
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请分析微前端架构对 SEO 的影响及应对方案。

**参考答案**：

核心思路如下：

1. 客户端渲染的微前端对 SEO 不友好，爬虫可能无法执行 JS
2. 方案：子应用 SSR/SSG，基座根据 UA 返回渲染结果
3. 对非 SEO 页面可用 CSR，关键页面用预渲染
4. 使用 History API 路由，避免 hash 路由影响收录
5. 提供 sitemap 和预渲染快照
...
- 所有子应用都用 CSR 还想 SEO
- 使用 hash 路由导致爬虫无法解析
- 忽略预渲染后的 hydration 状态同步

**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：客户端渲染的微前端对 SEO 不友好，爬虫可能无法执行 JS、方案 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 客户端渲染的微前端对 SEO 不友好，爬虫可能无法执行 JS；方案；对非 SEO 页面可用 CSR，关键页面用预渲染；使用 History API 路由，避免 hash 路由影响收录；提供 sitemap 和预渲染快照。同时要避免所有子应用都用 CSR 还想 SEO。

---

### FB-26-SC-A-016：微前端子应用如何独立开发与联调

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、开发体验、独立运行、Mock、代理
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 子应用支持独立运行，通过环境变量判断是否在基座中
2. 本地开发：子应用 standalone 模式 + mock 基座注入
3. 联调：基座本地指向子应用 dev server 地址
4. 统一代理配置，解决跨域和登录态
5. 文档化子应用接口与注册方式
...
在实际落地 微前端子应用如何独立开发与联调 时，建议结合 微前端、开发体验、独立运行 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：子应用支持独立运行，通过环境变量判断是否在基座中、本地开发 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 子应用支持独立运行，通过环境变量判断是否在基座中；本地开发；联调；统一代理配置，解决跨域和登录态；文档化子应用接口与注册方式。同时要避免子应用只能在基座中运行，开发效率低。

---

### FB-26-SD-R-028：设计一个支持离线能力的微前端应用

**题型**：系统设计题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：26 微前端
**标签**：微前端、PWA、Service Worker、离线、缓存
**出现频率**：低频
**预计回答时长**：15-30 分钟

**题目描述**：
请设计一个微前端架构下的离线可用方案，支持子应用资源缓存和更新。

**参考答案**：

核心思路如下：

1. 基座注册 Service Worker，统一拦截所有子应用资源请求
2. 缓存策略：基座核心 HTML 网络优先，子应用 JS/CSS 缓存优先 + hash
3. 子应用更新后通过注册表更新缓存，支持旧版清理
4. 离线时展示已缓存子应用，未缓存子应用提示联网
5. 后台同步关键用户操作
...
在实际落地 设计一个支持离线能力的微前端应用 时，建议结合 微前端、PWA、Service Worker 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：基座注册 Service Worker，统一拦截所有子应用资源请求、缓存策略 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 基座注册 Service Worker，统一拦截所有子应用资源请求；缓存策略；子应用更新后通过注册表更新缓存，支持旧版清理；离线时展示已缓存子应用，未缓存子应用提示联网；后台同步关键用户操作。同时要避免每个子应用各自注册 SW 导致冲突。

---

### FB-26-CO-P-005：微前端中的公共状态管理反模式
...
**题目描述**：
请说明微前端架构下共享状态的常见反模式与推荐做法。

**参考答案**：

核心思路如下：

1. 反模式：全局 store 被所有子应用读写，边界模糊
2. 推荐：状态按所有权归属，跨子应用状态通过事件传递
3. 全局只保留用户、主题、语言等基础上下文
4. 子应用内部状态自治，不暴露给外部
5. 需要共享的业务状态上升为基座或 BFF 管理
...
**口头回答版**：
> 反模式；推荐；全局只保留用户、主题、语言等基础上下文；子应用内部状态自治，不暴露给外部；需要共享的业务状态上升为基座或 BFF 管理。同时要避免所有子应用共享 Redux store。

---

### FB-26-PE-R-031：微前端大规模下的性能预算与治理

**题型**：性能优化题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：26 微前端
**标签**：微前端、性能预算、包体积、治理、度量
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请说明如何在微前端架构下制定和落实性能预算。

**参考答案**：

核心思路如下：

1. 按子应用制定 JS/CSS 体积预算、请求数、LCP 目标
2. CI 阶段检查子应用产物是否超预算
3. 共享依赖统一 external，避免重复打包
4. 建立性能看板，按子应用、版本、路由展示
5. 超标处理：阻断发布或要求负责人审批
...
在实际落地 微前端大规模下的性能预算与治理 时，建议结合 微前端、性能预算、包体积 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：按子应用制定 JS/CSS 体积预算、请求数、LCP 目标、CI 阶段检查子应用产物是否超预算 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 按子应用制定 JS/CSS 体积预算、请求数、LCP 目标；CI 阶段检查子应用产物是否超预算；共享依赖统一 external，避免重复打包；建立性能看板，按子应用、版本、路由展示；超标处理。同时要避免只关注基座性能忽略子应用。

---

### FB-26-SC-P-024：微前端中如何处理第三方脚本与全局组件

**题型**：场景设计题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：微前端、第三方脚本、全局组件、沙箱、污染
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
子应用需要加载第三方 SDK 或共享全局弹窗组件，如何设计？

**参考答案**：

核心思路如下：

1. 第三方脚本由基座统一加载一次，通过 props 或事件供子应用使用
2. 全局组件（如支付弹窗）封装为共享包或远程模块
3. 脚本加载使用异步加载和错误兜底
4. 防止第三方脚本污染 window，使用 Proxy 沙箱或 iframe 隔离
5. 明确第三方脚本的升级与回滚责任方

需要避免的典型误区：

- 每个子应用各自加载相同 SDK
- 第三方脚本直接操作 DOM 导致沙箱失效
- 全局组件与子应用状态耦合


**补充说明**：

在实际落地 微前端中如何处理第三方脚本与全局组件 时，建议结合 微前端、第三方脚本、全局组件 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：第三方脚本由基座统一加载一次，通过 props 或事件供子应用使用、全局组件（如支付弹窗）封装为共享包或远程模块 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 子应用需要加载第三方 SDK 或共享全局弹窗组件，如何设计？第三方脚本由基座统一加载一次，通过 props 或事件供子应用使用；全局组件（如支付弹窗）封装为共享包或远程模块；脚本加载使用异步加载和错误兜底；防止第三方脚本污染 window，使用 Proxy 沙箱或 iframe 隔离；明确第三方脚本的升级与回滚责任方。同时要避免每个子应用各自加载相同 SDK。

---

### FB-26-FS-A-012：微前端子应用样式突然错乱如何定位

**题型**：故障排查题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、CSS、样式隔离、优先级、排查
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
某子应用在基座中样式正常，另一子应用切换后回来样式错乱，如何排查？

**参考答案**：

核心思路如下：

1. 检查全局样式是否未隔离，如 antd 被重复加载覆盖
2. 查看 CSS 选择器优先级是否被子应用或基座全局样式影响
3. 确认子应用卸载时是否清除了动态注入的 style 标签
4. 检查 Shadow DOM 内外样式穿透
5. 用浏览器 DevTools 对比正常与异常时的 Computed Style

需要避免的典型误区：

- 只看自己子应用的样式，不检查基座
- 忽略子应用卸载未清理的样式
- 没有使用 Scoped CSS 或 CSS Modules

**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：检查全局样式是否未隔离，如 antd 被重复加载覆盖、查看 CSS 选择器优先级是否被子应用或基座全局样式影响 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 某子应用在基座中样式正常，另一子应用切换后回来样式错乱，如何排查？检查全局样式是否未隔离，如 antd 被重复加载覆盖；查看 CSS 选择器优先级是否被子应用或基座全局样式影响；确认子应用卸载时是否清除了动态注入的 style 标签；检查 Shadow DOM 内外样式穿透；用浏览器 DevTools 对比正常与异常时的 Computed Style。同时要避免只看自己子应用的样式，不检查基座。

---

### FB-26-CP-A-003：微前端落地中最大的风险是什么
...
**题目描述**：
如果要在公司落地微前端，你认为最大的三个风险是什么，如何规避？

**参考答案**：

核心思路如下：

1. 组织风险：团队边界不清，子应用成为互相扯皮的边界
2. 技术风险：集成复杂、调试困难、性能下降
3. 治理风险：公共依赖版本漂移、接口契约失控、质量不可见
4. 规避：明确业务域与团队责任，建立 RFC 与治理委员会
5. 逐步推进：先试点再推广，度量收益与成本
...
在实际落地 微前端落地中最大的风险是什么 时，建议结合 微前端、风险、组织 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：组织风险、技术风险 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 如果要在公司落地微前端，你认为最大的三个风险是什么，如何规避？组织风险；技术风险；治理风险；规避；逐步推进。同时要避免低估组织复杂度。

---

### FB-26-EN-P-022：微前端子应用的构建产物如何组织

**题型**：工程化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：26 微前端
**标签**：微前端、构建产物、CDN、版本、注册表
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明微前端子应用构建产物的目录、命名、版本管理和基座加载方式。

**参考答案**：

核心思路如下：

1. 产物按版本目录存放，如 /subapp/order/v1.2.3/
2. 入口文件命名固定：index.html 或 remoteEntry.js
3. 静态资源带 hash，长期缓存；入口文件短缓存
4. 基座通过注册表获取当前环境子应用版本列表
5. 支持按环境、灰度、用户分组指向不同版本
...
在实际落地 微前端子应用的构建产物如何组织 时，建议结合 微前端、构建产物、CDN 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：产物按版本目录存放，如 /subapp/order/v1.2.3/、入口文件命名固定 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 产物按版本目录存放，如 /subapp/order/v1.2.3/；入口文件命名固定；静态资源带 hash，长期缓存；入口文件短缓存；基座通过注册表获取当前环境子应用版本列表；支持按环境、灰度、用户分组指向不同版本。同时要避免所有子应用产物放在同一目录。

---

### FB-26-SD-R-029：设计一个无基座的去中心化微前端方案

**题型**：系统设计题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：26 微前端
**标签**：微前端、去中心化、路由、Module Federation、边缘
**出现频率**：低频
**预计回答时长**：15-30 分钟

**题目描述**：
请设计一个不需要统一基座的微前端架构，各子应用可独立运行也可组合。

**参考答案**：

核心思路如下：

1. 使用 Web Components 或 Module Federation 暴露组件
2. 入口聚合：边缘网关或 SSR 根据路由拼接页面
3. 共享设计系统组件库作为 Web Components
4. 路由：各应用使用约定 URL，通过 shell 页面或边缘编排
5. 状态：通过浏览器存储、URL、事件总线通信
6. 治理：统一组件注册与版本发现服务
...
在实际落地 设计一个无基座的去中心化微前端方案 时，建议结合 微前端、去中心化、路由 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：使用 Web Components 或 Module Federation 暴露组件、入口聚合 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 使用 Web Components 或 Module Federation 暴露组件；入口聚合；共享设计系统组件库作为 Web Components；路由；状态；治理。同时要避免去中心化导致无统一体验。

---

### FB-26-CO-B-010：single-spa 与 Qiankun 的关系

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：26 微前端
**标签**：single-spa、Qiankun、微前端、生命周期
**出现频率**：高频
**预计回答时长**：3-5 分钟

**题目描述**：
请解释 single-spa 和 Qiankun 的关系与区别。

**参考答案**：

核心思路如下：

1. single-spa 是微前端框架，提供路由分发和生命周期规范
2. Qiankun 基于 single-spa，增加了 JS 沙箱、CSS 隔离、HTML Entry、预加载等能力
3. Qiankun 更易用，适合快速落地；single-spa 更灵活但需要自己封装
4. 两者都不限定构建工具

需要避免的典型误区：

- 认为 Qiankun 和 single-spa 完全独立
- 忽略 single-spa 的生命周期约定
- 把 Qiankun 当唯一选择

**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：single-spa 是微前端框架，提供路由分发和生命周期规范、Qiankun 基于 single-spa，增加了 JS 沙箱、CSS 隔离、HTML Entry、预加载等能力 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> single-spa 是微前端框架，提供路由分发和生命周期规范；Qiankun 基于 single-spa，增加了 JS 沙箱、CSS 隔离、HTML Entry、预加载等能力；Qiankun 更易用，适合快速落地；single-spa 更灵活但需要自己封装；两者都不限定构建工具。同时要避免认为 Qiankun 和 single-spa 完全独立。

---

### FB-26-SC-A-017：微前端与 Monorepo 如何结合

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、Monorepo、依赖、复用、构建
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明微前端与 Monorepo 结合的优势和实践。

**参考答案**：

核心思路如下：

1. Monorepo 管理共享包、组件库、工具函数，子应用独立包
2. 统一代码规范、CI、依赖版本
3. 子应用可独立部署，也可通过 affected 构建提速
4. 公共依赖放在 workspace root，子应用通过 workspace 协议引用
5. 结合 Module Federation 在 Monorepo 内共享远程模块
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：Monorepo 管理共享包、组件库、工具函数，子应用独立包、统一代码规范、CI、依赖版本 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> Monorepo 管理共享包、组件库、工具函数，子应用独立包；统一代码规范、CI、依赖版本；子应用可独立部署，也可通过 affected 构建提速；公共依赖放在 workspace root，子应用通过 workspace 协议引用；结合 Module Federation 在 Monorepo 内共享远程模块。同时要避免Monorepo 与微前端概念混淆。

---

### FB-26-PE-A-017：微前端下的资源预加载策略

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、预加载、性能、加载时机、带宽
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请设计微前端子应用的资源预加载策略，平衡性能与带宽。

**参考答案**：

核心思路如下：

1. 基于路由预加载：用户 hover 菜单或进入父路由时预加载
2. 基于权限预加载：登录后预加载用户可见子应用入口
3. 基于空闲预加载：requestIdleCallback 加载低优先级资源
4. CDN 预热：将高频子应用资源推送到边缘节点
5. 避免预加载过多导致带宽浪费和主线程阻塞
...
在实际落地 微前端下的资源预加载策略 时，建议结合 微前端、预加载、性能 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：基于路由预加载、基于权限预加载 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 基于路由预加载；基于权限预加载；基于空闲预加载；CDN 预热；避免预加载过多导致带宽浪费和主线程阻塞。同时要避免一次性预加载所有子应用。

---

### FB-26-CO-B-011：微前端的四种基本集成方式

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、集成、iframe、Web Components、MF
**出现频率**：高频
**预计回答时长**：3-5 分钟
...
1. iframe：隔离强、体验弱，适合第三方页面
2. Web Components：标准组件化，适合共享 UI
3. Module Federation：同构技术栈组件共享
4. 路由分发：按路径加载子应用，最常见
5. 选择依据：技术栈、隔离要求、体验要求
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：iframe、Web Components 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> iframe；Web Components；Module Federation；路由分发；选择依据。同时要避免所有场景都用 iframe。

---

### FB-26-SC-A-018：微前端中的异常边界如何设计

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：26 微前端
**标签**：微前端、异常边界、降级、基座、子应用
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请设计微前端基座对子应用异常的捕获和降级机制。

**参考答案**：

核心思路如下：

1. 基座包裹子应用加载器，捕获加载和渲染异常
2. 异常时显示占位 UI 或重试按钮
3. 记录子应用名、版本、错误堆栈
4. 不影响其他子应用运行
5. 严重时通知运维并自动回滚版本
...
在实际落地 微前端中的异常边界如何设计 时，建议结合 微前端、异常边界、降级 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：基座包裹子应用加载器，捕获加载和渲染异常、异常时显示占位 UI 或重试按钮 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 基座包裹子应用加载器，捕获加载和渲染异常；异常时显示占位 UI 或重试按钮；记录子应用名、版本、错误堆栈；不影响其他子应用运行；严重时通知运维并自动回滚版本。同时要避免子应用异常导致基座白屏。

---

### FB-26-CP-A-004：微前端落地后的持续治理措施
...
在实际落地 微前端落地后的持续治理措施 时，建议结合 微前端、治理、版本 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：子应用注册表和版本管理、公共依赖升级 RFC 和迁移窗口 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 子应用注册表和版本管理；公共依赖升级 RFC 和迁移窗口；子应用质量评分与下线规则；跨子应用接口和事件契约维护；定期复盘集成问题和技术债。同时要避免上线后无人管理。
