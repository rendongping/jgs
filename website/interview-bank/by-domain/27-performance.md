...
> 本题库共收录 **86** 道面试题（基础 11 / 进阶 33 / 深入 26 / 架构 16）。
...
### FB-27-CO-B-009：浏览器渲染管线的关键阶段

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：27 性能优化
**标签**：渲染管线、CRP、重排、重绘、合成
**出现频率**：高频
**预计回答时长**：3-5 分钟

**题目描述**：
请描述从输入 URL 到页面显示的主要步骤，以及优化关键渲染路径的方法。

**参考答案**：

核心思路如下：

1. DNS/TCP/TLS、请求 HTML、解析 DOM/CSSOM、执行 JS、构建渲染树、布局、绘制、合成
2. 优化：减少关键资源数量、压缩体积、使用 preload/prefetch
3. 避免阻塞渲染的 JS：defer/async、代码分割
4. CSS 放头部，JS 放底部或异步
5. 减少重排重绘：使用 transform/opacity 触发合成层

需要避免的典型误区：

- 把重绘和重排混为一谈
- 认为所有 CSS 都会阻塞渲染
- 滥用 will-change 导致内存暴涨

**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：DNS/TCP/TLS、请求 HTML、解析 DOM/CSSOM、执行 JS、构建渲染树、布局、绘制、合成、优化 等关键点
...
**口头回答版**：
> DNS/TCP/TLS、请求 HTML、解析 DOM/CSSOM、执行 JS、构建渲染树、布局、绘制、合成；优化；避免阻塞渲染的 JS；CSS 放头部，JS 放底部或异步；减少重排重绘。同时要避免把重绘和重排混为一谈。

---

### FB-27-CO-A-013：Core Web Vitals 指标含义与优化

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：Web Vitals、LCP、INP、CLS、性能指标
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请解释 LCP、INP、CLS 的含义、目标阈值及优化手段。

**参考答案**：

核心思路如下：

1. LCP：最大内容绘制，反映加载体验，目标 ≤2.5s
2. INP：交互到下一次绘制，反映交互响应，目标 ≤200ms
3. CLS：累积布局偏移，反映视觉稳定性，目标 ≤0.1
4. LCP 优化：预加载关键资源、图片压缩、SSR、CDN、字体优化
5. INP 优化：长任务拆分、事件处理轻量化、使用 requestIdleCallback
6. CLS 优化：图片/视频设置尺寸、避免插入无尺寸内容、字体预留空间
...
**口头回答版**：
> LCP；INP；CLS；LCP 优化；INP 优化；CLS 优化。同时要避免只看 FP/FCP 忽略 LCP。

---

### FB-27-PE-A-005：首屏加载性能优化清单

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：首屏、性能优化、代码分割、SSR、预加载
**出现频率**：高频
**预计回答时长**：8-15 分钟
...
1. 减少请求体积：tree shaking、压缩、图片 WebP/AVIF、CDN
2. 代码分割：按路由/组件动态 import，首屏只加载必要代码
3. SSR/SSG：减少客户端渲染时间，直接输出 HTML
4. 资源优先级：preload 关键 CSS/字体，prefetch 下一页
5. 缓存策略：长缓存静态资源、Service Worker 离线缓存
6. 服务端优化：Brotli、HTTP/2、边缘缓存
...
**口头回答版**：
> 减少请求体积；代码分割；SSR/SSG；资源优先级；缓存策略；服务端优化。同时要避免只优化 JS 忽略字体和图片。

---

### FB-27-PE-P-015：如何优化大型列表渲染性能

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：虚拟列表、长列表、渲染优化、DOM、性能
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. 瓶颈：DOM 节点过多导致布局、绘制、内存开销大
2. 虚拟滚动：只渲染可视区 + 缓冲区的节点，如 react-window
3. 分页/无限滚动：减少一次性数据量
4. 避免在滚动时执行复杂计算和 setState
5. 使用 CSS contain、will-change 优化渲染层
6. 大数据量时考虑 Canvas/WebGL 渲染
...
**口头回答版**：
> 瓶颈；虚拟滚动；分页/无限滚动；避免在滚动时执行复杂计算和 setState；使用 CSS contain、will-change 优化渲染层；大数据量时考虑 Canvas/WebGL 渲染。同时要避免一次性渲染全部数据。

---

### FB-27-CD-A-006：React.memo/useMemo/useCallback 的使用边界

**题型**：辨析题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：React、memo、useMemo、useCallback、渲染优化
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请辨析 React 中三种缓存手段的作用、适用场景与误用风险。

**参考答案**：

核心思路如下：

1. React.memo：缓存组件渲染结果，props 浅比较
2. useMemo：缓存计算结果，避免重复复杂计算
3. useCallback：缓存函数引用，配合 memo 使用
4. 适用：高开销计算、子组件依赖稳定 callback、大数据处理
5. 误用：缓存简单计算增加开销、依赖数组错误导致 bug

需要避免的典型误区：

- 所有组件都包 React.memo
- useCallback 不加依赖或依赖过多
- 把 useMemo 当万能缓存

**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：React.memo、useMemo 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> React.memo；useMemo；useCallback；适用；误用。同时要避免所有组件都包 React.memo。

---

### FB-27-PE-P-016：如何减少 JavaScript 执行时间

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：JS 执行、长任务、性能、Web Worker、代码拆分
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. 代码分割与懒加载，减少首屏 JS 体积
2. 长任务拆分为多个小任务，使用 setTimeout/requestIdleCallback/yieldToMain
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：代码分割与懒加载，减少首屏 JS 体积、长任务拆分为多个小任务，使用 setTimeout/requestIdleCallback/yieldToMain 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 代码分割与懒加载，减少首屏 JS 体积；长任务拆分为多个小任务，使用 setTimeout/requestIdleCallback/yieldToMain；Web Worker 处理计算密集型任务；避免在循环中触发重排；使用 requestAnimationFrame 做动画；减少第三方脚本和 polyfill。同时要避免所有逻辑放主线程。

---

### FB-27-PE-R-007：设计一个前端性能治理体系

**题型**：性能优化题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：27 性能优化
**标签**：性能治理、性能预算、监控、CI、度量
**出现频率**：中频
**预计回答时长**：15-30 分钟

**题目描述**：
请为公司级前端产品设计性能治理体系，覆盖指标、工具、流程和组织。

**参考答案**：

核心思路如下：

1. 指标：Web Vitals、自定义业务指标、P75/P95 长尾
2. 工具：Lighthouse、web-vitals、RUM、包体积分析
3. 流程：MR 阶段性能门禁、发布前回归、线上监控告警
4. 性能预算：JS/CSS 体积、请求数、LCP/INP 阈值
5. 组织：性能负责人、定期 review、优秀案例分享
...
在实际落地 设计一个前端性能治理体系 时，建议结合 性能治理、性能预算、监控 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：指标、工具 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 指标；工具；流程；性能预算；组织。同时要避免只看平均值。

---

### FB-27-CO-A-014：HTTP 缓存策略在前端的使用

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：HTTP 缓存、Cache-Control、ETag、强缓存、协商缓存
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明强缓存与协商缓存的原理及前端如何配合。

**参考答案**：

核心思路如下：

1. 强缓存：Cache-Control/Expires，命中不请求服务器
2. 协商缓存：Last-Modified/ETag，询问服务器是否变化
3. HTML 使用协商缓存或短缓存，带 hash 资源使用强缓存长期缓存
4. 版本更新：文件名 hash 变化强制失效
5. CDN 与源站缓存策略一致
...
**口头回答版**：
> 强缓存；协商缓存；HTML 使用协商缓存或短缓存，带 hash 资源使用强缓存长期缓存；版本更新；CDN 与源站缓存策略一致。同时要避免HTML 强缓存导致发版不生效。

---

### FB-27-PE-A-006：图片性能优化手段

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：图片优化、WebP、懒加载、响应式、压缩
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. 格式选择：WebP/AVIF 替代 JPEG/PNG，SVG 适合图标
2. 响应式图片：srcset/sizes、picture 元素
3. 懒加载：loading="lazy" 或 Intersection Observer
4. 压缩：有损/无损压缩、动态生成多尺寸
5. 占位：低质量占位图或模糊背景，减少 CLS
6. CDN 图片处理：按需裁剪、格式转换
...
**口头回答版**：
> 格式选择；响应式图片；懒加载；压缩；占位；CDN 图片处理。同时要避免所有图片用 PNG。

---

### FB-27-SC-A-008：如何排查页面卡顿

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：性能排查、卡顿、Performance、Long Task、内存
**出现频率**：高频
**预计回答时长**：8-15 分钟
...
1. 复现：稳定复现路径、设备、浏览器版本
2. Performance 面板：定位长任务、重排、重绘、JS 执行
3. 监控 INP/FID、帧率、主线程阻塞时长
4. 检查事件处理函数、第三方脚本、大数据渲染
5. 使用火焰图分析调用栈
6. 验证修复后回归测试
...
在实际落地 排查页面卡顿 时，建议结合 性能排查、卡顿、Performance 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：复现、Performance 面板 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 用户反馈页面操作卡顿，复现；Performance 面板；监控 INP/FID、帧率、主线程阻塞时长；检查事件处理函数、第三方脚本、大数据渲染；使用火焰图分析调用栈；验证修复后回归测试。同时要避免只看网络不看主线程。

---

### FB-27-PE-P-017：CSS 性能优化与渲染层管理

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：CSS、渲染层、重排、重绘、合成、contain
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明 CSS 如何影响渲染性能，以及如何管理渲染层。

**参考答案**：

核心思路如下：

1. 重排属性：width/height/top/left 会触发布局
2. 重绘属性：color/background 触发绘制
3. 合成属性：transform/opacity 触发 GPU 合成，性能最好
4. 避免布局抖动：批量读写样式，使用 FLIP 动画
5. contain 属性隔离渲染范围，减少影响面
6. 谨慎使用 will-change，用完移除
...
**口头回答版**：
> 重排属性；重绘属性；合成属性；避免布局抖动；contain 属性隔离渲染范围，减少影响面；谨慎使用 will-change，用完移除。同时要避免动画使用 top/left 而非 transform。

---

### FB-27-PE-A-007：字体加载优化策略

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：字体、FOIT、FOUT、预加载、font-display
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请说明 web 字体加载的性能问题及优化方案。

**参考答案**：

核心思路如下：

1. 问题：字体阻塞渲染导致 FOIT/FOUT/CLS
2. font-display: swap 先显示fallback再替换
3. preload 关键字体，减少下载时间
4. 只加载必要字重和子集
5. 使用系统字体栈兜底，提升感知速度
6. 变量字体减少请求数
...
- 加载整个字体文件
- 不设置 font-display 导致白屏
- 自定义字体无 fallback 导致布局偏移


**补充说明**：

在实际落地 字体加载优化策略 时，建议结合 字体、FOIT、FOUT 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：问题、font-display: swap 先显示fallback再替换 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 问题；font-display；preload 关键字体，减少下载时间；只加载必要字重和子集；使用系统字体栈兜底，提升感知速度；变量字体减少请求数。同时要避免加载整个字体文件。

---

### FB-27-CO-P-006：Resource Hints 的使用场景

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：Resource Hints、preload、prefetch、preconnect、dns-prefetch
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请解释 preload、prefetch、preconnect、dns-prefetch 的区别与使用时机。

**参考答案**：

核心思路如下：

1. preload：当前页面必定需要的资源，提升优先级
2. prefetch：下一页可能需要的资源，低优先级空闲加载
3. preconnect：提前建立连接，减少 DNS/TCP/TLS 耗时
4. dns-prefetch：仅提前解析 DNS
5. 不要滥用 preload，避免带宽竞争
...
**口头回答版**：
> preload；prefetch；preconnect；dns-prefetch；不要滥用 preload，避免带宽竞争。同时要避免对所有资源都 preload。

---

### FB-27-PE-R-008：如何优化前端包体积

**题型**：性能优化题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：27 性能优化
**标签**：包体积、Tree Shaking、代码分割、依赖治理、构建
**出现频率**：高频
**预计回答时长**：8-15 分钟
...
1. Tree Shaking：ESM、sideEffects 配置
2. 代码分割：路由/组件动态加载
3. 依赖治理：移除无用依赖、替换大库、按需加载组件库
4. 压缩：Terser、CSS 压缩、Gzip/Brotli
5. 图片/字体/多语言资源懒加载
6. 构建分析：webpack-bundle-analyzer、rollup-plugin-visualizer
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：Tree Shaking、代码分割 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> Tree Shaking；代码分割；依赖治理；压缩；图片/字体/多语言资源懒加载；构建分析。同时要避免引入全量组件库。

---

### FB-27-PE-P-018：内存泄漏的常见原因与排查

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：内存泄漏、Performance、DevTools、闭包、事件监听
**出现频率**：中频
**预计回答时长**：8-15 分钟
...
1. 未清理的事件监听、定时器、WebSocket
2. 闭包持有 DOM 引用
3. 全局变量或缓存无限增长
4. 组件卸载未取消异步请求
5. 排查：Memory 面板 Heap Snapshot 对比、Allocation Timeline
6. 定位：Detached DOM、Retainers 链路

需要避免的典型误区：

- 只看 Performance 不看 Memory
- 组件卸载后仍保留订阅
- 缓存无淘汰策略
...
在实际落地 内存泄漏的常见原因与排查 时，建议结合 内存泄漏、Performance、DevTools 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：未清理的事件监听、定时器、WebSocket、闭包持有 DOM 引用 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 未清理的事件监听、定时器、WebSocket；闭包持有 DOM 引用；全局变量或缓存无限增长；组件卸载未取消异步请求；排查；定位。同时要避免只看 Performance 不看 Memory。

---

### FB-27-SC-P-001：如何优化大量 DOM 操作的动画

**题型**：场景设计题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：动画、DOM、requestAnimationFrame、Web Animations、Canvas
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
页面需要同时驱动大量元素动画，请给出性能优化方案。

**参考答案**：

核心思路如下：

1. 优先使用 transform 和 opacity，触发 GPU 合成
2. 使用 requestAnimationFrame，与渲染帧对齐
3. 批量更新：FLIP 技术先计算最终状态再动画
4. 复杂场景用 Canvas/WebGL 替代 DOM
5. 减少同时动画元素数量，使用虚拟化或层级裁剪
6. 避免在动画中读取布局属性
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：优先使用 transform 和 opacity，触发 GPU 合成、使用 requestAnimationFrame，与渲染帧对齐 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 页面需要同时驱动大量元素动画，优先使用 transform 和 opacity，触发 GPU 合成；使用 requestAnimationFrame，与渲染帧对齐；批量更新；复杂场景用 Canvas/WebGL 替代 DOM；减少同时动画元素数量，使用虚拟化或层级裁剪；避免在动画中读取布局属性。同时要避免使用 top/left 做动画。

---

### FB-27-CO-A-015：SSR 与 CSR 的性能权衡

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：SSR、CSR、首屏、交互、TTFB、hydration
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请比较 SSR 和 CSR 在首屏、交互、复杂度、成本上的差异。

**参考答案**：

核心思路如下：

1. SSR：首屏 HTML 直接可渲染，TTFB 略高，适合 SEO 和首屏敏感
2. CSR：TTFB 低但需下载执行 JS，首屏慢，交互后体验好
3. SSR 需要服务器资源，hydration 可能有额外开销
4. 混合：关键页面 SSR，后台页面 CSR； Islands Architecture
5. 选择：内容型站点偏 SSR，交互型应用偏 CSR
...
**口头回答版**：
> SSR；CSR；SSR 需要服务器资源，hydration 可能有额外开销；混合；选择。同时要避免所有页面 SSR 不考虑成本。

---

### FB-27-PE-A-008：如何优化第三方脚本性能

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：第三方脚本、性能、async、defer、预连接
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明第三方 SDK/分析脚本对性能的影响及优化方法。

**参考答案**：

核心思路如下：

1. 影响：阻塞解析、额外请求、执行长任务、隐私合规
2. 优化：延迟加载到关键内容之后、使用 async/defer
3. 非关键脚本用 Intersection Observer 触发加载
4. 自托管关键脚本减少 DNS/TCP
5. 定期审计第三方脚本，移除无用
6. 使用 Partytown 将脚本移入 Web Worker
...
在实际落地 优化第三方脚本性能 时，建议结合 第三方脚本、性能、async 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：影响、优化 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 影响；优化；非关键脚本用 Intersection Observer 触发加载；自托管关键脚本减少 DNS/TCP；定期审计第三方脚本，移除无用；使用 Partytown 将脚本移入 Web Worker。同时要避免头部加载所有第三方脚本。

---

### FB-27-PE-P-019：前端性能监控指标体系设计

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：性能监控、RUM、指标、P75、归因
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请设计一个前端性能监控指标体系，能反映真实用户体验。

**参考答案**：

核心思路如下：

1. 加载类：LCP、FCP、TTFB、页面完全加载时间
2. 交互类：INP、FID、点击响应时间
3. 稳定性：CLS、JS 错误率、资源加载失败率
4. 业务：白屏时间、可交互时间、关键流程耗时
5. 分位数：关注 P75/P95 长尾而非均值
6. 归因：按页面、版本、网络、设备、地域拆分
...
在实际落地 前端性能监控指标体系设计 时，建议结合 性能监控、RUM、指标 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：加载类、交互类 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 加载类；交互类；稳定性；业务；分位数；归因。同时要避免只看均值。

---

### FB-27-SC-A-009：如何优化首屏白屏时间

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：白屏、首屏、SSR、骨架屏、预渲染
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. 骨架屏或 loading 占位，提升感知速度
2. 内联关键 CSS，异步加载非关键 CSS
3. 延迟非关键 JS，使用 defer/async
4. 预渲染/SSR 直接输出首屏 HTML
5. 减少关键渲染路径上的资源数量
6. 字体使用 swap 避免 FOIT
...
在实际落地 优化首屏白屏时间 时，建议结合 白屏、首屏、SSR 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：骨架屏或 loading 占位，提升感知速度、内联关键 CSS，异步加载非关键 CSS 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 骨架屏或 loading 占位，提升感知速度；内联关键 CSS，异步加载非关键 CSS；延迟非关键 JS，使用 defer/async；预渲染/SSR 直接输出首屏 HTML；减少关键渲染路径上的资源数量；字体使用 swap 避免 FOIT。同时要避免骨架屏时间过长。

---

### FB-27-CO-B-010：什么是重排与重绘
...
2. 重绘：外观属性变化导致重新绘制像素
3. 重排代价通常大于重绘
4. 减少：批量修改样式、使用 transform/opacity、避免布局抖动
5. 用 Chrome DevTools 的 Rendering 面板可视化

需要避免的典型误区：

- 认为重绘比重排更耗性能
- 频繁读取 offsetWidth 触发强制同步布局
- 在循环中逐个修改样式
...
**口头回答版**：
> 重排；重绘；重排代价通常大于重绘；减少；用 Chrome DevTools 的 Rendering 面板可视化。同时要避免认为重绘比重排更耗性能。

---

### FB-27-PE-A-009：SPA 路由切换性能优化

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：SPA、路由、懒加载、预加载、keep-alive
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 路由级代码分割，按需加载页面组件
2. 预加载下一页资源：hover/prefetch
3. keep-alive 缓存常用页面状态
4. 路由切换时保留骨架屏或过渡动画
5. 减少全局状态更新导致的无关组件渲染
...
在实际落地 SPA 路由切换性能优化 时，建议结合 SPA、路由、懒加载 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：路由级代码分割，按需加载页面组件、预加载下一页资源 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 路由级代码分割，按需加载页面组件；预加载下一页资源；keep-alive 缓存常用页面状态；路由切换时保留骨架屏或过渡动画；减少全局状态更新导致的无关组件渲染。同时要避免首屏加载所有路由代码。

---

### FB-27-PE-P-020：如何优化大量 marker 的地图性能

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：地图、Marker、Canvas、聚合、性能
**出现频率**：低频
**预计回答时长**：8-15 分钟

**题目描述**：
地图需要展示上万 marker，请给出前端性能优化方案。

**参考答案**：

核心思路如下：

1. 点聚合：按缩放级别聚合相邻 marker
2. 视口裁剪：只渲染可视区 marker
3. Canvas/WebGL 替代 DOM marker
4. 分层渲染：重要 marker 优先，低级别简化样式
5. 数据分页或瓦片加载
6. 使用专门的地图库如 Mapbox GL/Deck.gl
...
在实际落地 优化大量 marker 的地图性能 时，建议结合 地图、Marker、Canvas 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：点聚合、视口裁剪 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 地图需要展示上万 marker，点聚合；视口裁剪；Canvas/WebGL 替代 DOM marker；分层渲染；数据分页或瓦片加载；使用专门的地图库如 Mapbox GL/Deck.gl。同时要避免上万 DOM marker 直接渲染。

---

### FB-27-CO-P-007：浏览器合成器线程与主线程的关系

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：合成器线程、主线程、渲染、滚动、事件
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明浏览器合成器线程的作用，以及它如何影响滚动和输入响应。
...
1. 主线程执行 JS、布局、绘制
2. 合成器线程负责将图层合成并显示，可独立于主线程
3. 合成属性动画可在合成器线程运行，不阻塞主线程
4. 滚动通常由合成器线程处理，绑定滚动事件会同步到主线程
5. 减少主线程长任务可提升 INP
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：主线程执行 JS、布局、绘制、合成器线程负责将图层合成并显示，可独立于主线程 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 主线程执行 JS、布局、绘制；合成器线程负责将图层合成并显示，可独立于主线程；合成属性动画可在合成器线程运行，不阻塞主线程；滚动通常由合成器线程处理，绑定滚动事件会同步到主线程；减少主线程长任务可提升 INP。同时要避免认为所有动画都走 GPU。

---

### FB-27-SC-P-002：如何设计性能回归测试

**题型**：场景设计题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：性能测试、Lighthouse、CI、基准、回归
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
请设计前端性能回归测试方案，防止发布导致性能劣化。

**参考答案**：

核心思路如下：

1. 确定基准：LCP/INP/CLS、包体积、请求数
2. 使用 Lighthouse CI 在 MR 阶段跑性能测试
3. 固定测试环境、网络、设备，减少波动
4. 与基线对比，超过阈值阻断合并
5. 监控线上 RUM，发现发布后性能回归
...
在实际落地 设计性能回归测试 时，建议结合 性能测试、Lighthouse、CI 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：确定基准、使用 Lighthouse CI 在 MR 阶段跑性能测试 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 确定基准；使用 Lighthouse CI 在 MR 阶段跑性能测试；固定测试环境、网络、设备，减少波动；与基线对比，超过阈值阻断合并；监控线上 RUM，发现发布后性能回归。同时要避免测试环境不稳定导致结果波动大。

---

### FB-27-PE-A-010：如何减少首屏 JavaScript 体积

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：JS 体积、Tree Shaking、代码分割、Polyfill
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. 代码分割：路由/组件懒加载
2. Tree Shaking 与 sideEffects 配置
3. 按需加载组件库和工具库
4. 移除未使用的 polyfill，按浏览器版本下发
5. 压缩与混淆，使用 Brotli
6. 利用 import() 条件加载
...
在实际落地 减少首屏 JavaScript 体积 时，建议结合 JS 体积、Tree Shaking、代码分割 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：代码分割、Tree Shaking 与 sideEffects 配置 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 代码分割；Tree Shaking 与 sideEffects 配置；按需加载组件库和工具库；移除未使用的 polyfill，按浏览器版本下发；压缩与混淆，使用 Brotli；利用 import() 条件加载。同时要避免整个应用打包成一个 JS。

---

### FB-27-CO-A-016：什么是 Time to Interactive

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：TTI、TBT、可交互、长任务、性能
**出现频率**：中频
**预计回答时长**：3-5 分钟

**题目描述**：
请解释 TTI、TBT 的含义及优化思路。
...
1. TTI：页面可稳定交互的时间
2. TBT：FCP 到 TTI 之间主线程被长任务阻塞的总时间
3. 优化：减少长任务、拆分 JS、延迟非关键脚本、Web Worker
4. TTI 与 INP 关联，长任务多则交互差
5. 使用 Performance 长任务 API 监控
...
**口头回答版**：
> TTI；TBT；优化；TTI 与 INP 关联，长任务多则交互差；使用 Performance 长任务 API 监控。同时要避免只看加载时间不看可交互时间。

---

### FB-27-PE-R-009：前端性能优化的顶层方法论

**题型**：性能优化题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：27 性能优化
**标签**：性能优化、方法论、监控、预算、治理
**出现频率**：中频
**预计回答时长**：8-15 分钟
...
1. 度量：建立 RUM + Lab 监控，定义北极星指标
2. 目标：设定性能预算和 SLO
3. 分析：识别瓶颈，关注 P75/P95 长尾
4. 优化：按加载、渲染、交互、网络分层治理
5. 验证：A/B 测试、回归测试、线上监控
6. 文化：建立性能意识和责任制
...
在实际落地 前端性能优化的顶层方法论 时，建议结合 性能优化、方法论、监控 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：度量、目标 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 度量；目标；分析；优化；验证；文化。同时要避免凭感觉优化。

---

### FB-27-SC-A-010：如何优化电商大促活动页性能
...
**题目描述**：
电商大促 H5 活动页需要承载高并发和复杂交互，请给出性能优化方案。

**参考答案**：

核心思路如下：

1. 静态化：活动页预渲染为 HTML，CDN 强缓存
2. 资源体积：图片压缩、WebP、精灵图/IconFont
3. 懒加载：非首屏模块、楼层、图片按需加载
4. 预加载：倒计时、主会场提前加载核心资源
5. 降级：接口失败展示静态兜底，防止白屏
6. 监控：实时采集 LCP/INP/错误率，快速回滚
...
在实际落地 优化电商大促活动页性能 时，建议结合 大促、活动页、性能 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：静态化、资源体积 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 电商大促 H5 活动页需要承载高并发和复杂交互，静态化；资源体积；懒加载；预加载；降级；监控。同时要避免活动页完全 CSR。

---

### FB-27-PE-P-021：Web Worker 在性能优化中的应用

**题型**：性能优化题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：Web Worker、主线程、计算、性能、Transferable
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明 Web Worker 适合做什么，以及前端如何安全高效地使用。

**参考答案**：

核心思路如下：

1. 适合：计算密集型任务、大数据处理、复杂序列化
2. 不适合：频繁 DOM 操作、需要同步返回的小任务
3. 使用 Comlink 等库简化通信
4. Transferable Objects 减少大数据拷贝
5. 注意 Worker 生命周期和错误处理
...
在实际落地 Web Worker 在性能优化中的应用 时，建议结合 Web Worker、主线程、计算 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：适合、不适合 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 适合；不适合；使用 Comlink 等库简化通信；Transferable Objects 减少大数据拷贝；注意 Worker 生命周期和错误处理。同时要避免所有任务都放 Worker。

---

### FB-27-CO-B-011：懒加载与预加载的区别
...
1. 懒加载：延迟非关键资源加载，减少首屏负担
2. 预加载：提前加载后续需要的资源，提升切换速度
3. 懒加载适合图片、非首屏组件、长列表
4. 预加载适合下一页、下一步流程的关键资源
5. 两者可结合：首屏懒加载非关键，空闲时预加载后续
...
**口头回答版**：
> 懒加载；预加载；懒加载适合图片、非首屏组件、长列表；预加载适合下一页、下一步流程的关键资源；两者可结合。同时要避免对首屏关键资源懒加载。

---

### FB-27-PE-A-011：如何优化 Webpack/Vite 构建速度

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：构建速度、Webpack、Vite、缓存、并行
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 缓存：persistent cache、远程缓存
2. 并行：thread-loader、swc/esbuild 替换 babel
3. 减少 loader/plugin：只处理必要文件
4. 代码分割与 external 大库
5. 升级构建工具：Vite/Rspack 利用原生 ESM
6. 按需构建：只构建 affected 包
...
在实际落地 优化 Webpack/Vite 构建速度 时，建议结合 构建速度、Webpack、Vite 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：缓存、并行 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 缓存；并行；减少 loader/plugin；代码分割与 external 大库；升级构建工具；按需构建。同时要避免没有任何缓存。

---

### FB-27-SC-P-003：如何优化可交互大屏的渲染性能

**题型**：场景设计题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：27 性能优化
**标签**：大屏、可视化、渲染、Canvas、WebGL、性能
**出现频率**：中频
**预计回答时长**：8-15 分钟

**题目描述**：
数据可视化大屏需要展示大量图表和实时数据，请给出性能优化方案。

**参考答案**：

核心思路如下：

1. 图表使用 Canvas/WebGL 渲染，减少 DOM
2. 数据抽稀：按屏幕像素聚合大数据
3. 动画使用 requestAnimationFrame
4. 分页或虚拟化展示表格/列表
5. WebSocket 数据做节流和批量更新
6. 对不变化区域使用缓存位图
...
在实际落地 优化可交互大屏的渲染性能 时，建议结合 大屏、可视化、渲染 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：图表使用 Canvas/WebGL 渲染，减少 DOM、数据抽稀 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 数据可视化大屏需要展示大量图表和实时数据，图表使用 Canvas/WebGL 渲染，减少 DOM；数据抽稀；动画使用 requestAnimationFrame；分页或虚拟化展示表格/列表；WebSocket 数据做节流和批量更新；对不变化区域使用缓存位图。同时要避免大量图表用 SVG/DOM。

---

### FB-27-PE-A-012：关键 CSS 提取与内联策略

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：Critical CSS、内联、渲染阻塞、性能
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
1. 关键 CSS：首屏渲染所需的样式
2. 内联到 HTML head，避免额外请求阻塞
3. 非关键 CSS 异步加载或延迟
4. 使用 Critical 或 Penthouse 工具自动提取
5. 注意内联体积不要过大
...
在实际落地 关键 CSS 提取与内联策略 时，建议结合 Critical CSS、内联、渲染阻塞 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：关键 CSS、内联到 HTML head，避免额外请求阻塞 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 关键 CSS；内联到 HTML head，避免额外请求阻塞；非关键 CSS 异步加载或延迟；使用 Critical 或 Penthouse 工具自动提取；注意内联体积不要过大。同时要避免内联整个 CSS 文件。

---

### FB-27-PE-A-013：如何优化前端资源加载优先级

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：27 性能优化
**标签**：优先级、preload、预加载、关键资源、性能
**出现频率**：高频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明如何通过控制资源优先级提升关键路径性能。

**参考答案**：

核心思路如下：

1. 用 preload 提升 LCP 图片和关键字体优先级
2. 用 prefetch 预加载下一页资源
3. 用 preconnect/dns-prefetch 减少连接耗时
4. defer/async 调整脚本优先级
5. 避免非关键资源阻塞首屏
...
在实际落地 优化前端资源加载优先级 时，建议结合 优先级、preload、预加载 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：用 preload 提升 LCP 图片和关键字体优先级、用 prefetch 预加载下一页资源 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 用 preload 提升 LCP 图片和关键字体优先级；用 prefetch 预加载下一页资源；用 preconnect/dns-prefetch 减少连接耗时；defer/async 调整脚本优先级；避免非关键资源阻塞首屏。同时要避免所有资源都 preload。
