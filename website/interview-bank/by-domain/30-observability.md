...
> 本题库共收录 **78** 道面试题（基础 15 / 进阶 34 / 深入 18 / 架构 11）。
...
### FB-30-CO-B-011：可观测性的三大支柱

**题型**：概念题
**难度**：🟢 基础
**岗位层级**：初级 / 高级
**面试知识域**：30 可观测性
**标签**：可观测、Metrics、Logs、Traces、监控
**出现频率**：高频
**预计回答时长**：3-5 分钟

**题目描述**：
请解释 Metrics、Logs、Traces 的区别与联系。

**参考答案**：

核心思路如下：

1. Metrics：聚合指标，如 QPS、错误率、P99 延迟
2. Logs：离散事件记录，如请求详情、错误堆栈
3. Traces：请求链路，串联跨服务调用
4. 三者关联：trace_id、时间戳、标签统一
5. 可观测性目标是快速定位未知问题
...
**口头回答版**：
> Metrics；Logs；Traces；三者关联；可观测性目标是快速定位未知问题。同时要避免只采集一种数据。

---

### FB-30-CO-A-017：RUM 与 Synthetic 监控的区别

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：RUM、Synthetic、真实用户、模拟、监控
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. RUM：采集真实用户数据，反映实际体验，数据波动大
2. Synthetic：在固定环境定时跑脚本，可复现、稳定
3. RUM 适合发现长尾问题和业务关联
4. Synthetic 适合基线对比、发布前验证、告警
5. 两者结合：Synthetic 守门，RUM 看真实影响
...
**口头回答版**：
> RUM；Synthetic；RUM 适合发现长尾问题和业务关联；Synthetic 适合基线对比、发布前验证、告警；两者结合。同时要避免只看 RUM 忽略 Synthetic。

---

### FB-30-PE-A-012：前端 RUM 指标体系设计

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：RUM、Web Vitals、指标、P75、归因
**出现频率**：高频
**预计回答时长**：8-15 分钟
...
1. 加载：LCP、FCP、TTFB、页面加载时间
2. 交互：INP、FID、点击响应时间
3. 稳定性：JS 错误率、资源加载失败率、API 错误率
4. 业务：白屏时间、关键流程耗时
5. 分位数：P75/P95 而非均值
6. 归因：按页面、版本、网络、设备拆分
...
在实际落地 前端 RUM 指标体系设计 时，建议结合 RUM、Web Vitals、指标 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：加载、交互 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 加载；交互；稳定性；业务；分位数；归因。同时要避免只看均值。

---

### FB-30-EN-A-013：前端错误监控 SDK 设计

**题型**：工程化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：错误监控、SDK、Source Map、上报、聚合
**出现频率**：高频
**预计回答时长**：8-15 分钟
...
1. 全局捕获：window.onerror、unhandledrejection、框架 Error Boundary
2. 上下文：URL、userAgent、版本、用户、trace_id
3. Source Map 还原堆栈
4. 采样与去重：避免相同错误海量上报
5. 聚合：按 fingerprint、版本、页面分组
6. 告警：按错误率、影响面分级
...
**口头回答版**：
> 全局捕获；上下文；Source Map 还原堆栈；采样与去重；聚合；告警。同时要避免只捕获同步错误。

---

### FB-30-SC-A-016：页面白屏如何定位
...
1. 确认范围：个别用户/浏览器/版本还是全部
2. 查看错误监控：JS 执行错误、资源加载失败
3. 查看网络：关键 JS/CSS 是否 404 或被拦截
4. Source Map 还原错误堆栈
5. 检查 CDN、发布回滚、第三方脚本
6. 用户端复现：同浏览器版本、网络环境
...
在实际落地 页面白屏如何定位 时，建议结合 白屏、排查、监控 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：确认范围、查看错误监控 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 确认范围；查看错误监控；查看网络；Source Map 还原错误堆栈；检查 CDN、发布回滚、第三方脚本；用户端复现。同时要避免只看网络不看 JS 错误。

---

### FB-30-CO-P-022：Trace 在前端如何与后端关联

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：30 可观测性
**标签**：Trace、OpenTelemetry、trace_id、链路、关联
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明前端如何实现分布式链路追踪并与后端串联。

**参考答案**：

核心思路如下：

1. 生成 trace_id 和 span_id，注入请求头
2. 使用 OpenTelemetry 或自研 SDK 采集 span
3. 关联：所有请求、资源加载、错误都带 trace_id
4. 后端服务透传 trace_id
5. 前端 trace 与 RUM 数据关联
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：生成 trace_id 和 span_id，注入请求头、使用 OpenTelemetry 或自研 SDK 采集 span 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 生成 trace_id 和 span_id，注入请求头；使用 OpenTelemetry 或自研 SDK 采集 span；关联；后端服务透传 trace_id；前端 trace 与 RUM 数据关联。同时要避免trace_id 只在后端生成。

---

### FB-30-SD-R-032：设计前端全链路可观测平台

**题型**：系统设计题
**难度**：🔴 架构
**岗位层级**：架构师 / 专家
**面试知识域**：30 可观测性
**标签**：可观测、平台、Metrics、Logs、Traces、RUM
**出现频率**：高频
**预计回答时长**：15-30 分钟
...
1. 采集层：RUM、错误、性能、业务埋点、日志
2. 关联层：trace_id、release、user_id 打通
3. 存储层：时序数据库、日志系统、对象存储
4. 展示层：仪表盘、下钻、告警、事件关联
5. 治理：采样策略、数据保留、成本优化
6. 应用：发布回归、故障定位、业务分析
...
在实际落地 设计前端全链路可观测平台 时，建议结合 可观测、平台、Metrics 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：采集层、关联层 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 采集层；关联层；存储层；展示层；治理；应用。同时要避免数据孤岛无关联。

---

### FB-30-CO-A-018：前端日志采集的最佳实践
...
1. 分级：DEBUG/INFO/WARN/ERROR/FATAL
2. 关键字段：时间、页面、版本、用户、trace_id
3. 批量上报，减少请求数
4. 采样：高频日志采样，错误全量
5. 脱敏：不上传敏感信息
6. 本地队列与失败重试
...
**口头回答版**：
> 分级；关键字段；批量上报，减少请求数；采样；脱敏；本地队列与失败重试。同时要避免所有日志实时全量上报。

---

### FB-30-PE-A-013：前端性能监控如何影响性能
...
**题目描述**：
请分析前端监控 SDK 自身对性能的影响及优化。

**参考答案**：

核心思路如下：

1. 影响：额外 JS、网络请求、主线程计算
2. 优化：异步加载 SDK、延迟初始化
3. 采样：只监控部分用户
4. 批量上报、使用 sendBeacon
5. 避免在关键路径执行复杂计算
6. 监控监控 SDK 自身的性能开销
...
在实际落地 前端性能监控如何影响性能 时，建议结合 性能监控、开销、采样 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：影响、优化 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 影响；优化；采样；批量上报、使用 sendBeacon；避免在关键路径执行复杂计算；监控监控 SDK 自身的性能开销。同时要避免监控脚本阻塞首屏。

---

### FB-30-SC-P-023：如何设计前端告警策略

**题型**：场景设计题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：30 可观测性
**标签**：告警、阈值、P75、异常检测、降噪
**出现频率**：中频
**预计回答时长**：8-15 分钟
...
在实际落地 设计前端告警策略 时，建议结合 告警、阈值、P75 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：按业务影响分级、基于 P75/P95 而非均值 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 按业务影响分级；基于 P75/P95 而非均值；动态阈值；告警收敛； on-call 响应与升级策略；定期 review 告警有效性。同时要避免阈值过严导致告警疲劳。

---

### FB-30-CO-A-019：Source Map 在安全与监控中的权衡

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：Source Map、安全、监控、堆栈、构建
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明 Source Map 的使用场景及安全考虑。
...
1. 价值：线上错误还原到源码位置
2. 风险：暴露源码和目录结构
3. 方案：Source Map 不上传公网，只上传内部监控系统
4. 使用 hidden source map
5. 构建时生成并关联版本号
...
**口头回答版**：
> 价值；风险；方案；使用 hidden source map；构建时生成并关联版本号。同时要避免Source Map 公开可访问。

---

### FB-30-EN-A-014：前端埋点方案对比

**题型**：工程化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：埋点、代码埋点、无埋点、可视化埋点、数据
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
1. 代码埋点：精确、可控，但侵入业务代码
2. 无埋点：自动采集所有交互，数据量大、语义弱
3. 可视化埋点：产品运营通过工具配置，无需发版
4. 混合方案：关键事件代码埋点，探索性分析无埋点
5. 数据质量：代码埋点 > 可视化 > 无埋点
...
在实际落地 前端埋点方案对比 时，建议结合 埋点、代码埋点、无埋点 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：代码埋点、无埋点 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 代码埋点；无埋点；可视化埋点；混合方案；数据质量。同时要避免所有埋点都用无埋点导致噪音。

---

### FB-30-SC-A-017：如何监控微前端子应用健康度
...
在实际落地 监控微前端子应用健康度 时，建议结合 微前端、监控、子应用 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：基座统一采集各子应用加载耗时、错误、资源、子应用上报时带上应用名和版本 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 基座统一采集各子应用加载耗时、错误、资源；子应用上报时带上应用名和版本；区分基座错误与子应用错误；注册表与版本维度展示；子应用独立监控也保留。同时要避免所有错误混在一起。

---

### FB-30-CO-P-023：什么是 SLO、SLI、SLA

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：30 可观测性
**标签**：SLO、SLI、SLA、可靠性、目标
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请解释 SLO、SLI、SLA 的含义及在前端的应用。

**参考答案**：

核心思路如下：

1. SLI：服务等级指标，如 LCP P75
2. SLO：目标值，如 LCP P75 ≤2.5s
3. SLA：对外承诺，违反有赔偿
4. 前端 SLI：可用性、错误率、性能指标
5. 设定 SLO 要基于用户影响和数据基础
...
**口头回答版**：
> SLI；SLO；SLA；前端 SLI；设定 SLO 要基于用户影响和数据基础。同时要避免SLO 过高无法实现。

---

### FB-30-PE-P-020：前端异常如何归因到具体发布
...
在实际落地 前端异常如何归因到具体发布 时，建议结合 异常归因、发布、版本 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：构建产物带版本号或 commit hash、错误上报携带版本信息 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 构建产物带版本号或 commit hash；错误上报携带版本信息；监控按版本聚合错误率；发布时间与异常上升时间对比；灰度阶段加大监控粒度；快速回滚验证。同时要避免版本信息缺失。

---

### FB-30-SC-R-030：设计前端监控数据平台架构
...
**题目描述**：
请设计一个前端监控数据平台，支持高性能查询和可视化。

**参考答案**：

核心思路如下：

1. 采集端：SDK 采集 RUM、错误、埋点、日志
2. 传输层：批量压缩、失败重试、采样
3. 存储层：时序 DB 存指标、日志系统存日志、对象存储存原始数据
4. 计算层：实时聚合、离线分析
5. 查询层：多维下钻、漏斗、留存
6. 可视化：仪表盘、告警、报告
...
在实际落地 设计前端监控数据平台架构 时，建议结合 监控平台、数据、采集 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：采集端、传输层 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 采集端；传输层；存储层；计算层；查询层；可视化。同时要避免存储所有原始数据导致成本爆炸。

---

### FB-30-CO-A-020：前端性能预算如何制定

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：性能预算、LCP、包体积、指标、阈值
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 基于当前基线和用户数据设定
2. 指标：JS/CSS 体积、请求数、LCP/INP/CLS
3. 预算分级：严格阻断、警告、观察
4. 纳入 CI 门禁
5. 定期 review 和调整
...
**口头回答版**：
> 基于当前基线和用户数据设定；指标；预算分级；纳入 CI 门禁；定期 review 和调整。同时要避免预算随意设定。

---

### FB-30-EN-A-015：前端 SDK 版本管理与灰度

**题型**：工程化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：监控 SDK、版本、灰度、回滚、采样
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明前端监控 SDK 自身如何灰度和版本管理。
...
在实际落地 前端 SDK 版本管理与灰度 时，建议结合 监控 SDK、版本、灰度 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：SDK 版本化，构建产物带版本、通过配置中心控制采样率和功能开关 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> SDK 版本化，构建产物带版本；通过配置中心控制采样率和功能开关；灰度；错误回滚；自监控。同时要避免所有用户同版本上线。

---

### FB-30-SC-A-018：如何利用监控数据驱动性能优化

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：监控、数据驱动、性能优化、P75、归因
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明如何通过监控数据找到并验证性能优化方向。

**参考答案**：

核心思路如下：

1. 从 P75/P95 找出最差页面和指标
2. 按网络、设备、地域拆分定位问题
3. 关联发布版本找回归
4. 设立优化目标并 A/B 验证
5. 优化后继续监控确认效果
...
在实际落地 利用监控数据驱动性能优化 时，建议结合 监控、数据驱动、性能优化 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：从 P75/P95 找出最差页面和指标、按网络、设备、地域拆分定位问题 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 从 P75/P95 找出最差页面和指标；按网络、设备、地域拆分定位问题；关联发布版本找回归；设立优化目标并 A/B 验证；优化后继续监控确认效果。同时要避免凭感觉优化。

---

### FB-30-CO-P-024：前端可观测与业务埋点的关系

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：30 可观测性
**标签**：可观测、业务埋点、技术指标、业务指标
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 技术可观测：系统运行状态、错误、性能
2. 业务埋点：用户行为、转化、漏斗
3. 两者可共享采集通道和 ID 体系
4. 技术问题可通过业务指标发现，如转化率下降
5. 统一平台展示关联
...
**口头回答版**：
> 技术可观测；业务埋点；两者可共享采集通道和 ID 体系；技术问题可通过业务指标发现，如转化率下降；统一平台展示关联。同时要避免技术监控和业务数据完全隔离。

---

### FB-30-FS-A-001：线上 JS 错误率突增如何排查
...
在实际落地 线上 JS 错误率突增如何排查 时，建议结合 错误率、排查、发布 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：确认时间点和影响范围、关联发布、配置变更、第三方脚本更新 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 确认时间点和影响范围；关联发布、配置变更、第三方脚本更新；查看错误聚合和堆栈；按版本、浏览器、页面拆分；使用 Source Map 还原；回滚或热修复验证。同时要避免不关联发布。

---

### FB-30-PE-A-014：如何监控和优化 LCP
...
1. LCP 元素通常是最大图片或文本块
2. 优化：预加载 LCP 资源、压缩图片、CDN、SSR、减少阻塞资源
3. 监控：web-vitals 库采集，关注 P75
4. 归因：按资源类型、页面、网络拆分
5. 避免布局偏移导致 LCP 元素变化
...
在实际落地 监控和优化 LCP 时，建议结合 LCP、最大内容绘制、优化 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：LCP 元素通常是最大图片或文本块、优化 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> LCP 元素通常是最大图片或文本块；优化；监控；归因；避免布局偏移导致 LCP 元素变化。同时要避免只优化 FCP 忽略 LCP。

---

### FB-30-CO-B-012：什么是 FID 与 INP
...
1. FID：首次输入延迟，只测第一次交互
2. INP：交互到下一次绘制，覆盖所有交互，更全面
3. INP 取代 FID 成为 Core Web Vital
4. 优化：拆分长任务、减少主线程阻塞
5. 目标是 INP ≤200ms
...
**口头回答版**：
> FID；INP；INP 取代 FID 成为 Core Web Vital；优化；目标是 INP ≤200ms。同时要避免只看 FID 忽略 INP。

---

### FB-30-SC-P-024：如何设计监控采样策略
...
**题目描述**：
请设计前端监控的采样策略，平衡成本与数据价值。

**参考答案**：

核心思路如下：

1. 错误全量或高采样，性能 RUM 按用户/会话采样
2. 高流量页面可降低采样率
3. 保留关键用户/核心路径高采样
4. 支持动态调整采样率
5. 采样后数据需能代表总体分布
...
在实际落地 设计监控采样策略 时，建议结合 采样、监控、成本 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：错误全量或高采样，性能 RUM 按用户/会话采样、高流量页面可降低采样率 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 错误全量或高采样，性能 RUM 按用户/会话采样；高流量页面可降低采样率；保留关键用户/核心路径高采样；支持动态调整采样率；采样后数据需能代表总体分布。同时要避免所有数据全量采集。

---

### FB-30-EN-R-001：前端可观测工程的落地路径
...
在实际落地 前端可观测工程的落地路径 时，建议结合 可观测、工程化、落地 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：统一 SDK 和埋点规范、建立北极星指标和 SLO 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 统一 SDK 和埋点规范；建立北极星指标和 SLO；建设可视化平台和告警机制；培训团队使用数据定位问题；将可观测纳入发布流程；持续优化成本和数据质量。同时要避免工具分散无统一规范。

---

### FB-30-CO-B-013：什么是前端埋点
...
在实际落地 前端埋点 时，建议结合 埋点、事件、监控 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：埋点用于记录用户行为和业务事件、类型 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 埋点用于记录用户行为和业务事件；类型；数据用于分析转化、优化产品；需与业务目标对齐；保证数据准确和合规。同时要避免埋点无规范。

---

### FB-30-CO-A-021：前端监控采样策略
...
在实际落地 前端监控采样策略 时，建议结合 采样、监控、RUM 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：错误事件高采样或全量、性能事件按用户/会话采样 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 错误事件高采样或全量；性能事件按用户/会话采样；高流量页面降低采样；保留核心路径高采样；采样后仍能反映分布。同时要避免全量采集成本爆炸。

---

### FB-30-EN-A-016：前端监控 SDK 的加载方式
...
在实际落地 前端监控 SDK 的加载方式 时，建议结合 监控、SDK、异步 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：异步加载，不阻塞首屏、延迟到关键资源加载后初始化 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 异步加载，不阻塞首屏；延迟到关键资源加载后初始化；使用 CDN 分发；失败时降级或重试；配置采样和功能开关。同时要避免同步加载阻塞。

---

### FB-30-SC-A-019：如何利用监控快速定位线上问题

**题型**：场景设计题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：监控、排查、定位、trace、错误
**出现频率**：高频
**预计回答时长**：5-8 分钟
...
在实际落地 利用监控快速定位线上问题 时，建议结合 监控、排查、定位 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：从告警中确认影响范围和时间、查看错误聚合和堆栈 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 从告警中确认影响范围和时间；查看错误聚合和堆栈；按版本、页面、浏览器拆分；通过 trace_id 串联前后端；关联发布和配置变更；验证修复后指标回落。同时要避免不关联发布。

---

### FB-30-PE-A-015：如何监控前端资源加载

**题型**：性能优化题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：资源、Performance、监控、RUM、失败
**出现频率**：中频
**预计回答时长**：5-8 分钟
...
1. 使用 PerformanceObserver 监听 resource timing
2. 采集 JS/CSS/图片加载耗时和失败率
3. 按 CDN、域名、资源类型聚合
4. 识别慢资源和 404
5. 结合 RUM 关联用户影响
...
- 只监控 API 不监控静态资源
- PerformanceObserver 不支持时无兜底
- 忽略资源缓存命中率


**补充说明**：

在实际落地 监控前端资源加载 时，建议结合 资源、Performance、监控 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：使用 PerformanceObserver 监听 resource timing、采集 JS/CSS/图片加载耗时和失败率 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 使用 PerformanceObserver 监听 resource timing；采集 JS/CSS/图片加载耗时和失败率；按 CDN、域名、资源类型聚合；识别慢资源和 404；结合 RUM 关联用户影响。同时要避免只监控 API 不监控静态资源。

---

### FB-30-CO-A-022：日志级别与使用场景

**题型**：概念题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：日志、级别、DEBUG、INFO、ERROR
**出现频率**：中频
**预计回答时长**：3-5 分钟
...
**口头回答版**：
> DEBUG；INFO；WARN；ERROR；FATAL；线上一般采集 WARN 及以上。同时要避免所有日志用 console.log。

---

### FB-30-SC-A-020：前端灰度发布监控
...
在实际落地 前端灰度发布监控 时，建议结合 灰度、监控、发布 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：错误率对比、核心性能指标 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 错误率对比；核心性能指标；业务转化率；资源加载失败率；按灰度组细分；设置自动回滚阈值。同时要避免只放量不看指标。

---

### FB-30-CO-P-025：什么是 OpenTelemetry

**题型**：概念题
**难度**：🟣 深入
**岗位层级**：高级 / 资深
**面试知识域**：30 可观测性
**标签**：OpenTelemetry、可观测、Trace、Metrics、标准化
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请解释 OpenTelemetry 及其对前端可观测的意义。

**参考答案**：

核心思路如下：

1. OpenTelemetry 是开源的可观测标准
2. 统一 Trace、Metrics、Logs 数据采集
3. 提供跨语言、跨平台的 SDK
4. 前端可用 OTel Web SDK 采集
5. 便于与后端链路打通
...
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：OpenTelemetry 是开源的可观测标准、统一 Trace、Metrics、Logs 数据采集 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> OpenTelemetry 是开源的可观测标准；统一 Trace、Metrics、Logs 数据采集；提供跨语言、跨平台的 SDK；前端可用 OTel Web SDK 采集；便于与后端链路打通。同时要避免认为 OTel 只是后端工具。

---

### FB-30-FS-A-002：前端错误归因方法

**题型**：故障排查题
**难度**：🔵 进阶
**岗位层级**：中级 / 高级
**面试知识域**：30 可观测性
**标签**：错误、归因、版本、Source Map、trace
**出现频率**：中频
**预计回答时长**：5-8 分钟

**题目描述**：
请说明如何将线上错误归因到具体代码和发布。

**参考答案**：

核心思路如下：

1. 错误上报携带版本和 commit hash
2. Source Map 还原堆栈到源码
3. 按 fingerprint 聚合
4. 关联发布时间和变更
5. 区分第三方脚本错误
...
在实际落地 前端错误归因方法 时，建议结合 错误、归因、版本 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：错误上报携带版本和 commit hash、Source Map 还原堆栈到源码 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 错误上报携带版本和 commit hash；Source Map 还原堆栈到源码；按 fingerprint 聚合；关联发布时间和变更；区分第三方脚本错误。同时要避免无版本信息。

---

### FB-30-EN-A-017：监控数据存储与成本控制
...
在实际落地 监控数据存储与成本控制 时，建议结合 监控、存储、成本 的真实场景做验证。重点关注可观测性埋点、异常降级路径和性能基线回归；同时通过灰度发布、指标看板和复盘机制持续迭代，确保方案从“能跑”演进为“可维护、可扩展”。
**评分维度**：
- 问题理解与分析（35%）：是否抓住核心矛盾
- 方案设计（45%）：采样减少数据量、分级存储 等关键点
- 落地与优化（20%）：是否考虑监控、回滚、演进
...
**口头回答版**：
> 采样减少数据量；分级存储；聚合指标替代原始日志；设置数据保留期限；按业务重要性差异化。同时要避免永久保存所有原始日志。
