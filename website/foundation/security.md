...
现代前端项目依赖大量 npm 包，一个项目的间接依赖往往有上千个。这些依赖中只要有一个存在恶意代码或漏洞，整个应用都可能被攻陷。供应链攻击不像 XSS 那样直接面向用户，它像从“后方”渗透，更难察觉。

生活化比喻：你开了一家餐厅，食材都从供应商采购。如果某家供应商的原料被污染，所有顾客都会中毒，而问题可能出在千里之外的工厂。
...
`npm audit` 会读取 `package-lock.json` 中的依赖树，与漏洞数据库比对，列出风险等级（low/moderate/high/critical）和修复建议。

### 8.3 lockfile 安全

`package-lock.json` 或 `yarn.lock` 记录了精确的依赖版本，是保证安装一致性的关键。但它也存在安全风险：

- **篡改风险**：如果 lockfile 被恶意修改，可能引入恶意包版本。
- **依赖膨胀**：间接依赖过多，难以人工审计。
- **版本锁定过死**：无法及时获得安全补丁。
...
1. 将 lockfile 纳入版本控制。
2. CI 中校验 lockfile 与 `package.json` 一致。
3. 定期运行 `npm audit` 并升级有漏洞的包。
4. 审查新引入依赖的维护状态、下载量和已知问题。

### 8.4 Prototype Pollution（原型污染）

原型污染是指攻击者通过修改 `Object.prototype`，影响所有对象的默认行为。常见于不安全的对象合并、JSON 解析、URL 参数处理。
...
- 合并前检查键名是否为 `__proto__`、`constructor`、`prototype`。
- 使用 lodash 等库的 `mergeWith` 并自定义安全策略。
- 升级已知存在原型污染漏洞的库。
...
| 实践 | 说明 |
|------|------|
| 最小依赖原则 | 只引入必要的依赖，减少攻击面 |
| 锁定版本 | 用 lockfile 保证一致性 |
| 定期审计 | `npm audit` / Snyk / Dependabot |
| 私有仓库审查 | 对内部包做安全扫描 |
| SRI 校验 | 对 CDN 资源做子资源完整性校验 |
| 依赖图谱可视化 | 使用 `npm ls` 或工具分析依赖树 |

---

## 九、WebAuthn / Passkeys：强认证时代

### 9.1 什么是 WebAuthn？

WebAuthn（Web Authentication）是 W3C 和 FIDO 联盟制定的标准，允许网站使用公钥密码学进行强认证。Passkeys 是 WebAuthn 的一种用户友好实现，通常由操作系统或密码管理器管理，支持跨设备同步。

### 9.2 核心概念

| 概念 | 说明 |
|------|------|
| **Authenticator** | 验证器，如手机、安全密钥、笔记本的 TPM |
| **Relying Party (RP)** | 依赖方，即你的网站 |
| **Credential** | 凭据，包含公钥和私钥句柄 |
| **Challenge** | 服务器生成的随机值，防止重放攻击 |
| **User Verification** | 用户验证，如指纹、面容、PIN |
...
const credential = await navigator.credentials.create({
  publicKey: {
    challenge: new Uint8Array([/* 服务器生成的随机值 */]),
...
// 把 credential.id 和公钥发送给服务器保存

// 认证（使用凭据）
const assertion = await navigator.credentials.get({
  publicKey: {
    challenge: new Uint8Array([/* 新随机值 */]),
    allowCredentials: [{ id: credentialId, type: "public-key" }],
    userVerification: "preferred"
  }
});

// 把 assertion 发送给服务器验证签名
```

### 9.4 WebAuthn 的安全优势

- **抗钓鱼**：凭据绑定 RP ID，无法在其他域名使用。
- **无共享密钥**：服务器不存密码，只存公钥。
- **抵抗重放攻击**：每次认证都有新的 challenge。
- **设备级安全**：私钥通常存储在硬件安全模块或操作系统密钥链中。

### 9.5 前端落地建议

- 检测浏览器支持：`if (window.PublicKeyCredential)`。
- 同时保留用户名/密码登录作为降级。
- 对 Passkeys 注册失败给出友好提示。
- 服务器端必须正确验证 challenge、origin、signature 等字段。
...
7. **供应链安全** 要求定期审计依赖、保护 lockfile、防范原型污染。
8. **WebAuthn / Passkeys** 代表了强认证的未来方向，能显著降低密码泄露和钓鱼风险。
9. **前端安全开发规范** 贯穿日常编码，从输入输出到 Cookie、依赖、响应头都需要谨慎处理。
